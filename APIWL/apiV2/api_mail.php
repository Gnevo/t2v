        ...
        foreach ($obj as $key => $mails) {
            ...
            if (!empty($attachments)) {
                ...
                if (empty($obj[$key]['attachments'])) {
                        ...
                        if ($attachment->ids_mail !== '' && $attachment->ids_mail !== null) {
                            ...
                            if (in_array($mails['id'], $ids)) {
                                if (file_exists($mailAttachFolder . $attachment->origin_file_name)) {
                                    ...
                                    $attachments[$item]['path'] = $preference['url'] . '/' . $folder . '/mail_attatch/' . $fileExtension = strtolower(end(explode('.', $attachment->origin_file_name)));
                                }
                    ...
                    if (is_array($obj[$key]['attachments'])) {
                        $attachments = $obj[$key]['attachments'];
                    } else {
                        $attachments = explode(',', (string)$obj[$key]['attachments']);
                    }
                    ...
                    foreach ($attachments as $index => $attachment) {
                        ...
                        $attachments[$index]['path'] = $preference['url'] . '/' . $folder . '/mail_attatch/' . $attachment;
                ...
                if ($attachment['ids_mail'] !== '' && $attachment['ids_mail'] !== null) {
                    ...
                    if (in_array($mail_id, $ids)) {
                        if (file_exists($mailAttachFolder . $attachment['id'] . '.' . $fileExtension)) {
                            $attachments[$item]['title'] = $attachment['origin_file_name'];
                            $attachments[$item]['path'] = $preference['url'] . '/' . $folder . '/mail_attatch/' . $attachment['id'] . '.' . $fileExtension;
                        } else {
            ...
            foreach ($attachments as $key => $attachment) {
                ...
                foreach ($mailAttachments as $mailAttachment) {
                    ...
                    if (substr($mailAttachment['ids_mail'], 0, -1) == $mail_id) {
                        ...
                        $attachments[$key]['path'] = $preference['url'] . '/' . $folder . '/mail_attatch/' . $mailAttachment['id'] . '.' . $fileExtension;
