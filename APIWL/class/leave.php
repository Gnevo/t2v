...
//                                                if ($slot_period['customer'] != '' && $_REQUEST['saveTimeslot'] == 1) 
    ...
    function get_employees_in_charge($employee, $customer, $email_check = TRUE){
        $obj_company = new company();
        $role_hierarchy = array(1=>1,2=>7,3=>2,4=>3);
        $employees_on_customer = $this->get_employees_on_customer($employee,$customer);
        if(!$employees_on_customer) {
            $employees_on_customer = array();
        }
        ...
        if($employee_role){
            ...
            for($i=$employee_role_key-1; $i>1; $i--){
                ...
                if($in_charge_key_on_list){
                    ...
                    if($in_charge_details['email'] || $email_check == FALSE){
    ...
    function update_user_task($user_id, $status, $candg, $customer, $break_time, &$display_message = '',$page = 'employee') {
        //candg =0 with slots candg=1 without slots
        ...
        if ($status == "0") {
            ...
            if (count($task_slots) <= 0 && $candg == 0) {
                ...
                if ($check_report_sign) {
                    if ($check_leave) {
                        ...
                        //echo "<pre>".print_r($slot_det,1)."</pre>";exit();
                        if (!empty($slot_det)) {
                            $slot_date_time = strtotime($slot_det['time_from']);
                            $slot_date_time_to = strtotime($slot_det['time_to']);
                            $time_to = date('G.i', strtotime($slot_det['time_to']));
                            if ($time_to == 0.00)
                                $time_to = 24.00;
                            if($candg == 0 && $current_date_time >= $slot_date_time_to){
                                return -5;
                            }
                            if (abs($current_date_time - $slot_date_time) >= 60) { // checkingg whether there is a difference of 5 minutes with start time
                                //echo date("Y-m-d H:i:s", $current_date_time)."-";
                                //echo date("Y-m-d H:i:s", $slot_date_time);
                                if ($current_date_time < $slot_date_time) {
                                    $display_message = str_replace("{{MINUTES}}", round((($slot_date_time - $current_date_time)/60),2), $smarty->translate['cg_early_starting_msg_to_user']);
                              
                                }elseif($current_date_time > $slot_date_time && $current_date_time < $slot_date_time_to){
                                    //echo "2";exit();
                                    $display_message =str_replace("{{MINUTES}}", round((($current_date_time - $slot_date_time)/60),2), $smarty->translate['cg_late_starting_msg_to_user']);
                                ...
                                if ($flag == 1 && $second_flag == 1) {
                                    if ($current_date_time > strtotime($slot_det['time_from'])) {
                                        ...
                                        if (!$this->query_update()) {
                                            $flag = 0;
                                        }elseif(!$this->slot_add_custom_type('', $customer, $slot_det['date'], date('G.i',strtotime($slot_det['time_from'])), date('G.i',strtotime($time_from)), $login_user, $slot_det['fkkn'], $slot_det['type'], '', null, 0)){
                                            $flag = 0;
            ...
            $round_start = $previous_end;
            $round_stop = strtotime($stop_time);
            ...
            if($check_report_sign && $check_leave){
                ...
                if ($candg == 1 && empty($datas)) {
                    ...
                    if (strtotime($stop_time) - $previous_end >= 60) {
                    ...
                    if($candg == 0){
                            ...
                            foreach ($datas as $slot_between_dets) {
                                ...
                                if($temp_i == count($datas) &&
                                    ...
                                    $obj_equipment->time_difference(date('G.i',$temp_slot_end), date('G.i',$round_stop)) >= $break_time){
                                        ...
                                        $this->tables = array('timetable');
                                        $this->fields = array('time_to','status', 'created_status');
                                        $this->field_values = array(date('G.i',$round_stop), 4,1);
                                        $this->conditions = array('id = ?');
                                        $this->condition_values = array($slot_between_dets['id']);
                                        if (!$this->query_update()) {
                                            $flag = 0;
                                            break;
                                        }elseif(!$this->slot_add_custom_type('', $customer, $slot_between_dets['date'], date('G.i',$round_stop), $slot_between_dets['time_to'], $login_user, $slot_between_dets['fkkn'], $slot_between_dets['type'], '', null, 0)){
                                            $flag = 0;
                                            break;
                                        }else{
                                    ...
                                    $obj_equipment->time_difference($slot_between_dets['slot_end'], $round_stop) >= $break_time){
                                        $display_message = str_replace("{{MINUTES}}", round(($round_stop - strtotime($slot_between_dets['slot_end'])),2)/60, $smarty->translate['cg_late_going_msg_to_user']);
                                    }
                                    $this->tables = array('timetable');
                        ...
                        foreach ($datas as $row) {
                            ...
                            if(strtotime($stop_time) > $slot_start && (strtotime($stop_time) - $slot_start) >= 60){
                                $this->tables = array('timetable');
                            ...
                            if (strtotime($stop_time) <= $slot_end) {
                                ...
                                // if($_SESSION['user_id'] == 'cifo001')
                                //     echo "1";
                                if ($slot_end - strtotime($stop_time) >= 60) {
                    ...
                    if ($candg_flag == 0) {
                        ...
                        if (strtotime($stop_time) > $slot_end || $slot_end == '') {
                            if ($slot_end != '') {
                                ...
                                $round_start = strtotime($start_date_time);
                                $round_stop = strtotime($stop_time);
...
//        $this->field_values = array($tag_id, $local_sms_id, $_SESSION['company_id'], date('Y-m-d H:i:s'), round(microtime(true) * 1000));
    ...
    function get_untreated_leave_slots($this_employee, $year, $month) {
        $this->flush();
        $this->sql_query = "SELECT t.id, t.employee, t.customer, t.time_from, t.time_to, t.status FROM timetable t LEFT JOIN `leave` l ON t.date=l.date AND l.employee=t.employee AND month(t.date)='".$month."' AND year(t.date)='".$year."' AND l.employee='".$this_employee."' and t.time_from >= l.time_from AND t.time_to <=l.time_to WHERE month(t.date) ='".$month."' AND year(t.date) ='".$year."' AND t.employee LIKE '".$this_employee."' AND t.status = 2 AND l.status = 0 ORDER BY t.time_from DESC";
...
//          $max_start_time->add(new DateInterval('PT15M'));
    ...
    function remove_leave_from_leave_tbl_by_group($gid, $employee, $date) {
        //echo $gid . '-' . $employee . '-' .$date . ';';
        $this->tables = array('leave');
        $this->conditions = array('AND', 'group_id = ?', 'employee = ?', 'date = ?');
        $this->condition_values = array($gid, $employee, $date);
    ...
    function customer_employee_slot_remove_for_leave($id, $is_multiple_slots = FALSE) {
        $this->tables = array('timetable');
