...
if($action == 'emploee_add'){
    ...
    if(!empty($slot_ids)){
        foreach ($slot_ids as $slot_id) {
            if($slot_id == '') continue;
            $slot_det = $obj_tmp->customer_employee_slot_details($slot_id);
            ...
            if(!$obj_tmp->findout_slot_alteration_bug($process_params)){
                $process_flag = false;
                break;
            }
        }
    ...
    if($process_flag){
        if($obj_tmp->employee_add_to_slot_multiple($ids, $select_emp, $alloc_emp))
...
/*else if($action == 'customer_add'){ // changing customer on slot.
