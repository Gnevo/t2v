...
//    $leave_details  = $employee->get_leave_details_byID($employee->leave_id);
    ...
    $date_from = substr($leave_details[0]['From_date'],0,10); //$leave_details[0]['leave_date'];
    $time_from = substr($leave_details[0]['From_date'], 11); //$leave_details[0]['time_from'];
    $date_to = substr($leave_details[0]['To_date'],0,10); //$leave_details[0]['leave_date'];
    $time_to = substr($leave_details[0]['To_date'], 11); //$leave_details[0]['time_to'];
...
if($transaction_flag){
    if($employee->update_leave_status()){
        $obj->result = TRUE;
        $obj->message = ($employee->leave_status == '2' ? $smarty->translate['leave_reject_success'] : $smarty->translate['leave_accept_success']);
        ...
        $obj->result = FALSE;
        $obj->message = ($employee->leave_status == '2' ? $smarty->translate['leave_reject_fail'] : $smarty->translate['leave_accept_fail']);
    }
} else
    $obj->result = FALSE;
