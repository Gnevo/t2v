    ...
    //if($_SESSION['user_id'] == $user->username && !empty($cur_user)){
            ...
            if($headers[1]['X-Diglias-Location']){
              //http://192.168.0.234/works/app/t2v/cirrus-r/report/work/employee/detail/2016/9/diks001/ipix001/
                ...
                file_put_contents('sign.txt', $headers[1]['X-Diglias-Location']);
                //echo '<script>window.location.href="'.$headers[1]['X-Diglias-Location'].'&sign_returnurl='.$smarty->url.'apiV2/api_sign_back_url.php"</script>';
                //echo '<script>window.location.href="https://www.time2view.se/"</script>';
                header('Location:'.$headers[1]['X-Diglias-Location'].'&sign_returnurl='.$smarty->url.'apiV2/api_sign_back_url.php');exit();
            }
            else{
