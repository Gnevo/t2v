...
$selected_employee = trim($_REQUEST['user']);

if($start_date != NULL && $end_date != NULL && $selected_employee != ''){
    // $data = $leave->get_employee_schedule_between_dates($selected_employee, $start_date, $end_date);
    $data = $obj_timetable->employee_slots_btwn_dates($selected_employee, $start_date, $end_date);
}else 
    $data = array();
...
if(!empty($data)){
    ...
    foreach($data as $slot) {
        if ($_SESSION['user_role'] != 3 || ($_SESSION['user_role'] == 3 && ($privileges_gd['not_show_employees']) == 0 || ($privileges_gd['not_show_employees'] == 1 && $slot['employee'] == $_SESSION['user_id']))){
            ...
            // $prev_week = $slot['week'];
            if($slot['customer'] != '')
                $listed_slot_customers[] = $slot['customer'];
        }
    }
}
...
if(!empty($listed_slot_customers)){
    $listed_slot_customers = array_unique($listed_slot_customers);
    foreach ($listed_slot_customers as $listed_slot_customer) {
        $__privileges_gd = $obj_employee->get_privileges($_SESSION['user_id'], 1, $listed_slot_customer);
        if(empty($__privileges_gd)) $__privileges_gd = new stdClass();
        else{ $__privileges_gd['customer'] = $listed_slot_customer; }

        $privilege_customers_set[] = $__privileges_gd;
    }
}
