...
if($sel_year != NULL && $sel_month != NULL){
    $data = $leave->get_all_employee_leave($sel_year, $sel_month, $_REQUEST['start']);
//    echo "<pre>".print_r($data, 1)."</pre>";
    if(!empty($data)){
        foreach($data as $leave_details) {
                ...
                $j = 0;
                $slot_details = "";
                $obj[$i]->id = $leave_details['group_id'];
                ...
                while(strtotime($date) <= strtotime($leave_details['end_date'])){
                    ...
                    foreach($slots as $slot) {
                            ...
                            $slot_details[$j]->first_name = $user_details['first_name'];
                            $slot_details[$j]->last_name = $user_details['last_name'];
