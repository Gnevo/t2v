...
//        print_r($data);
    ...
    function Customer_pdf_work_report($customer, $month, $year, $fkkn, $this_bargaining = NULL, $txt_other_bargaining = NULL, $this_agreement = array(), $employee = Null, $sam_sida = Null, $provider_of_pa = NULL, $obj_pdf = NULL) {
        ...
        for ($i = 0; $i < count($employee_names); $i++) {
            ...
            $work_partition = array_chunk($work_details, 40);   //chunk data in to 31 rows, because max 31 entries included @ one page
            foreach ($work_partition as $works) {
                ...
                $pagecount = $pdf->setSourceFile('./pdf_forms/FK3059_015_F_001__16_08_25.pdf');
                $pdf->AddPage();
        ...
        if($obj_pdf != NULL && is_object($obj_pdf)){
            ...
            // $pagecount = $pdf->setSourceFile('./pdf_forms/FK3057_015_F_001__16_08_25.pdf');
            $pagecount = $pdf->setSourceFile('./pdf_forms/FK3057_016_F_001__17_05_02.pdf');
            $tppl = $pdf->importPage(1);
            $pdf->useTemplate($tppl, -2, 0, 210);
...
//            $this->group_by = array('date', 'time_from', 'time_to');
    ...
    function Employee_contract_pdf($employee = Null, $id = Null, $customer = Null) {
        ...
        //echo "<pre>".print_r($obj_emp->company_data(),1)."</pre>";
...
//        global $preference;
        ...
        if(!empty($chunked_summary_data)){
            foreach($chunked_summary_data as $sdata){
                $pdf->AddPage();        //page3
                $pagecount = $pdf->setSourceFile('./pdf_forms/Template_Arbgivarintyg.pdf');
                $tppl = $pdf->importPage(3);
                $pdf->useTemplate($tppl, -6, 0, 210);
                $pdf->P3_SubPart1($emp_details);
                if(!empty($sdata)){
                    $pdf->P3_SubPart2($sdata, $this->oncall_kr_times, $this->ob_kr_times);
                }
                $pdf->P3_SubPart3($company_details);
            }
        }
...
//        $this->group_by = array('year(date)', 'month(date)');
        ...
        $this->tables = array('timetable');
        $this->fields = array('DISTINCT month(date) as month', 'year(date) as year', 'employee as emp', "(" . $work_hour . ") as work_hours", "(" . $leave_hour . ") as leave_hours", "(" . $filling_hour . ") as filling_hours");
//        if ($year != $end_year) {
...
//        echo "<pre>".print_r($this->condition_values, 1)."</pre>";
        $this->query_generate();
//        if ($year == '2015'){
//            echo "<pre>" . $this->sql_query . "<br />" . "</pre>"; 
//            echo "<pre>".print_r($this->condition_values, 1)."</pre>";
//            exit();
//        }
...
//        echo "<pre>".print_r($datas, 1)."</pre>"; //exit();
        ...
        $data_work_hour_indexed = $data_leave_hour_indexed = $data_filling_hour_indexed = array();
        ...
        /*if ($year == '2015'){
            echo "data_work_hour<pre>".print_r($data_work_hour_indexed, 1)."</pre>";
            echo "data_leave_hour<pre>".print_r($data_leave_hour_indexed, 1)."</pre>";
            echo "data_filling_hour<pre>".print_r($data_filling_hour_indexed, 1)."</pre>";
            echo "datas<pre>".print_r($datas, 1)."</pre>";
            exit();
        }*/
    ...
    function Financial_payment_pdf() {
        ...
        //echo "---------------------<pre>qualifying_day".print_r($qualifying_leave_works__, 1)."</pre>";
        ...
        if ($all_leave_works) {
            if ($qualifying_leave_works && $qualifying_leave_works[0]['date'] != $all_leave_works[0]['date']) {   // this block is taken only when first qualifying day not equal to first leave day
                ...
                //$date_14 = date('Y-m-d', strtotime($date1 . ' +13 day'));
                $incov_details = $leave_times_15_90 = array();
                
                if(!isset($qualifying_leave_works__['first_day_category']) || $qualifying_leave_works__['first_day_category'] == '' || $qualifying_leave_works__['first_day_category'] == '15-90'){
                    $leave_times_15_90 = $this->get_employee_leaved_works_sum_between_two_dates($this->leave_employee, $this->leave_customer, $date1, $date2, NULL);
                    $leave_15_90_periods[] = array('from' => $date1, 'to' => $date2);
                }
                else if($qualifying_leave_works__['first_day_category'] == '2-14'){
                    $starting_day_number = $qualifying_leave_works__['first_day_number'];
                    $day_2_14_remaining_no_of_days = 14-$starting_day_number;
                    $day_2_14_end_date = date('Y-m-d', strtotime($date1 . ' +'.$day_2_14_remaining_no_of_days.' day'));
                    
                    if($day_2_14_end_date <= $date2){
                        $incov_details = $this->get_employee_inconv_leaved_works_between_two_dates_copy($this->leave_employee, $this->leave_customer, $date1, $day_2_14_end_date, NULL, $company_details);
                        if($day_2_14_end_date < $date2){
                            $first_date_15_90 = date('Y-m-d', strtotime($day_2_14_end_date . ' +1 day'));
                            $leave_times_15_90 = $this->get_employee_leaved_works_sum_between_two_dates($this->leave_employee, $this->leave_customer, $first_date_15_90, $date2, NULL);
                            $leave_15_90_periods[] = array('from' => $first_date_15_90, 'to' => $date2);
                        }
                ...
                /*if($date_14 <= $date2){
                    ...
                    $incov_details = $this->get_employee_inconv_leaved_works_between_two_dates_copy($this->leave_employee, $this->leave_customer, $date1, $date_14, NULL, $company_details);
                    if($date_14 < $date2){
                        $first_date_15_90 = date('Y-m-d', strtotime($date_14 . ' +1 day'));
                        $leave_times_15_90 = $this->get_employee_leaved_works_sum_between_two_dates($this->leave_employee, $this->leave_customer, $first_date_15_90, $date2, NULL);
                    }
                ...
                $ord_table_sum = $pdf->get_P2_table1_sum($date1, $date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, array(), $leave_times_15_90);
                $this->make_Financial_payment_pdf_table_header($pdf, $date1, $date2, $customer_details, $company_details, $customer_gardian_details, $emp_details, array(), $all_leave_works, $this->reference[0], $ord_table_sum);
                $pdf->AddPage();
                $pdf->P2_top($this->collective);
                $pdf->P2_table1($date1, $date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, array(), $leave_times_15_90);
                $this->make_Financial_payment_pdf_table_footer($pdf, $date1, $date2, $additional_overlaped_day_karenses, $Q_work_dates, $company_details, $leave_15_90_periods);
                $flag = 1;
                ...
                // echo '<pre>'.print_r($qualifying_leave_works__, 1).'</pre>';
                $incov_details = $leave_times_15_90 = array();
                if(!isset($qualifying_leave_works__['first_day_category']) || $qualifying_leave_works__['first_day_category'] == '' || $qualifying_leave_works__['first_day_category'] == '15-90'){
                    
                    // echo '<pre>'.print_r(array($this->leave_employee, $this->leave_customer, $date1, $date2), 1).'</pre>';
                    $leave_times_15_90 = $this->get_employee_leaved_works_sum_between_two_dates($this->leave_employee, $this->leave_customer, $date1, $date2, NULL);
                    // echo '<pre>'.print_r($leave_times_15_90, 1).'</pre>';
                    $leave_15_90_periods[] = array('from' => $date1, 'to' => $date2);
                }
                else if($qualifying_leave_works__['first_day_category'] == '2-14'){
                    $starting_day_number = $qualifying_leave_works__['first_day_number'];
                ...
                /*$incov_details = $this->get_employee_inconv_leaved_works_between_two_dates_copy($this->leave_employee, $this->leave_customer, $date1, $date2, NULL, $company_details);
                //echo "incov_details <pre>".print_r($incov_details, 1)."</pre>";
                
                $date1 = $this->leavePeriod_year . "-" . $this->leavePeriod_month . "-15";
                $num = cal_days_in_month(CAL_GREGORIAN, $this->leavePeriod_month, $this->leavePeriod_year);
                $date2 = $this->leavePeriod_year . "-" . $this->leavePeriod_month . "-" . $num;
                $leave_times_15_90 = $this->get_employee_leaved_works_sum_between_two_dates($this->leave_employee, $this->leave_customer, $date1, $date2, NULL);
                //echo "leave_times_15_90<pre>".print_r($leave_times_15_90, 1)."</pre>";
                */
                $table_no += 1;
                //$referenses_usage_count++;
                $ord_table_sum = $pdf->get_P2_table1_sum($date1, $date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, array(), $leave_times_15_90);
        ...
        if (count($qualifying_leave_works) > 1) {
            for ($i = 0; $i < count($qualifying_leave_works); $i++) {
                ...
                $date_14 = date('Y-m-d', strtotime($date1 . ' +12 day'));
                $incov_details = $leave_times_15_90 = array();
                if($date_14 <= $date2){
                    ...
                    if($date_14 < $date2){
                        $first_date_15_90 = date('Y-m-d', strtotime($date_14 . ' +1 day'));
                        $leave_times_15_90 = $this->get_employee_leaved_works_sum_between_two_dates($this->leave_employee, $this->leave_customer, $first_date_15_90, $date2, $this_qday_exceeded_karense_id);
                        $leave_15_90_periods[] = array('from' => $first_date_15_90, 'to' => $date2);
                    }
                ...
                $ord_table_sum = $pdf->get_P2_table1_sum($table_period_date1, $table_period_date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, $qualifying_day_total_hour, $leave_times_15_90);
                $this->make_Financial_payment_pdf_table_header($pdf, $table_period_date1, $table_period_date2, $customer_details, $company_details, $customer_gardian_details, $emp_details, array($qualifying_leave_works[$i]['date']), $all_leave_works, $this->reference[$referenses_usage_count+$i], $ord_table_sum);
                $pdf->AddPage();
                $pdf->P2_top($this->collective);
                $pdf->P2_table1($table_period_date1, $table_period_date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, $qualifying_day_total_hour, $leave_times_15_90);
                $this->make_Financial_payment_pdf_table_footer($pdf, $table_period_date1, $table_period_date2, $additional_overlaped_day_karenses, $Q_work_dates, $company_details, $leave_15_90_periods);
                
                $total_ordinarie_time = $this->calculate_total_time_from_dataset_4_Financial_payment_footer($all_leave_works, $table_period_date1, $table_period_date2, $equipment);
                $pdf->P2_table3($total_ordinarie_time);
                $pdf->P2_bottom();
            }
        }
            ...
            if (count($qualifying_leave_works) == 1) {
                // echo '<pre>'.print_r($qualifying_leave_works, 1).'</pre>';
                ...
                //echo "<pre>".print_r($qualifying_day_total_hour, 1)."</pre>";
                $date_14 = date('Y-m-d', strtotime($date1 . ' +12 day'));
                // echo '<pre>'.print_r(array($date1, $date2, $date_14), 1).'</pre>';
                $incov_details = $leave_times_15_90 = array();
                if($date_14 <= $date2){
                    $incov_details = $this->get_employee_inconv_leaved_works_between_two_dates_copy($this->leave_employee, $this->leave_customer, $date1, $date_14, $this_qday_exceeded_karense_id, $company_details);
                    if($date_14 < $date2){
                ...
                $ord_table_sum = $pdf->get_P2_table1_sum($qualifying_leave_works[0]['date'], $date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, $qualifying_day_total_hour, $leave_times_15_90);
                $this->make_Financial_payment_pdf_table_header($pdf, $qualifying_leave_works[0]['date'], $date2, $customer_details, $company_details, $customer_gardian_details, $emp_details, array($qualifying_leave_works[0]['date']), $all_leave_works, $this->reference[$referenses_usage_count], $ord_table_sum);
                $pdf->AddPage();
                $pdf->P2_top($this->collective);
                
                $pdf->P2_table1($qualifying_leave_works[0]['date'], $date2, $this->insurance_word_person, $this->SS_contibution, $incov_details, $qualifying_day_total_hour, $leave_times_15_90);
                $this->make_Financial_payment_pdf_table_footer($pdf, $qualifying_leave_works[0]['date'], $date2, $additional_overlaped_day_karenses, $Q_work_dates, $company_details, $leave_15_90_periods);
                
                $total_ordinarie_time = $this->calculate_total_time_from_dataset_4_Financial_payment_footer($all_leave_works, $qualifying_leave_works[0]['date'], $date2, $equipment);
                $pdf->P2_table3($total_ordinarie_time);
                $pdf->P2_bottom();
            } 
            else if ($flag == 0) {
                $ord_table_sum = $pdf->get_P2_table1_sum(NULL, NULL, $this->insurance_word_person, $this->SS_contibution, array(), array());
                $this->make_Financial_payment_pdf_table_header($pdf, NULL, NULL, $customer_details, $company_details, $customer_gardian_details, $emp_details, array(), $all_leave_works, $this->reference[0], $ord_table_sum);
                $pdf->AddPage();
                $pdf->P2_top($this->collective);
                $pdf->P2_table1(NULL, NULL, $this->insurance_word_person, $this->SS_contibution, array(), array());
                $this->make_Financial_payment_pdf_table_footer($pdf, NULL, NULL, $additional_overlaped_day_karenses, $Q_work_dates, $company_details, $leave_15_90_periods);
    ...
    function make_Financial_payment_pdf_table_header($pdf, $date1, $date2, $customer_details, $company_details, $customer_gardian_details, $emp_details, $Q_work_dates, $all_leave_works, $reference, $ord_table_sum){
        ...
        if ($list_relations_partition) {
            for ($p = 0; $p < count($list_relations_partition); $p++) {
                ...
                //echo "emp_details<pre>".print_r($emp_details, 1)."</pre>";
                //echo "customer_gardian_details<pre>".print_r($customer_gardian_details, 1)."</pre>";
                ...
                $pdf->P1_table_4($emp_details, $Q_work_dates, $all_leave_works, $leave_date_starts, $leave_date_ends);
    ...
    function make_Financial_payment_pdf_table_footer($pdf, $date_start, $date_end, $additional_overlaped_day_karenses, $Q_work_dates, $company_details, $leave_15_90_periods = array()){
        ...
        // echo '<pre>'.print_r($leave_15_90_periods, 1).'</pre>';
        ...
        if ($group_vikaries) {
            ...
            for ($i = 0; $i < count($group_vikaries); $i++) {
                ...
                $inconv_details_15_90 = array('inconv_details' => array());
                ...
                if ($vikarie_age[0]['age'] < 25)
                    $vikarie_sociala = 31.42;// 15.49;
                else if ($vikarie_age[0]['age'] < 65)
                    $vikarie_sociala = 31.42;
                else if ($vikarie_age[0]['age'] >= 65)
                    $vikarie_sociala = 16.36;//31.42// 15.49;
                for ($j = 0; $j < count($group_vikaries[$i]); $j++) {
                    $date1 = $group_vikaries[$i][$j]['date'];
                    $date2 = $group_vikaries[$i][$j]['date'];

                    $is_15_90_day = FALSE;
                    if(!empty($leave_15_90_periods)){
                        foreach($leave_15_90_periods as $leave_15_90_period){
                            if($leave_15_90_period['from'] <= $date1 && $leave_15_90_period['to'] >= $date1){
                                $is_15_90_day = TRUE;
                                break;
                            }
                        }
                    ...
                    if (!empty($grouped_additional_overlaped_day_karenses_ids) && in_array($group_vikaries[$i][$j]['relation_id'], $grouped_additional_overlaped_day_karenses_ids)) {
                        ...
                        if($is_15_90_day){
                            ...
                            if (!empty($this_incov_details['inconv_details']) && !empty($inconv_details_15_90['inconv_details'])) {
                                foreach ($this_incov_details['inconv_details'] as $key => $time_values) {
                                    foreach ($time_values as $inc_salary => $h_values) {
                                        if (isset($inconv_details_15_90['inconv_details'][$key][$inc_salary])) {
                                            $inconv_details_15_90['inconv_details'][$key][$inc_salary]['hour'] = $equipment->time_sum($inconv_details_15_90['inconv_details'][$key][$inc_salary]['hour'], $h_values['hour']);
                                        } else {
                                            $inconv_details_15_90['inconv_details'][$key][$inc_salary]['hour'] = $h_values['hour'];
                                        }
                                    }
                                }
                            } else if (!empty($this_incov_details['inconv_details']))
                                $inconv_details_15_90['inconv_details'] = $this_incov_details['inconv_details'];
                        }
                        // 2-14 day slots
                        else {
                            //echo "<pre>this_incov_details: ".print_r($this_incov_details, 1)."</pre>";
                ...
                // echo '<pre>'.print_r($inconv_details_15_90, 1).'</pre>';
                $pdf->P2_table2($group_vikaries[$i][0]['employee'], $vikaries_normal_salary, $vikarie_insurance, $incov_details, $qualifying_day_total_hour, $vikarie_sociala, $inconv_details_15_90);
                //echo '<br>**********************************************************<br>';
            }
        }
        else
    ...
    function calculate_total_time_from_dataset_4_Financial_payment_footer($all_leave_works, $date1, $date2, $equipment){
        ...
        if(!empty($all_leave_works) && $date1 !== NULL && $date2 !== NULL){
            foreach ($all_leave_works as $ord_work) {
                if($ord_work['date'] >= $date1 && $ord_work['date'] <= $date2){
                    $total_ordinarie_time = $equipment->time_sum($total_ordinarie_time, $equipment->time_difference($ord_work['time_to'], $ord_work['time_from']));
                }
            }
        }
            ...
            foreach ($leave_employees as $this->leave_employee) {
                ...
                if (count($qualifying_leave_works) > 1) {
                    for ($i = 0; $i < count($qualifying_leave_works); $i++) {
                        ...
                        if($i+1 != count($qualifying_leave_works)){
                            $pdf->footer_section();
                            $pdf->AddPage();
                            $pdf->report_top($emp_details, $customer_contract_details);
                            $pdf->section2($customer_details, $this->leavePeriod_year, $this->leavePeriod_month);
...
//            echo "<pre>Inconvenient slots: ".print_r($inconv_normal_slots, 1)."</pre>";
    ...
    function get_employee_inconv_leaved_works_between_two_dates_for_vikarie_copy($work_data, $company_details = array()) {
         ...
         */
//        echo "work_data<pre>".print_r($work_data, 1)."</pre>";
        $inconvenient_process_obj = new inconvenient();
        $equipment = new equipment();
...
//                    echo "<pre>oncall_type_salaries".print_r($oncall_type_salaries, 1)."</pre>";
                    if (!empty($oncall_type_salaries)) {    //check already exist the salary key
                        foreach ($oncall_type_salaries as $salary_key => $values) {
                            if (!isset($additional_oncall_exceeded_salaries[$salary_key])) {
                                $additional_oncall_exceeded_salaries[$salary_key] = array();
                            }
                            if (isset($additional_oncall_exceeded_salaries[$salary_key][$values['amount']]) && !empty($additional_oncall_exceeded_salaries[$salary_key][$values['amount']])) {
                                $additional_oncall_exceeded_salaries[$salary_key][$values['amount']]['hour'] = $equipment->time_sum($additional_oncall_exceeded_salaries[$salary_key][$values['amount']]['hour'], $values['hour']);
...
//        echo "<pre>".print_r($leave_works, 1)."</pre>";
...
//        echo "<pre>".print_r($previous_month_leave_works, 1)."</pre>";
...
//                echo "$difference<pre>".print_r($q_days, 1)."</pre>";
        ...
        //****************************************************************************************
        $first_day_category = NULL; //2-14 | 15-90 | NULL
        $first_day_number = NULL;
        $this_month_first_karense_date = !empty($q_days) ? $q_days[0]['date'] : NULL;
//        echo "<pre>".print_r($q_days, 1)."</pre>";
        
        //if first day of this month is a karense day, then no need calculate category of first day
        if($this_month_first_karense_date == NULL || date('d', strtotime($this_month_first_karense_date)) != 1){
            //this for assigning for last date as first date of this month
            $reversed_month_leave_works = array(array(
                'date' => $this->leavePeriod_year . '-' . $this->leavePeriod_month . '-01',
                'status' => 2,
                'no_pay' => 1
            ));
            $reversed_month_leave_works = array_merge($reversed_month_leave_works, array_reverse($previous_month_leave_works));
            
//            echo "<pre>".print_r($reversed_month_leave_works, 1)."</pre>";
            
            
            $obj_converter = new Converter(NULL, NULL, NULL);
//            //return difference between last karense date and last leave day
            $leaveDays = $obj_converter->get_15_90_count($reversed_month_leave_works, $date_diff_for_qlfying)-2;
//            $leaveDays = -1;
//            echo "leaveDays<pre>".print_r($leaveDays, 1)."</pre>";
            if($leaveDays < 0){
                // if $leaveDays == -1 => no previous month leaves
                $first_day_category = '15-90';
            }
            else if($leaveDays <= 14){
                $first_day_category = '2-14';
                $first_day_number = $leaveDays;
            }
        }
        //****************************************************************************************
        
        return array('Qualifying_days' => $q_days, 
            'ovarlaped_karenses'    => $additional_overlaped_day_karenses,
            'first_day_category'    => $first_day_category,
            'first_day_number'      => $first_day_number);
    }
...
//            echo $__this_tot_normal.'-'.$__this_tot_oncall.'<br>';
    ...
    function save_fkkn_form_defaults($this_customer, $this_bargaining, $provider_of_pa, $company_cp_name, $company_cp_phone, $agreement_type, $agreement_type2_company, $agreement_type2_orgno) {
        /**
         * @author: Shamsudheen <shamsu@arioninfotech.com>
        ...
        if (!empty($defaults)) {
            ...
            //insertion
            $this->tables = array('fkkn_form_defaults');
            $this->fields = array('customer', 'bargaining_new', 'company_cp_name', 'company_cp_phone', 'provider_of_pa_flag', 'agreement_types_new', 'agreement_type2_company', 'agreement_type2_orgNo');
            $this->field_values = array($this_customer, $this_bargaining, $company_cp_name, $company_cp_phone, $provider_of_pa, $agreement_type, $agreement_type2_company, $agreement_type2_orgno);
    ...
    function check_exists_fkkn_form_defaults($this_customer) {
         ...
         */
        $this->tables = array('fkkn_form_defaults');
        $this->fields = array('id', 'customer', 'bargaining_new', 'company_cp_name', 'company_cp_phone', 'provider_of_pa_flag', 'agreement_types_new', 'agreement_type2_company', 'agreement_type2_orgNo', 'employer_role');
        $this->conditions = array('customer = ?');
        $this->condition_values = array($this_customer);
        $this->query_generate();
...
//        echo "<pre>".print_r($list_relations, 1)."</pre>";
        ...
        $qualifying_leave_works__ = $this->get_employee_leave_Qualifying_day_works($sick_details);
        $qualifying_leave_works = $qualifying_leave_works__['Qualifying_days'];
        $Q_work_dates = array();
        if(!empty($qualifying_leave_works)){
            foreach ($qualifying_leave_works as $Qworks)
                $Q_work_dates[] = $Qworks['date'];
        }
        ...
        // echo '<pre>'.print_r($list_relations_grouped, 1).'</pre>';
        // $list_relations_grouped['jejo005'][0]['date'] = '2017-01-03';
        ...
        if(!empty($list_relations_grouped)){
            foreach($list_relations_grouped as $vikarie_emp => $work_details){
                ...
                if(!empty($work_details)){
                    $temp_Q_work_dates = $Q_work_dates;
                    $qdate_partitioned_data_index = 0;
                    foreach($work_details as $ind => $w_data){
                        $block_break = FALSE;
                        if(!empty($temp_Q_work_dates)){
                            foreach($temp_Q_work_dates as $qd){
                                if($w_data['date'] >= $qd){
                                    $block_break = TRUE;
                                    $qday_index = array_search($qd, $temp_Q_work_dates);
                                    if( $qday_index !== FALSE){
                                        unset($temp_Q_work_dates[$qday_index]);
                                    }
                                }
                            }
                        }
                        if( $block_break && $ind !== 0) $qdate_partitioned_data_index++;
                        $qdate_partioned_work_details[$qdate_partitioned_data_index][] = $w_data;
                    }
                }
...
//        echo "<pre>".print_r($Q_work_dates, 1)."</pre>";
        ...
        if(!empty($sick_details)){
            $qdate_partitioned_sick_data_index = 0;
            foreach($sick_details as $ind => $sick_data){
                $qday_index = array_search($sick_data['date'], $Q_work_dates);
                if( $qday_index !== FALSE){
                    unset($Q_work_dates[$qday_index]);
                    if($ind !== 0) $qdate_partitioned_sick_data_index++;
                    $qdate_partitioned_sick_details[$qdate_partitioned_sick_data_index][] = $sick_data;
                } else{
                    $qdate_partitioned_sick_details[$qdate_partitioned_sick_data_index][] = $sick_data;
                }
            }
...
//        ob_clean();
//        echo "<pre>".print_r($sick_details_partition, 1)."</pre>";
...
//            $pdf->Output();
    ...
    function Customer_employee_sick_details_and_work_report($customer, $employee, $year, $month){
...
//        $pdf->report_title();
    ...
    function make_fk_export_xml($sel_year, $sel_month, $cust_key, $emp_key) {
        ...
        if($sign_details && $sign_details[0]['xml'] != NULL){
            ...
            $company_data = $obj_company->get_company_detail($_SESSION['company_id']);
            $fkkn_form_defaults = $this->check_exists_fkkn_form_defaults($cust_key);
            ...
            <ns:Tidredovisning xmlns:ns="http://schema.forsakringskassan.se/integration/isassistansersattning/tidredovisning/1">
                    ...
                    if(!empty($fkkn_form_defaults) && $fkkn_form_defaults[0]['provider_of_pa_flag'] == 1){
                        ...
                        $xml .= '<ns:assistansanordnare>
                                    ...
                                    if((!empty($fkkn_form_defaults) && $fkkn_form_defaults[0]['agreement_types_new'] == 1) || empty($fkkn_form_defaults))
    ...
    function get_employee_certification_summery_data($employee, $start_year, $start_month, $end_year, $end_month, $remove_if_no_vals = FALSE){
        ...
        for($year = $start_year ; $year <= $end_year ; $year++){
            ...
            $tmp_oncall_sum = $this->get_employee_oncall_hour_sum($employee, $year);
            $tmp_oncall_sum_indexed = array();
            if(!empty($tmp_oncall_sum)){
                foreach($tmp_oncall_sum as $twd)
                    $tmp_oncall_sum_indexed[$twd['month']] = $twd;
            }
            ...
            for($month = $iteraton_start_month ; $month <= $iteraton_end_month ; $month++){
                ...
                if($remove_if_no_vals 
                    ...
                    || ((!isset($tmp_oncall_sum_indexed[(int) $month]) || $tmp_oncall_sum_indexed[(int) $month]['total_oncall_time'] == 0)
                    && empty($tmp_ob_works))))
                    continue;
                ...
                $summary_data[] = array(
                    ...
                    'oncall_total'  => isset($tmp_oncall_sum_indexed[(int) $month]) ? $tmp_oncall_sum_indexed[(int) $month]['total_oncall_time'] : 0,
                    'have_ob'       => !empty($tmp_ob_works) ? TRUE : FALSE
                );
            }
        }
        return $summary_data;
