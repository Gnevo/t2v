...
//            $holidays = array();
        ...
        foreach ($holidays as $holiday) {
            ...
            if ($slot_time_from < $holiday['start'] && $slot_time_to > $holiday['end']) {
                ...
                $rpt_content[$inconv_normal_slot['date']][$holidayTmp]['times'][] = array($holiday['start'], $holiday['end'], round(($holiday['end'] - $holiday['start']) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $holidayTmp, $leave_no_pay, $leave_max_date);
                ...
                $rpt_content[$inconv_normal_slot['date']][$holidayTmp]['times'][] = array($slot_time_from, $holiday['end'], round(($holiday['end'] - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $holidayTmp, $leave_no_pay, $leave_max_date);
                ...
                $rpt_content[$inconv_normal_slot['date']][$holidayTmp]['times'][] = array($holiday['start'], $slot_time_to, round(($slot_time_to - $holiday['start']) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $holidayTmp, $leave_no_pay, $leave_max_date);
                ...
                $rpt_content[$inconv_normal_slot['date']][$holidayTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $holidayTmp, $leave_no_pay, $leave_max_date);
                unset($inconv_normal_slots[$key]);
                $flag = 1;
                break;
            }
            ...
            foreach ($inconv_normal_category as $inconv_normal) {
                ...
                if (in_array(date('N', strtotime($inconv_normal_slot['date'])), $days)) {
                    if ($slot_time_from < $inconv_time_from && $slot_time_to > $inconv_time_to) {
                        ...
                        $rpt_content[$inconv_normal_slot['date']][$leaveTypeTmp]['times'][] = array($inconv_time_from, $inconv_time_to, round(($inconv_time_to - $inconv_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $leaveTypeTmp, $leave_no_pay, $leave_max_date);
                        ...
                        $rpt_content[$inconv_normal_slot['date']][$leaveTypeTmp]['times'][] = array($slot_time_from, $inconv_time_to, round(($inconv_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $leaveTypeTmp, $leave_no_pay, $leave_max_date);
                        ...
                        $rpt_content[$inconv_normal_slot['date']][$leaveTypeTmp]['times'][] = array($inconv_time_from, $slot_time_to, round(($slot_time_to - $inconv_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $leaveTypeTmp, $leave_no_pay, $leave_max_date);
                        ...
                        $rpt_content[$inconv_normal_slot['date']][$leaveTypeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type, $leaveTypeTmp, $leave_no_pay, $leave_max_date);
                        unset($inconv_normal_slots[$key]);
                        $flag = 1;
                        break;
                    }
    ...
    private function identifyTimeCode($timeCode, $iInc = 0, $salary_mod = 1) {
...
//        }
    ...
    private function processWorkingTime($passed_employee, $paseed_customers, $month, $yr, &$rpt_content_normal, &$rpt_content_over, &$rpt_content_quality, &$rpt_content_more, &$rpt_content_some, &$rpt_content_oncall, &$rpt_content_leave, &$rpt_content_leave_over, &$rpt_content_leave_quality, &$rpt_content_leave_more, &$rpt_content_leave_some, &$rpt_content_leave_oncall, &$rpt_content_training, &$rpt_content_personal, &$rpt_content_calltraining, &$rpt_content_leave_training, &$rpt_content_leave_personal, &$rpt_content_leave_calltraining, &$rpt_content_travell, &$rpt_content_break, &$rpt_content_voluntary, &$rpt_content_complementary, &$rpt_content_complementary_oncall, &$rpt_content_leave_voluntary, &$rpt_content_more_oncall, &$rpt_content_leave_more_oncall, &$rpt_content_standby, &$rpt_content_leave_standby) {
        //$oncall_status = $this->employeeObj->get_salary_code_count(1, array(3003, 3012, 3013)) && $this->customerObj->get_salary_code($_SESSION['company_id']) == 3 ? 1 : 0;
        $oncall_status = $this->customerObj->get_salary_code($_SESSION['company_id']) == 3 ? 1 : 0;
        ...
        foreach ($inconv_normal_slots as $inconv_normal_slot) {
            ...
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '11')) {
                ...
                $rpt_content_voluntary[$inconv_normal_slot['date']]['3010']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '12')) {
                ...
                $rpt_content_complementary[$inconv_normal_slot['date']]['3011']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '15')) {
                ...
                $rpt_content_standby[$inconv_normal_slot['date']]['3014']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            // TODO: test this
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '3')) {
                ...
                $rpt_content_oncall[$inconv_normal_slot['date']]['3003']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '9')) {
                ...
                $rpt_content_calltraining[$inconv_normal_slot['date']]['3003']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '13')) {
                ...
                $rpt_content_complementary_oncall[$inconv_normal_slot['date']]['3012']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            if ($inconv_normal_slot['status'] == 1 && ($inconv_normal_slot['type'] == '14')) {
                ...
                $rpt_content_more_oncall[$inconv_normal_slot['date']]['3013']['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer']);
            }
            ...
            if ($inconv_normal_slot['status'] == 2) {
                ...
                if ($leave_type) {
                    ...
                    if ($inconv_normal_slot['type'] == '0' || $inconv_normal_slot['type'] == '1' || $inconv_normal_slot['type'] == '2') {
                        ...
                        $rpt_content_leave[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    } else if ($inconv_normal_slot['type'] == '4') {
                        $rpt_content_leave_over[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_over[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    } else if ($inconv_normal_slot['type'] == '5') {
                        $rpt_content_leave_quality[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_quality[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    } else if ($inconv_normal_slot['type'] == '6') {
                        $rpt_content_leave_more[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_more[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    } else if ($inconv_normal_slot['type'] == '7') {
                        $rpt_content_leave_some[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_some[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                        // TODO: test this
                    } else if ($inconv_normal_slot['type'] == '8') {
                        $rpt_content_leave_training[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_training[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                        // TODO: test this
                    } else if ($inconv_normal_slot['type'] == '10') {
                        $rpt_content_leave_personal[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_personal[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                        // TODO: test this
                    } else if ($inconv_normal_slot['type'] == '11') {
                        $rpt_content_leave_voluntary[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_voluntary[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                        // TODO: test this
                    } else if ($inconv_normal_slot['type'] == '15') {
                        $rpt_content_leave_standby[$inconv_normal_slot['date']][$timeCodeTmp]['total'] += round(($slot_time_to - $slot_time_from) / (60 * 60), 2);
                        $rpt_content_leave_standby[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                        // TODO: test this
                    } else if ($inconv_normal_slot['type'] == '3') {
                        ...
                        $rpt_content_leave_oncall[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    } else if ($inconv_normal_slot['type'] == '9') {
                        //$timeCodeTmp = '2001.'.$leave_type['type'];
                        if($timeCodeTmp == '2002'){
                            $timeCodeTmp = '2002.1';
                        ...
                        $rpt_content_leave_calltraining[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    } else if ($inconv_normal_slot['type'] == '14') {
                        if($timeCodeTmp == '2002'){
                            $timeCodeTmp = '2002.1';
                        }
                        ...
                        $rpt_content_leave_more_oncall[$inconv_normal_slot['date']][$timeCodeTmp]['times'][] = array($slot_time_from, $slot_time_to, round(($slot_time_to - $slot_time_from) / (60 * 60), 2), $inconv_normal_slot['customer'], $leave_type['type'], $timeCodeTmp, $leave_type['no_pay'], $leave_max_date);
                    }
                }
            }
        }
        ...
        //echo "<pre>".print_r($rpt_content_leave_oncall, 1)."</pre>";
                ...
                foreach ($valueNormal as $subkeyNormal => $subvalueNormal) {
                    ...
                    if ($subkeyNormal < 2000) {
                        ...
                        foreach ($subvalueNormal['times'] as $subsubkeyNormal => $subsubvalueNormal) {
                            ...
                            $add_key = ($rpt_key == 11 && $oncall_status != 1) ? $add_key + 2 : $add_key;
...
//        echo "<pre>\n".print_r($rpt_content_leave_training, 1)."</pre>"; 
//        echo "--------------------------------------------------";
        ...
        $prev_sick_end_date = date('Y-m-d',  strtotime("-1 day", strtotime("+3 months", strtotime($prev_sick_start_date))));                                                                                                
                    ...
                    if ($rpt_content_key_float >= 2000 && $rpt_content_key_float < 2002) {
                        ...
                        if (!empty($prevDateKarens) && strtotime($prevDate) != strtotime($rpt_date_key)) {
                            ...
                            if ($intervalDate->format('%r%a') >= $isKarensInterval && $rpt_content_value['times'][0][6] == 1) {
                                ...
                                if($leaveDays == 1){
                                    $leaveDays = $this->get_15_90_count($karens_date_data, $isKarensInterval)+1;
                                }
                ...
                foreach ($rpt_content as $rpt_date_key => $rpt_date_value) {
                    ...
                    foreach ($rpt_date_value as $rpt_content_key => $rpt_content_value) {
                        if ($rpt_content_key == '2000' && $sick_array[$rpt_date_key]['total'] < 8) {
                            ...
                            $rpt_content_all[$rpt_key][$rpt_date_key][2000]['times'][] = array(0, 0, (8 - $rpt_content_value['total']), $rpt_content_value['times'][0][3]);
                        }
                    }
                }
            }
                ...
                foreach ($rpt_content as $keyDate => $valueDate) {
                    ...
                    foreach ($valueDate as $timeCode => $timeValue) {
                        ...
                        foreach ($timeValue['times'] as $keyTimeValue => $valueTimeValue) {
                            $extern_time_code = '';
                            if ($key_array == 13 || $key_array == 12 || $key_array == 7 || $key_array == 14) {
                                if ($timeCode != '3008' && $timeCode != '3011') {
...
{';
    ...
    public function toHogia($return = false, $date = null, $c_list = null, $userSigned = array()) {
        ...
        foreach ($userSigned as $value => $key) { //value and key interchanged 
            ...
            //echo "<pre>\n".print_r($rpt_content_leave, 1)."</pre>";
            unset(
                    $rpt_content_normal, $rpt_content_over, $rpt_content_quality, $rpt_content_more, $rpt_content_some, $rpt_content_oncall, $rpt_content_leave, $rpt_content_leave_over, $rpt_content_leave_quality, $rpt_content_leave_more, $rpt_content_leave_some, $rpt_content_leave_oncall, $rpt_content_training, $rpt_content_personal, $rpt_content_calltraining, $rpt_content_leave_training, $rpt_content_leave_personal, $rpt_content_leave_calltraining, $rpt_content_travell, $rpt_content_break, $rpt_content_voluntary, $rpt_content_complementary, $rpt_content_complementary_oncall, $rpt_content_leave_voluntary, $rpt_content_more_oncall, $rpt_content_leave_more_oncall
            );
            ...
            $temp_output = utf8_decode("; {$userSocialSecurity} {$userSignedEmp[$value]['first_name']} {$userSignedEmp[$value]['last_name']} {$userSignedEmp[$value]['address']} {$userSignedEmp[$value]['city']}") . "\r\n";
            ...
            foreach ($timePerTimeCode as $timeCode => $timeCodeData) {
                ...
                if ($key_array == 7) {
                    ...
                    $external_code = 
                            sprintf('%03d', $this->identifyTimeCode($timeCode . '.3', 0, $salary_mod));
                } else {
                    $external_code = sprintf('%03d', $this->identifyTimeCode($timeCode, 0, $salary_mod));
                }
                //echo $timeCode."---".$external_code."<br>";
                if($external_code != '000' && ($external_code == '027' || $external_code == '012')){
                    $outputLine = '214003' .
...
//                        echo "<pre>" . print_r($consolidated_leave, 1) . "</pre>";
                        if (array_key_exists($timeCode, $consolidated_leave) &&
                            ...
                            if (array_key_exists($timeCode, $consolidated_leave))
                                $i_consol = end(array_keys($rpt_content_leave_consolidated[$timeCode])) + 1;
                            else
                                $i_consol = 0;
...
//            }
            ...
            foreach ($rpt_content_leave_consolidated as $timeCode => $rpt_content_leave_consolidated_group) {
                foreach ($rpt_content_leave_consolidated_group as $time_values) {
                    ...
                    if ($timeCode == 2003 || $timeCode == 2004) {
                        if ($time_values['used_days'])
                            $lonetrans->appendChild($xmlDoc->createElement('antal', $time_values['used_days']));
                        else
    ...
    //bl salary
    public function toBl($return = false, $date = null, $c_list = null, $userSigned = array()) {
        $smarty = new smartySetup(array('export-config.xml'),FALSE);
        $userSignedEmp = array();
        $customer_list = array();
        $obj_timetable = new timetable();
