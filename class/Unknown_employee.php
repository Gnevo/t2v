...
*/
		...
		if($fromdate != '0000-00-00' && $todate == '0000-00-00')
		{			
			$this->conditions = array('AND','(timetable.status = 1 OR timetable.status = 2)','leave.employee = "'.$empunm.'"','timetable.employee = "'.$empunm.'"','leave.date >= "'.$fromdate.'" ','timetable.date >= "'.$fromdate.'" ','timetable.date = leave.date');				
		}	
		if($fromdate == '0000-00-00' && $todate != '0000-00-00')
		{			
			$this->conditions = array('AND','(timetable.status = 1 OR timetable.status = 2)','leave.employee = "'.$empunm.'"','timetable.employee = "'.$empunm.'"','leave.date <= "'.$todate.'" ','timetable.date <= "'.$todate.'" ','timetable.date = leave.date');				
		}			
		if($fromdate != '0000-00-00' && $todate != '0000-00-00')
		{			
		...
		if($fromdate == '0000-00-00' && $todate == '0000-00-00')
		{	
			$this->conditions = array('AND','(timetable.status = 1 OR timetable.status = 2)','leave.employee = "'.$empunm.'"','timetable.employee = "'.$empunm.'"', 'timetable.date = leave.date');	
		}
		$this->group_by = array('leave.time_from','leave.date');
		$this->query_generate();
		// echo $this->sql_query;
        ...
        if ($key == NULL) {
            switch ($login_user_role) {
                ...
                case 6:
                    ...
                    $this->order_by = array('LOWER('.$this->db_name.'.employee.last_name) collate utf8_bin');
                    $this->query_generate();
                    $employee_data = $this->query_fetch();
                    break;
                case 2:
            ...
            switch ($login_user_role) {
                ...
                case 6:
                    ...
                    $this->order_by = array('LOWER('.$this->db_name.'.employee.first_name) collate utf8_bin', 'LOWER('.$this->db_name.'.employee.last_name) collate utf8_bin');
                    $this->query_generate();
                    $employee_data = $this->query_fetch();
                    break;
                case 2:
        ...
        if ($key == NULL) {
            switch ($login_user_role) {
                ...
                case 6:
                    ...
                    $this->order_by = array('LOWER('.$this->db_name.'.employee.last_name) collate utf8_bin ASC');
                    $this->query_generate();
                    $employee_data = $this->query_fetch();
                    break;
                case 2:
...
//            }    
    ...
    function employee_add() {
        ...
        if ($this->username != NULL) {
            ...
            $this->tables = array('employee');
            $this->fields = array('username', 'century', 'code', 'social_security', 'first_name', 'last_name', 'address', 'city', 'post', 'phone', 'mobile', 'email', 'date', 'color', 'status','date_inactive','max_hours');
            $this->field_values = array($this->username, $this->century, $this->code, $this->social_security, $this->first_name, $this->last_name, $this->address, $this->city, $this->post, $this->phone, $this->mobile, $this->email, $this->date, $this->color_code, $this->status,$this->inactive_date,$this->max_hours);
    ...
    function employee_update() {
        ...
        if ($this->username != NULL) {
            ...
            $this->tables = array('employee');
            $this->fields = array('code', 'century', 'social_security', 'first_name', 'last_name', 'address', 'city', 'post', 'phone', 'mobile', 'email', 'date', 'color', 'status','date_inactive','max_hours');
            $this->field_values = array($this->code, $this->century, $this->social_security, $this->first_name, $this->last_name, $this->address, $this->city, $this->post, $this->phone, $this->mobile, $this->email, $this->date, $this->color_code, $this->status,$this->inactive_date,$this->max_hours);
            $this->conditions = array('username = ?');
            $this->condition_values = array($this->username);
            if ($this->query_update()) {
				$this->employee_inconviniant_time($this->username);
			...
			if(count($datas) > 0){
				$datas = $datas[0];
					...
					if($datas['on_call'] != $this->on_call || $datas['on_call_holiday'] != $this->oncall_holiday ||
						 $datas['on_call_bigholiday'] != $this->oncall_bigholiday ||  $datas['inconvinient_night'] != $this->inconvinient_night
			||  $datas['inconvinient_evening'] != $this->inconvinient_evening ||  $datas['inconvinient_holiday'] != $this->inconvinient_holiday
			|| $datas['inconvinient_week_holiday'] != $this->inconvinient_week_holiday){
    ...
    function employee_detail($username ="") {
        $this->tables = array('employee');
        $this->fields = array('username', 'century', 'code', 'social_security', 'first_name', 'last_name', 'address', 'city', 'post', 'phone', 'mobile', 'email', 'date', 'color', 'status','date_inactive','inconvinient_week_holiday','inconvinient_holiday','inconvinient_night','inconvinient_evening','on_call');
...
//getting customer contacts
    ...
    function get_available_users($customer, $time_from, $time_to, $date) {
        ...
        if ($_SESSION['user_role'] == 3) {
            $this->condition_values = array('1', (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, $date, '', '2', (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, $date, 2, $date_month, $date_year);
        } else {

            $this->condition_values = array('1', (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, $date, '', '2', (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, (float) $time_from, (float) $time_to, $date, 2, $date_month, $date_year, $customer);
        }
        $this->query_generate();
//        echo $this->sql_query;
...
//getting toatal time for perticulal user and cusomer for the date
    ...
    function get_holiday_details($month, $year) {
        ...
        $this->fields = array('id', 'effect_from', 'effect_to', 'date_from', 'date_to', 'name', 'start_time', 'end_time', 'type');
        if($month == 1){
...
//        $this->fields = array('l.id as id', 'l.group_id as gid', 'l.employee as emp_id', "concat(e1.first_name,' ',e1.last_name) as leave_employee", 'l.date as leave_date', 'l.time_from as time_from', 'l.time_to as time_to', 'l.type as type', 'l.appr_date as date', "concat(e.first_name,' ',e.last_name) as empname");
        $this->fields = array('l.id as id', 'l.group_id as gid', 'l.employee as emp_id', "concat(e1.first_name,' ',e1.last_name) as leave_employee", 'l.date as leave_date', 'l.time_from as time_from', 'l.time_to as time_to', 'l.type as type', 'l.appr_date as appr_date', "concat(e.first_name,' ',e.last_name) as appr_empname");
...
//        print_r($data);
    ...
    function checkATL($employee, $date, $time_from, $time_to, $exceptional_ids='', $remove = 0) {
        ///////////////// start of ATL laws///////////////////
        // for FIRST user
        $max_day_hours = 26;
        $tot_work_hours = 0;
        ...
        foreach ($datas as $row_atl) {
            ...
            //echo "<script>alert(\"".date('Y-m-d',$start_time)."\")</script>";
            //echo "<script>alert(\"".date('Y-m-d',$prev_week)."\")</script>";
            ...
            if (($start_time >= $prev_week) && ($interval >= 129600)  && (count($datas) != $i)) { //7 days period  getting a rest of 36 hrs
                $week_law = 0;
                $output = "ATL varning " . $employee . "=>" . $row_atl['date'] . " " . str_pad($row_atl['time_from'], 5, '0', STR_PAD_LEFT) . "-" . str_pad($row_atl['time_to'], 5, '0', STR_PAD_LEFT);
        ...
        if ($atl_flag == 0 && $day_law == 0 && $week_law == 0) {
            ...
            foreach ($datas as $row_atl) {
                ...
                if (($prev_end_time <= $after_week) && ($interval >= 129600) && (count($datas) != $i)) { //7 days period  getting a rest of 36 hrs
                    $week_law = 0;
                    $output = "ATL varning " . $employee . "=>" . $row_atl['date'] . " " . str_pad($row_atl['time_from'], 5, '0', STR_PAD_LEFT) . "-" . str_pad($row_atl['time_to'], 5, '0', STR_PAD_LEFT);
                    break;
                }
        ...
        if ($atl_flag == 1 || $day_law == 1 || $week_law == 1) {
            ...
            $this->field_values = array($employee, $row_atl['date'], $row_atl['time_from'], $row_atl['time_to']);
            if($this->query_insert()){
                ...
                if (!empty($email_receipients)) {
                    ...
                    $mail_message .= 'Anställd: ' . $employe_data['first_name'] . ' ' . $employe_data['last_name'] . '<br/>';
                    $mail_message .= $row_atl['date'] . " " . str_pad($row_atl['time_from'], 5, '0', STR_PAD_LEFT) . "-" . str_pad($row_atl['time_to'], 5, '0', STR_PAD_LEFT);
                    $mail_message .=  $smarty->translate['atl_mail_body2'];
                    $mail_subject = $smarty->translate['atl_mail_subject1']. $employe_data['first_name'] . ' ' . $employe_data['last_name'];
                    $mail = new SimpleMail($mail_subject, $mail_message);
                    $mail->addSender("Cirrus");
...
//                }
    ...
    function skip_a_time_slot_from_leave_slot($leave_id, $time_from, $time_to) {     //by shamsu............
...
//        $this->fields = array('status', 'date', 'date_inactive');
    ...
    function employee_work_slots_day($employee, $date, $time_from, $time_to) {
        ...
        $this->condition_values = array($employee, $date, $time_from, $time_to);
        $this->order_by = array('time_from');
        $this->query_generate();
        $slots = $this->query_fetch();
        $datas = array();
