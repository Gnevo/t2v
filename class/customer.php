...
//                    $this->conditions = array('AND', 'username = ?', 'status = 1', array('OR', 'last_name LIKE ?', 'last_name LIKE ?'));
    ...
    function get_filter_date_report_query3 ($customer, $fdate, $tdate, $full_emp, $table){
        if($customer != 'All'){
            if ($full_emp == 1) {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND fkkn='2' AND status='1' AND ( employee != '' OR employee != NULL ) AND type IN(0,3,4,5,6,14,15);";
            } else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND employee='" . $_SESSION['user_id'] . "' AND fkkn='2' AND status='1' AND type IN(0,3,4,5,6,14,15);";
            }
        }
        else{
            if($_SESSION['user_role'] == 1){
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND fkkn='2' AND status='1' AND ( employee != '' OR employee != NULL ) AND type IN(0,3,4,5,6,14,15);";
            }elseif ($full_emp == 1) {
                $this->sql_query = "SELECT t.time_to,t.time_from,t.employee,t.type FROM `".$table."` t"
                        . " INNER JOIN team tm  ON t.customer = tm.customer WHERE tm.employee='".$_SESSION['user_id']."' AND `t.date` BETWEEN '$fdate' AND '$tdate' AND t.fkkn='2' AND t.status='1' AND ( t.employee != '' OR t.employee != NULL ) AND t.type IN(0,3,4,5,6,14,15);";
            } else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND employee='" . $_SESSION['user_id'] . "' AND fkkn='2' AND status='1' AND type IN(0,3,4,5,6,14,15);";
            }
        }
    ...
    function get_filter_date_report_query4 ($customer, $fdate, $tdate, $full_emp, $table){
        if($customer != 'All'){
            if ($full_emp == 1) {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND fkkn='2' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            } 
            else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND ( employee = '' OR employee = NULL OR employee = '" . $_SESSION['user_id'] . "' ) AND fkkn='2' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            }
        }
        else{
            if($_SESSION['user_role'] == 1){
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND fkkn='2' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            }
            elseif ($full_emp == 1) {
                $this->sql_query = "SELECT t.time_to,t.time_from,t.employee,t.type FROM `".$table."` t"
                        . " INNER JOIN team tm  ON t.customer = tm.customer WHERE tm.employee='".$_SESSION['user_id']."' AND `t.date` BETWEEN '$fdate' AND '$tdate' AND t.fkkn='2' AND t.status IN (0,1) AND ( t.employee = '' OR t.employee = NULL OR t.employee ='" . $_SESSION['user_id'] . "' )  AND t.type IN(0,3,4,5,6,14,15);";
            } 
            else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND ( employee = '' OR employee = NULL OR employee ='" . $_SESSION['user_id'] . "' ) AND fkkn='2' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            }
    ...
    function get_filter_date_report_query5 ($customer, $fdate, $tdate, $full_emp, $table){
        if($customer != 'All'){
            if ($full_emp == 1) {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND fkkn='3' AND status='1' AND ( employee != '' OR employee != NULL ) AND type IN(0,3,4,5,6,14,15);";
            } else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND employee='" . $_SESSION['user_id'] . "' AND fkkn='3' AND status='1' AND type IN(0,3,4,5,6,14,15);";
            }
        }
        else{
            if($_SESSION['user_role'] == 1){
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND fkkn='3' AND status='1' AND ( employee != '' OR employee != NULL ) AND type IN(0,3,4,5,6,14,15);";
            }elseif ($full_emp == 1) {
                $this->sql_query = "SELECT t.time_to,t.time_from,t.employee,t.type FROM `".$table."` t"
                        . " INNER JOIN team tm  ON t.customer = tm.customer WHERE tm.employee='".$_SESSION['user_id']."' AND `t.date` BETWEEN '$fdate' AND '$tdate' AND t.fkkn='3' AND t.status='1' AND ( t.employee != '' OR t.employee != NULL ) AND t.type IN(0,3,4,5,6,14,15);";
            } else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND employee='" . $_SESSION['user_id'] . "' AND fkkn='3' AND status='1' AND type IN(0,3,4,5,6,14,15);";
            }
        }
    ...
    function get_filter_date_report_query6 ($customer, $fdate, $tdate, $full_emp, $table){
        if($customer != 'All'){
            if ($full_emp == 1) {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND fkkn='3' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            } 
            else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND ( employee = '' OR employee = NULL OR employee = '" . $_SESSION['user_id'] . "' ) AND fkkn='3' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            }
        }
        else{
            if($_SESSION['user_role'] == 1){
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND fkkn='3' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            }
            elseif ($full_emp == 1) {
                $this->sql_query = "SELECT t.time_to,t.time_from,t.employee,t.type FROM `".$table."` t"
                        . " INNER JOIN team tm  ON t.customer = tm.customer WHERE tm.employee='".$_SESSION['user_id']."' AND `t.date` BETWEEN '$fdate' AND '$tdate' AND t.fkkn='3' AND t.status IN (0,1) AND ( t.employee = '' OR t.employee = NULL OR t.employee ='" . $_SESSION['user_id'] . "' )  AND t.type IN(0,3,4,5,6,14,15);";
            } 
            else {
                $this->sql_query = "SELECT time_to,time_from,employee,type FROM `".$table."` WHERE `date` BETWEEN '$fdate' AND '$tdate' AND customer='$customer' AND ( employee = '' OR employee = NULL OR employee ='" . $_SESSION['user_id'] . "' ) AND fkkn='3' AND status IN (0,1) AND type IN(0,3,4,5,6,14,15);";
            }
    ...
    function get_filter_date_report($customer, $fdate, $tdate, $per) {  
       ...
       // $fk_used = $this->query_fetch(); // This will calculate FK used by specified customer
        ...
        //Calculating for Used TU

        if($fdate <= $boundary_date && $tdate > $boundary_date){ // union
            $real_table_data   = $this->get_filter_date_report_query5($customer, $fdate, $tdate, $full_emp, 'timetable');
            $backup_table_data = $this->get_filter_date_report_query5($customer, $fdate, $tdate, $full_emp, 'backup_timetable');
            $this->sql_query   = '( ' . $real_table_data . ' )' . ' UNION ' . '( ' . $backup_table_data . ' )  ' ;
        }
        else if($fdate <= $boundary_date && $tdate <= $boundary_date){
            $this->sql_query = $this->get_filter_date_report_query5($customer, $fdate, $tdate, $full_emp, 'backup_timetable');
        }
        else if($fdate > $boundary_date && $tdate > $boundary_date){
            $this->sql_query = $this->get_filter_date_report_query5($customer, $fdate, $tdate, $full_emp, 'timetable');
        }


        $tu_used_temp = $this->query_fetch();
        $tu_used = array();
        $tu_used[0]['tu_sum'] = 0.00;
        $normal_sum = 0.00;
        $oncall_sum = 0.00;
        $sum_standby = 0.00;
        for ($i = 0; $i < count($tu_used_temp); $i++) {
            if ($tu_used_temp[$i]['type'] == 15) {
                $t1 = $this->time_difference($tu_used_temp[$i]['time_to'], $tu_used_temp[$i]['time_from']);
                $sum_standby = $this->time_sum($sum_standby, $t1);
            }
            else if ($tu_used_temp[$i]['type'] != 3 && $tu_used_temp[$i]['type'] != 14) {
                $normal_sum = $this->time_sum($normal_sum, $this->time_difference($tu_used_temp[$i]['time_to'], $tu_used_temp[$i]['time_from']));
        ...
        //Calculating for Used TU over
        $time_oncall = $this->time_divide($oncall_sum, 4);
        $sum_standby_final = $this->time_divide($sum_standby, 7);
        $normal_sum = $this->time_sum($normal_sum, $sum_standby_final);
        $tu_used[0]['tu_sum'] = $equipment->time_user_format($this->time_sum($normal_sum, $time_oncall),100);


        //tu used including unmanned-----------------------------------------------------------

        if($fdate <= $boundary_date && $tdate > $boundary_date){ // union
            $real_table_data   = $this->get_filter_date_report_query6($customer, $fdate, $tdate, $full_emp, 'timetable');
            $backup_table_data = $this->get_filter_date_report_query6($customer, $fdate, $tdate, $full_emp, 'backup_timetable');
            $this->sql_query   = '( ' . $real_table_data . ' )' . ' UNION ' . '( ' . $backup_table_data . ' )  ' ;
        }
        else if($fdate <= $boundary_date && $tdate <= $boundary_date){
            $this->sql_query = $this->get_filter_date_report_query6($customer, $fdate, $tdate, $full_emp, 'backup_timetable');
        }
        else if($fdate > $boundary_date && $tdate > $boundary_date){
            $this->sql_query = $this->get_filter_date_report_query6($customer, $fdate, $tdate, $full_emp, 'timetable');
        }

        
        $tu_used_unmanned_temp = $this->query_fetch();
        $tu_used_unmanned = array();
        $tu_used_unmanned[0]['tu_sum'] = 0.00;
        $normal_sum = 0.00;
        $oncall_sum = 0.00;
        $sum_standby = 0.00;
        for ($i = 0; $i < count($tu_used_unmanned_temp); $i++) {
            if ($tu_used_temp[$i]['type'] == 15) {
                $t1 = $this->time_difference($tu_used_unmanned_temp[$i]['time_to'], $tu_used_unmanned_temp[$i]['time_from']);
                $sum_standby = $this->time_sum($sum_standby, $t1);
            }
            elseif ($tu_used_unmanned_temp[$i]['type'] != 3 && $tu_used_unmanned_temp[$i]['type'] != 14) {
                $normal_sum = $this->time_sum($normal_sum, $this->time_difference($tu_used_unmanned_temp[$i]['time_to'], $tu_used_unmanned_temp[$i]['time_from']));
        ...
        $sum_standby_final = $this->time_divide($sum_standby, 7);
        $normal_sum = $this->time_sum($normal_sum, $sum_standby_final);
        $tu_used_unmanned[0]['tu_sum'] = $equipment->time_user_format($this->time_sum($normal_sum, $time_oncall),100);
        //tu Cal over--------------------------------------------------------------------------------------

        $fk_used = sprintf("%.2f", floatval($fk_used[0]['fk_sum'])); // FK
        $kn_used = sprintf("%.2f", floatval($kn_used[0]['kn_sum'])); // KN
        $tu_used = sprintf("%.2f", floatval($tu_used[0]['tu_sum'])); // TU
        ...
        $tu_used_unmanned = sprintf("%.2f", floatval($tu_used_unmanned[0]['tu_sum'])); // TU
        ...
        $this->sql_query = "SELECT * FROM customer_contract WHERE
                    ...
                    AND customer='$customer' AND fkkn='2'";
        }else{
            if($_SESSION['user_role'] == '1'){
                $this->sql_query = "SELECT * FROM customer_contract WHERE
                    ( 
        ...
        $this->sql_query = "SELECT * FROM customer_contract WHERE
                    ...
                    AND customer='$customer' AND fkkn='3'";
        }else{
            if($_SESSION['user_role'] == '1'){
                $this->sql_query = "SELECT * FROM customer_contract WHERE
                    ( 
        ...
        $granted_tu_arr = $this->query_fetch(); // Fetch all KN granted data come in specified date range

        $granted_tu = 0;

        foreach ($granted_tu_arr as $gtu) {
            $days = $this->calculate_day_difference($gtu['date_from'], $gtu['date_to']);
            $hours_per_day = $gtu['hour'] / $days;
            $hours_per_day = $hours_per_day; // Hours per day in partucular month for KN

            if ($fdate >= $gtu['date_from'] && $tdate <= $gtu['date_to']) { // Specified dates are between db dates
                $calculated_days = $this->calculate_day_difference($fdate, $tdate);
                $temp_granted_tu = $calculated_days * $hours_per_day;
                $granted_tu = $granted_tu + $temp_granted_tu;
            } elseif ($fdate <= $gtu['date_from'] && $tdate >= $gtu['date_to']) {  // Specified dates cover db dates
                $calculated_days = $this->calculate_day_difference($gtu['date_from'], $gtu['date_to']);
                $temp_granted_tu = $calculated_days * $hours_per_day;
                $granted_tu = $granted_tu + $temp_granted_tu;
            } elseif ($fdate >= $gtu['date_from'] && $fdate <= $gtu['date_to']) { // First date between db from date and to date
                $calculated_days = $this->calculate_day_difference($fdate, $gtu['date_to']);
                $temp_granted_kn = $calculated_days * $hours_per_day;
                $granted_tu = $granted_tu + $temp_granted_tu;
            } elseif ($tdate >= $gtu['date_from'] && $tdate <= $gtu['date_to']) {
                $calculated_days = $this->calculate_day_difference($gtu['date_from'], $tdate);
                $temp_granted_tu = $calculated_days * $hours_per_day;
                $granted_tu = $granted_tu + $temp_granted_tu;
            }
        }
        $granted_tu = sprintf("%.2f", floatval($granted_tu));
        //Calculating for Granted TU over

        $fk_diff = $granted_fk - $fk_used;
        $kn_diff = $granted_kn - $kn_used;
        $tu_diff = $granted_tu - $tu_used;
        ...
        $tu_valid_diff = ( ($granted_tu / 100 ) * $per);
        $tu_valid_lower_range = $granted_tu - $tu_valid_diff;
        $tu_valid_lower_range = floatval($tu_valid_lower_range);

        $tu_valid_higher_range = $granted_tu + $tu_valid_diff;
        $tu_valid_higher_range = floatval($tu_valid_higher_range);

        if (($tu_valid_lower_range <= $tu_used) && ($tu_used <= $tu_valid_higher_range)) {
            $tu_text_color = 'BLACK';
        } else {
            $tu_text_color = 'RED';
        }
        ...
        $data['tu_granted'] = $granted_tu;
        $data['tu_used'] = $tu_used;
        $data['tu_used_including_unmanned'] = $tu_used_unmanned;
        $data['tu_unmanned_hours'] = sprintf("%.2f", floatval($tu_used_unmanned - $tu_used));
        $data['tu_diff'] = sprintf("%.2f", $tu_diff);
        $data['tu_color'] = $tu_text_color;
        if (intval($granted_tu) == 0) {
            $data['tu_diff_per'] = 0;
        } else {
            $data['tu_diff_per'] = ($tu_diff / $granted_tu) * 100;
        }
        $data['tu_diff_per'] = number_format((float) $data['tu_diff_per'], 2, '.', '');
    ...
    function time_sum_three($t1, $t2, $t3) {
        ...
        $time1 = (($a1[0] * 60 * 60) + (str_pad($a1[1], 2, '0', STR_PAD_RIGHT) * 60));
        $time2 = (($a2[0] * 60 * 60) + (str_pad($a2[1], 2, '0', STR_PAD_RIGHT) * 60));
        $time3 = (($a3[0] * 60 * 60) + (str_pad($a3[1], 2, '0', STR_PAD_RIGHT) * 60));
        $sum = abs($time1 + $time2 + $time3);
        $hours = floor($sum / (60 * 60));
        $mins = floor(($sum - ($hours * 60 * 60)) / (60));
        $result = $hours . "." . str_pad($mins, 2, '0', STR_PAD_LEFT);
        return $result;
    ...
    function get_bank_signed_timetable_employees($this_month, $this_year, $allowed_employees = NULL){
        ...
        $this->sql_query = 'SELECT DISTINCT t.employee as employee_id  FROM `timetable` AS t LEFT JOIN `report_signing` AS r ON (t.customer = r.customer AND t.employee = r.employee AND MONTH(r.date) = '.$this_month.' AND YEAR(r.date) = '.$this_year.' AND r.employee = r.signin_employee) 
                            LEFT JOIN `signing_employer` AS ers ON (t.customer = ers.customer AND ers.month = '.$this_month.' AND ers.year = '.$this_year.' AND ers.fkkn = 1)
                            LEFT JOIN `signing_employer_data` AS ersd ON (ersd.master_id = ers.id AND t.employee = ersd.employee)
                            WHERE t.status = 1 AND t.fkkn = 1 AND MONTH(t.date) = ? AND YEAR(t.date) = ? AND t.employee IS NOT NULL AND t.employee != "" 
                            AND (r.employee_sign IS NOT NULL AND  r.employee_sign != "")  
    ...
    function fk_export_3066_add($year, $month, $customer, $employee, $uuid, $xml, $tx_id) {
        //echo $year."*</br>".$month."*</br>".$customer."*</br>".$employee."*</br>".$uuid."*</br>".$xml."*</br>".$tx_id;
        $start_time = new DateTime;
        $start_time->setTimezone(new DateTimeZone('Europe/Stockholm'));
        $date = $start_time->format('Y-m-d G:i:s');
        $this->flush();
        $this->tables = array('fk_export_3066');
        $this->fields = array('year', 'month', 'customer', 'employee', 'uuid', 'xml', 'tx_id', 'created_by', 'created_date');
        $this->field_values = array($year, $month, $customer, $employee, $uuid, $xml, $tx_id, $_SESSION['user_id'], $date);
        return $this->query_insert();
    }
