...
//            $this->conditions[] = array('IN', 'employee', $employees);
    ...
    /*********** Signed Employee and non signed 27/01/2021***********/
    function customer_3066_get_signing_employee($customer_username, $employees = NULL) {

        $this->tables = array('report_signing_3066` as `r');
        $this->fields = array('r.id', 'r.customer', 'r.employee', 'r.signin_date', '(SELECT first_name from employee where username = r.employee) as re_fname',
            '(SELECT last_name from employee where username = r.employee) as re_lname',
            '(SELECT phone from employee where username = r.employee) as re_phone',
            '(SELECT mobile from employee where username = r.employee) as re_mobile');
        $this->conditions = array('AND', 'r.customer = ?');
        $this->condition_values = array($customer_username);
        if(is_array($employees) && !empty($employees))
            $this->conditions[] = array('IN', 'r.employee', '\'' . implode('\',\'', $employees) . '\'');
//            $this->conditions[] = array('IN', 'employee', $employees);
        else if(!is_array($employees) && $employees != NULL){
            $this->conditions[] = 'r.employee = ?';
            $this->condition_values[] = $employees;
        }
    ...
    function customer_3066_get_unsigning_employee($customer_username, $employee_array = array(), $except_roles = array()) {
        ...
        foreach($data as $dt){
            ...
            if(!in_array($dt['employee'], $employee_array)){
                $ar[] = $dt;
            }
            }else{
                $ar[] = $dt;
    ...
    /*function send_to_fk($customer_username,$employee_array = array()) {
        $this->sql_query = "SELECT * FROM `send_fk_3066` WHERE customer = '".$customer_username."'";
        $data = $this->query_fetch();
        foreach($data as $dt){
            if(in_array($dt['employee'], $employee_array)){
                $ar[] = $dt;
    ...
    function allready_send_to_fk($customer_username,$employee_array = array()) {
        $this->sql_query = "SELECT id, customer, employee, (SELECT first_name FROM employee WHERE username = employee) as first_name, 
        (SELECT last_name FROM employee WHERE username = employee) as last_name, (SELECT phone from employee where username = employee) as re_phone,
            (SELECT mobile from employee where username = employee) as re_mobile FROM `fk_export_3066` WHERE customer = '".$customer_username."'";
        $data = $this->query_fetch();
        foreach($data as $dt){
            if(in_array($dt['employee'], $employee_array)){
                $ar[] = $dt;
