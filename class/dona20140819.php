...
//        }
        ...
        if ($relation_id == '') {
            ...
            $this->fields = array('employee', 'customer', 'fkkn', 'type', 'date', 'time_from', 'time_to', 'status', 'created_status', 'relation_id', 'alloc_emp','comment');
            $this->field_values = array($employee, $customer, $fkkn, $type, $date, $time_from, $time_to, $status, $created_status, $relation_id, $alloc_emp,$comment);
        }
        if ($this->query_insert()) {
            return true;
        } else {
...
//        $month_end_date = date('Y-m-t', strtotime($year.'-'.$month.'-01'));
        ...
        $tmp_calculation_start_month = $company_data['employee_contract_start_month'] != '' ? $company_data['employee_contract_start_month'] : 1;
        $calculation_period_length = $company_data['employee_contract_period_length'] != '' ? $company_data['employee_contract_period_length'] : 1;
        ...
        while(!$flag_found_calculation_period){
            ...
            if($tmp_calculation_end_month > 12){
                ...
                if($tmp_calculation_start_month <= $month && $month <= $tmp_calculation_end_month){
                    ...
                    $contract_calculation_values['start_year'] = $contract_calculation_values['end_year'] = $year;
                }
            }
...
//                $pagecount = $pdf->setSourceFile('./pdf_forms/FK3059_011_F_001_2013.pdf');
            ...
            $calculation_period_start_date = date('Y-m-01', strtotime($contract_calculation_values['start_year'].'-'.$contract_calculation_values['start_month'].'-01'));
            $calculation_period_end_date = date('Y-m-t', strtotime($contract_calculation_values['end_year'].'-'.$contract_calculation_values['end_month'].'-01'));
            $this_month_end_date = date('Y-m-t', strtotime($year.'-'.$month.'-01'));
//            $employee_period_contract_hours = $obj_contract->get_employee_contract_between_dates($employee_names[$i], $calculation_period_start_date, $this_month_end_date);
            
            $employee_period_actual_hours = $this->customer_employee_total_actual_hours_btwn_dates($customer, $calculation_period_start_date, $this_month_end_date, $fkkn, $employee_names[$i]);
            
            $pdf->SubPart7_new($calculation_period_start_date, $calculation_period_end_date, $employee_period_actual_hours);    // this new block since 2014-07-16
...
//                $pagecount = $pdf->setSourceFile('./pdf_forms/FK3057_013_F_001_2013.pdf');
    ...
    function get_employee_not_signing($customer, $month, $year, $nk = 1) {
        ...
        $this->flush();
        $this->sql_query = "SELECT  DISTINCT t.employee, CONCAT(e.first_name,' ',e.last_name) AS emp  
                                    FROM timetable t
                                    LEFT JOIN employee e ON t.employee = e.username
...
//        return $output_2;
    ...
    function customer_employee_total_actual_hours_btwn_dates($customer, $start_date, $end_date, $fkkn, $employee) {
        /**
         * @author: Shamsu
...
//        echo "<pre>additional_overlaped_day_karenses: ".print_r($additional_overlaped_day_karenses, 1)."</pre>";
        ...
        if (count($qualifying_leave_works) > 1) {
            for ($i = 0; $i < count($qualifying_leave_works); $i++) {
                ...
                $incov_details = $this->get_employee_inconv_leaved_works_between_two_dates_copy($this->leave_employee, $this->leave_customer, $date1, $date2, $this_qday_exceeded_karense_id);
...
//                echo "++++++++++++++<pre>".print_r($group_vikaries[$i][0]['employee_id'], 1)."</pre>";
                ...
                $incov_details = array('inconv_details' => array(), 'semestersattn_jour_dag_2_14_salaries' => array());
...
//                echo "<pre>Incov : ".print_r($incov_details, 1)."</pre>------------";
    ...
    function get_employee_inconv_leaved_works_between_two_dates_copy($employee_id, $customer, $sdate, $edate, $except_id = NULL) {
        ...
        $total_sal = $semestersattn_jour_dag_2_14_salaries = array();
...
//        echo "<pre>Sal ".print_r($total_sal, 1)."</pre>";
        return array('inconv_details' => $total_sal, 'semestersattn_jour_dag_2_14_salaries' => $semestersattn_jour_dag_2_14_salaries);
    }
...
//            echo "<pre>Inconvenient slots: ".print_r($inconv_normal_slots, 1)."</pre>";
    ...
    function get_employee_inconv_leaved_works_between_two_dates_for_vikarie_copy($work_data) {
        ...
        $total_salaries = $semestersattn_jour_dag_2_14_salaries = array();
        if (!empty($inconv_normal_slots)) {
//            echo 'Inconvenient groups:         <br>';
//            echo "<pre>Inconvenient slots: ".print_r($inconv_normal_slots, 1)."</pre>";
            $inconvenient_process_obj->reset_inconvenient_variables();
...
//        echo "<pre>Result salary".print_r($total_salaries, 1)."</pre>-----";
        ...
        return array('inconv_details' => $total_salaries, 'semestersattn_jour_dag_2_14_salaries' => $semestersattn_jour_dag_2_14_salaries);
    }
...
//        echo "<pre>".print_r($this->condition_values, 1)."</pre>";
    ...
    function get_employee_leaved_timetable_works_before_the_date($employee, $date, $time_from) {
        ...
        $this->fields = array('t.id as id', 't.employee as employee', 't.customer as customer', 't.date as date', 't.fkkn as fkkn',
            't.time_from as time_from', 't.time_to as time_to', 't.type as type', 't.status as status', 'l.no_pay as no_pay');
        $this->conditions = array('AND', array('OR', 't.date < ?', array('AND', 't.date = ?', 't.time_to < ?')), 't.status = 2', 't.employee = ?',
            'l.type = 1', 't.employee like l.employee', 't.date = l.date', 'l.time_from <= t.time_from', 'l.time_to >= t.time_to');
...
//                echo "-----------<pre>sick_details $key: ".print_r($sick, 1)."</pre>";
                ...
                $inconvenient_process_obj->inconv_normal_slots = array($sick);
                $inconvenient_process_obj->generate_work_report_using_input_slots($employee, $sdate, $edate);
                $inconvenient_process_obj->categorize_salary_hours();
                ...
                if(!empty($inconvenient_process_obj->salary_hours)){
                    foreach($inconvenient_process_obj->salary_hours as $sal_date => $params){
                        foreach($params as $key_name => $key_hours){
                            if(in_array($key_name, $holiday_names)){
                                if(isset($sick_details[$key]['inconv_details']['__holiday__']))
                                    $sick_details[$key]['inconv_details']['__holiday__'] = $equipment->time_sum($sick_details[$key]['inconv_details']['__holiday__'], $key_hours);
                                else
                                    $sick_details[$key]['inconv_details']['__holiday__'] = $key_hours;
...
//        ob_clean();
        if(!empty($sick_details_partition)){
            foreach($sick_details_partition as $sick_datas){
                $pdf->AddPage();
                $pdf->report_header($year, $month, $company_data, $cust_details);
                $pdf->content_table($sick_datas, $distinct_ob_field_names, $company_data['salary_system']);
