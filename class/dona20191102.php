...
//        echo "func_get_args<pre>".print_r(func_get_args(), 1)."</pre>";
    ...
    function Customer_pdf_work_report($customer, $month, $year, $fkkn, $this_bargaining = NULL, $txt_other_bargaining = NULL, $this_agreement = array(), $employee = Null, $sam_sida = Null, $provider_of_pa = NULL, $obj_pdf = NULL) {
        ...
        for ($i = 0; $i < count($employee_names); $i++) {
            ...
            foreach ($work_partition as $works) {
                ...
                // $pagecount = $pdf->setSourceFile('./pdf_forms/FK3059_014_F_004_17_12_01.pdf');
                $pdf->AddPage();
    ...
    function Customer_pdf_work_report_for_kn($customer, $month, $year, $fkkn, $this_bargaining = NULL, $txt_other_bargaining = NULL, $this_agreement = array(), $employee = Null, $sam_sida = Null, $provider_of_pa = NULL, $obj_pdf = NULL) {
        ...
        foreach($full_kn_periods as $kn_period){
            ...
            if ($flag_local && $obj_pdf == NULL) {
                ...
                // $pagecount = $pdf->setSourceFile('./pdf_forms/FK3057_015_F_001__16_08_25.pdf');
                // $pagecount = $pdf->setSourceFile('./pdf_forms/FK3057_016_F_001__17_05_02.pdf');
                $pagecount = $pdf->setSourceFile('./pdf_forms/KN_3057__30_10_2017.pdf');
                $tppl = $pdf->importPage(1);
                $pdf->useTemplate($tppl, -2, 0, 210);
    ...
    function get_work_details_For_certification($employee_id, $year, $start_month, $end_month) {
        ...
        $this->sql_query = "SELECT DISTINCT month(date) AS month, YEAR(date) AS year, employee as emp,
                ROUND(SUM(time_to_sec(timediff(time(replace(cast(time_to as char),'.',':')) , time(replace(cast(time_from as char),'.',':')))) )/3600,2) AS work_hours 
                FROM (
                    ( SELECT date, time_from, time_to, employee FROM `timetable` WHERE employee = ? AND year(date) = ? AND month(date) >= ? AND month(date) <= ? AND relation_id is null AND status = 1 ) 
                    UNION 
                    ( SELECT date, time_from, time_to, employee FROM `backup_timetable` WHERE employee = ? AND year(date) = ? AND month(date) >= ? AND month(date) <= ? AND relation_id is null AND status = 1 ) 
                ) As sample1
    ...
    function Financial_payment_pdf($company_details = array(), $customer_details = array(), $customer_gardian_details = array(), $obj_pdf = NULL, $print_report = TRUE) {
        ...
        if($karense_dataset){
            foreach ($karense_dataset as $karense_data){
                ...
                $incov_details_claim = $this->get_employee_inconv_leaved_works_from_dataset($this->leave_employee, $this->leave_customer, $this->crop_sick_with_vikarie($non_karens_data_set, $vikaries,$all_leave_works_grouped), $date1, $date2, $company_details);
                //echo "<pre>".print_r($this->crop_sick_with_vikarie($non_karens_data_set, $vikaries,$all_leave_works_grouped), 1)."</pre>";
                $incov_details_claim['15_180'] = $this->get_employee_inconv_leaved_works_from_dataset($this->leave_employee, $this->leave_customer, $this->crop_sick_with_vikarie($non_karens_15_180_data_set, $vikaries,$all_leave_works_grouped), $date1, $date2, $company_details);
                $incov_details_claim['180_onwards'] = $this->get_employee_inconv_leaved_works_from_dataset($this->leave_employee, $this->leave_customer, $this->crop_sick_with_vikarie($non_karens_180_onwards_data_set, $vikaries,$all_leave_works_grouped), $date1, $date2, $company_details);
        ...
        foreach($dataset as $data){
            ...
            if(!empty($curresponding_vikarie_slots)){
                foreach($curresponding_vikarie_slots as $vikarie_slot){
                    if($data['customer'] == $vikarie_slot['customer'] &&  $data['date'] == $vikarie_slot['date']){
                        $temp = $data;
                        if($vikarie_slot['time_from'] > $data['time_from'])
                            $temp['time_from'] = $vikarie_slot['time_from'];
                        if($vikarie_slot['time_to'] < $data['time_to'])
                        ...
                        if(is_array($this_hours)){   
                            foreach ($this_hours as $sub_item => $sub_hours) {
                                if((stripos(strtolower($this_type), 'jour') !== FALSE || 
                                        ...
                                        stripos(strtolower($this_type), 'Vänt') !== FALSE)&& $sub_hours != 0){
