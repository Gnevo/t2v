...
class employee_ext extends db {
    ...
    function save_employee_non_prefered_time($user_name,$fromDate,$toDate,$dayInterval,$group_id, $mode){
        $group_id == null ? $group_id = 1 : $group_id++;
        $this->tables = array('employee_non_preferred_timings');
        $this->fields = array('group_id', 'employee','date_from','date_to','day','time_from','time_to','mode');
        $this->field_values = array();
        foreach ($dayInterval as $day => $date_times) {
            foreach ($date_times as $key => $value) {
                $this->field_values[] = array($group_id,$user_name,$fromDate,$toDate,$day,$value['timeFrom'],$value['timeTo'], $mode);
            }
        }
        return $this->query_insert();
    }

    function get_all_employee_non_prefered_rime($employee, $mode){
        $this->sql_query = "select * from `employee_non_preferred_timings` WHERE `employee` = '".$employee."' AND `mode` = '".$mode."' ORDER BY `group_id`,`day` ASC ";
        return $this->query_fetch();
    }

    function get_all_employee_non_prefered_time($customer, $date_from, $date_to, $mode, $possible_employees = array()){
        $this->sql_query = "select enpt.*, e.first_name, e.last_name FROM `employee_non_preferred_timings` enpt INNER JOIN `employee` e ON enpt.`employee` = e.`username` WHERE enpt.`mode` = '".$mode."'"; 
        if($customer){
            $this->sql_query = "select enpt.*, e.first_name, e.last_name FROM `employee_non_preferred_timings` enpt INNER JOIN `employee` e  INNER JOIN `team` t ON enpt.`employee` = e.`username` AND enpt.employee like t.employee    WHERE enpt.`mode` = '".$mode."' AND  t.customer ='".$customer."'";
        }
        if($date_from != ''){
            $this->sql_query .= " AND enpt.date_to >= '".$date_from."'";
        }
        if($date_to != ''){
            $this->sql_query .= " AND enpt.date_from <= '".$date_to."'";
        }
        if(!empty($possible_employees)){
            $this->sql_query .= " AND enpt.employee IN('".  implode('\',\'', $possible_employees) . "')";
        }

        $this->sql_query .= " ORDER BY enpt.employee, enpt.`group_id`,enpt.`day` ASC ";
        return $this->query_fetch();
    }
    ...
    function get_email_option_of_employee($user_id){
    ...
    function insert_email_options($user_id,$email_option,$action = null){
        $email_option = implode(',', $email_option);
        $email_option .= ',';
        $this->tables = array('leave_notification');
        $this->fields = array('employee','email');
        $this->field_values = array($user_id,$email_option);
    ...
    function get_all_email_option(){
        $this->tables           = array('leave_notification');
        $this->query_generate();
        return $this->query_fetch();
    }

    function get_email_option_mail_person($mail_persons_employee){
        // 27 - non prefered email option
        $this->sql_query = "select  ln.`employee`,ln.`email` as `email_option`,e.`email` from `leave_notification` ln LEFT JOIN `employee` e ON ln.`employee` = e.`username`  WHERE ln.`employee` IN ('".implode("','",$mail_persons_employee)."') AND FIND_IN_SET(27,ln.`email`)";
        return $this->query_fetch();
    }

    function get_max_value_of_checklist_sortorder(){
        $this->sql_query = "SELECT MAX(`sort_order`) as `max_sortorder` FROM `employee_check_list`";
        return $this->query_fetch()[0]['max_sortorder'];
    }

    function save_new_checklist($checklist_description,$max_sortorder,$alloc_employee){
        
        $this->tables = array('employee_check_list');
        $this->fields = array('description', 'sort_order','alloc_employee');
        $this->field_values = array($checklist_description,$max_sortorder,$alloc_employee);
    ...
    function get_all_employee_checklist($sort_order = null){
        $this->tables           = array('employee_check_list');
        if($sort_order == null){
            $this->order_by         = array('sort_order ASC');
            $this->query_generate();
            return $this->query_fetch();
        }
        else{
    ...
    function edit_single_checklist($id,$description){
        $this->tables           = array('employee_check_list');
        $this->fields           = array('description');
        $this->field_values     = array($description);
        $this->conditions       = array('id = ?');
        $this->condition_values = array($id);
        return $this->query_update();
    }

    function delete_single_checklist($id){
        $this->tables           = array('employee_check_list');
        $this->conditions       = array('id = ?');
        $this->condition_values = array($id);
        return $this->query_delete();
    ...
    function change_the_sort_order($new_sort_order){
        $string = '';
        foreach ($new_sort_order as $id => $sort_order) {
            $string .= '
                WHEN `id` = '.$id.' THEN '.$sort_order.'
            ';
        }
        $id_to_update = array_keys($new_sort_order);
        $id_to_update = implode("','",$id_to_update);
        $this->sql_query = " UPDATE `employee_check_list`
                            SET `sort_order` = (CASE    
                                                    ".$string."
                                                END)
                            WHERE `id` IN('".$id_to_update."')";
        if(empty($this->query_fetch())){
            return true;
        }
        else{
            return FALSE;
    ...
    function get_all_cv($employee_name = '',$cv_title = '',$cv_year = ''){
        $conditions = array();
        $this->sql_query = "SELECT e.username,e.first_name, e.last_name,YEAR(es.exam_date) as `year`,es.skill,es.description,es.attachment1,es.attachment2,es.attachment3 FROM `employee_skill` es LEFT JOIN `employee` e ON es.employee = e.username ";
        if($employee_name != ''){
            $name_search = $_SESSION['company_sort_by'] == 1 ? "LOWER(CONCAT(e.first_name,' ',e.last_name)) LIKE '%$employee_name%'" : "LOWER(CONCAT(e.last_name,' ',e.first_name)) LIKE '%$employee_name%'" ;
            $conditions[] .= $name_search;
        }
        if($cv_title != '')
            $conditions[] = "(LOWER(es.skill) LIKE '%$cv_title%' OR LOWER(es.description) LIKE '%$cv_title%')";
        if($cv_year != '')
            $conditions[] = "YEAR(es.exam_date) = $cv_year";
        // var_dump($conditions);
        if(count($conditions) > 0){
            $this->sql_query .= "WHERE " . implode (' AND ', $conditions);
        }
        return $this->query_fetch();
    }

    function employee_with_skills(){
        $this->sql_query = "SELECT e.username,e.first_name,e.last_name from `employee_skill` es LEFT JOIN `employee` e on es.employee = e.username GROUP by e.username ";
        return $this->query_fetch();
    }

    function distinct_timetable_years(){
        $this->sql_query = "SELECT DISTINCT year(`exam_date`) year FROM `employee_skill` WHERE exam_date IS NOT NULL";
        return $this->query_fetch(2);
        
    }

    function check_unique_employee_code($code, $username = NULL){
        $this->sql_query = "SELECT * FROM `employee` WHERE `code` LIKE '$code' ";
        if($username) {
            $this->sql_query .= " AND username != '$username'";
        }
    ...
    function get_employee_contract($current_date){
        $this->sql_query = "SELECT ec.id,ec.employee,ec.date_from,ec.date_to,e.email,e.first_name,e.last_name FROM `employee_contract` ec LEFT JOIN `employee` e ON ec.employee = e.username WHERE ec.date_to > '$current_date' ";
        return $this->query_fetch() ;
    }
