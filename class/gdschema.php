...
$expire_days = floor(($date_after-time())/(60*60*24));
if($query_string[0] == 'l'){
...
foreach ($week_shedules as $week_shedule){
                    $tbl_data .= '<tr>';
                        $tbl_data .= '<td width="236" style="text-align: left;" class="cust_name">';
                        ...
                        foreach ($week_shedule['week_datas'] as $week_data){
                            $tbl_data .= '<td  width = "112 " ';
                            if ($week_data['allocation'] >= $week_data['total_hours'] && $week_data['total_hours'] != 0)
                                    $tbl_data .= 'style="background: none repeat scroll 0 0 #d7ffe8"';
                                else if ($week_data['total_hours'] != 0)
                                    $tbl_data .= 'style="background: none repeat scroll 0 0 #F5BE87"';
                                else if ($week_data['week']['year_week'] == $year_week)
                                    $tbl_data .= 'style="background: none repeat scroll 0 0 #c7edf5"';
                                $tbl_data .= '>';
                                
                                /*$tmp_year_week = explode('|', $week_data['week']['year_week']);
                                $temp_year = $tmp_year_week[0];
                                $temp_month = date('n', strtotime("$temp_year-W".$week_data['week']['week']));*/
