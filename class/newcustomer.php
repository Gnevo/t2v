...
class newcustomer extends db {
	...
	function get_Unalloc_schedule_Hours_of_cust($cust,$date) {
        ...
        if($fromdate <= $boundary_date && $todate > $boundary_date){
			...
			//$this->tables = array('backup_timetable` as `t');
			//$proceed      = true;
			$this->sql_query = "SELECT SUM(TIMESTAMPDIFF(MINUTE,STR_TO_DATE(CONCAT(date,' ',time_from),'%Y-%m-%d %H.%i'),COALESCE(STR_TO_DATE(CONCAT(date,' ',time_to),'%Y-%m-%d %H.%i'),date + INTERVAL 1 DAY))/60) AS hrsmins FROM `backup_timetable` WHERE customer='".$cust."' AND status=0 AND date='".$date."' AND employee IS NULL";
        }
        else if($fromdate > $boundary_date && $todate > $boundary_date){
			//$this->tables = array('timetable` as `t');
			//$proceed      = true;
			$this->sql_query = "SELECT SUM(TIMESTAMPDIFF(MINUTE,STR_TO_DATE(CONCAT(date,' ',time_from),'%Y-%m-%d %H.%i'),COALESCE(STR_TO_DATE(CONCAT(date,' ',time_to),'%Y-%m-%d %H.%i'),date + INTERVAL 1 DAY))/60) AS hrsmins FROM `timetable` WHERE customer='".$cust."' AND status=0 AND date='".$date."' AND employee IS NULL";
        }
        //echo $cust."_".$date."</br>";
        //if($proceed == true){
			//$this->fields = array("SUM(TIMESTAMPDIFF(MINUTE,STR_TO_DATE(CONCAT(t.date,' ',t.time_from),'%Y-%m-%d %H.%i'),COALESCE(STR_TO_DATE(CONCAT(t.date,' ',t.time_to),'%Y-%m-%d %H.%i'),t.date + INTERVAL 1 DAY))/60) AS hrsmins");
			//$this->conditions = array('AND', 't.status = 0',"t.employee = ''","t.customer = '".$cust."'","t.date = '".$date."'");
			//$this->conditions = array('AND', 't.status = 0',"t.employee = NULL","t.customer = '".$cust."'","t.date = '".$date."'");
			//$this->query_generate();
			//$this->sql_query = "SELECT SUM(TIMESTAMPDIFF(MINUTE,STR_TO_DATE(CONCAT(date,' ',time_from),'%Y-%m-%d %H.%i'),COALESCE(STR_TO_DATE(CONCAT(date,' ',time_to),'%Y-%m-%d %H.%i'),date + INTERVAL 1 DAY))/60) AS hrsmins FROM `timetable` WHERE customer='".$cust."' AND status=0 AND date='".$date."' AND employee IS NULL";
