...
$smarty->assign('menu', array('mainmenu' => 1, 'submenu' => 2, 'tabmenu'=>'employee_privileges'));
$smarty->assign('employee_username',$_SERVER['QUERY_STRING']);
$smarty->assign('employee_detail',$employee_detail);
$smarty->assign('pre_role',$user->user_role($_SERVER['QUERY_STRING']));
$smarty->assign('tab','1');
$smarty->assign('customer_selected','');
...
if($privilege_general['employee_settings_privileges'] != 1){
    $messages->set_message('fail', 'permission_denied');
    $obj_general->going_to_startup_view($smarty);
    exit();
}

$smarty->assign('customers_to_employee',$team->customers_for_employee_team($employee_detail[0]['username']));
...
if(isset($_POST['curr_tab'])){
    ...
    if($curr_tab == 1){
        ...
        $data = $employee->add_previleges($employees,$_POST['cust']);
        $smarty->assign('customer_selected',$_POST['cust']);
        ...
        // var_dump( $employee->mc_notes_attchment );exit();

        $data = $employee->add_previleges_mc($employees);  
        // if($employee->mc_notes_attchment == null) $employee->mc_notes_attchment=0;
...
/*if(isset($_POST['user'])){
    if($_POST['new1'] == "new"){
       ...
       if($equipment->add_employee_previlege($_POST['user'],$_POST['swap'],$_POST['process'])){
            $message = 'previlege_add_success';
            $type = "success";
            $messages->set_message($type, $message);
       } 
       else{
           $message = 'previlege_add_fail';
            $type = "fail";
            $messages->set_message($type, $message);
       }
    }
    else{
        if($equipment->update_employee_previlege($_POST['user'],$_POST['swap'],$_POST['process'])){
            $message = 'previlege_update_success';
            $type = "success";
            $messages->set_message($type, $message);
        }
        else{
            $message = 'previlege_update_fail';
            $type = "fail";
            $messages->set_message($type, $message);
        }
    }
...
$previleges = $equipment->get_employee_previlege($_SERVER['QUERY_STRING']);
if($previleges == 1){
    $smarty->assign('new','new');
}
else{
    $smarty->assign('previleges',$previleges);
}*/
$smarty->assign('message', $messages->show_message());
...
$smarty->display('extends:layouts/dashboard.tpl|employee_privileges.tpl|layouts/sub_layout_employee_tabs.tpl');
?>
