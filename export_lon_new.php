...
require_once('class/converter_new.php');
...
function exportExists($month, $year, $app, $signedUsers) {
    ...
    //echo $year.$month.$app;
    $query = $sql->prepare("SELECT employee_exported,customers FROM exports_lon_test WHERE year=? AND month=? AND app=?");
    $query->execute(array(
        $year,
        $month,
        $app
...
if (isset($_POST['del_file'])) {
     $query = $sql->prepare("DELETE from exports_lon_test WHERE filename= ?");
    ...
    $filenNameCommon = $_POST['year'] . sprintf('%02d', $_POST['month']) . '_' . $_SESSION['user_id'] . '_' . $dtz->format('Ymd_His'); //date('Ymd_His');
        if(isset($_POST['export']) && isset($_POST['export_data']) && $_POST['export_data'] !=''){
            ...
            foreach($aUsersSigned as $employee => $customers_data){
                ...
                $query = $sql->query(
                            "INSERT IGNORE INTO exports_lon_test (year,month,timestamp,employee,employeeName,filename,employee_exported,customers,app) VALUES (" .
                            (int) $_POST['year'] . "," .
                            (int) $_POST['month'] . "," .
            ...
            file_put_contents(dirname(__FILE__) . '/' . $company_folder['upload_dir'] . '/salary_new/' . $fileName, html_entity_decode($_POST['export_data']));
        }
        else{
        ...
        if (count($aUsersSigned) == 0) {
            ...
            if ($aUsersSigned !== false) {
                ...
                switch (strtolower(trim($_POST['app']))) {
                    ...
                    case 'crona':
                        $fileName = 'crona_' . $filenNameCommon . '.pax';
                        $x = $conv->Crona(true, $date, $c_list_post, $aUsersSigned, $export, $error, $company_details['fkkn_split']);
                        //echo $error;
                        break;
...
//                echo "-------------------------------------------";
                ...
                /*if ($_POST['customer'] == "") {
                    $query = $sql->query(
                            "INSERT IGNORE INTO exports_lon_test (year,month,timestamp,employee,employeeName,filename,employees,app) VALUES (" .
                            (int) $_POST['year'] . "," .
                            (int) $_POST['month'] . "," .
                            strtotime($dtz->format('Y-m-d H:i:s')) . "," .
                            $sql->quote($_SESSION['user_id']) . "," .
                ...
                if($error){
                    ...
                    file_put_contents(dirname(__FILE__) . '/' . $company_folder['upload_dir'] . '/salary_new/' . $fileName, $export);
                    }else{
                        //echo "<pre>\n".print_r($aUsersSigned , 1)."</pre>";
...
// download exports
$query = $sql->prepare("SELECT distinct filename, count(distinct customers) as customer_count, customers, timestamp,employee,year,month,app FROM exports_lon_test WHERE year=? AND month=? GROUP BY filename ORDER BY `timestamp` DESC");
$query->execute(array(
    (int) $_POST['year'],
    (int) $_POST['month']
));
    ...
    if ($existing !== false) {
        foreach ($existing as $key => $value) {
            if ($value['filename'] == $_POST['filename']) {
                $dona = new dona();
                $company_folder = $dona->get_company_directory($_SESSION['company_id']);
                $file = dirname(__FILE__) . '/' . $company_folder['upload_dir'] . '/salary_new/' . $value['filename'];
...
$query = $sql->prepare("SELECT e.first_name,e.last_name,employee_exported,customers FROM exports_lon_test INNER JOIN employee e ON employee_exported LIKE e.username WHERE year=? AND month=? AND app=?");
    $query->execute(array(
        (int) $_POST['year'],
        (int) $_POST['month'],
        $salary_format
...
$smarty->display('extends:layouts/dashboard.tpl|export_lon_new.tpl');