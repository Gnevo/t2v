...
if($_POST['action'] == 'save' && $_POST['customer'] != '') {
    ...
    $review_id = $customer_forms->form_3_insert($form_data);
    if($review_id) {
        $messages->set_message("success", 'form_3_adding_success');
    } else {
        $messages->set_message("fail", 'form_3_adding_fail');
    }
    header("Location: form_3.php?action=pdf&review=$review_id&customer=$customer_id");
    exit();
}
if($_POST['action'] == 'pdf' && $_POST['review'] != '' && $_POST['customer'] != '') {
    $review_id = ($_REQUEST['review'] ? $_REQUEST['review'] : 0);
    if($review_id) {
        $review_data = $customer_forms->get_form_3_by_id($review_id);
        $form_questions = $customer_forms->get_form_3_questions();
        if(!empty($review_data)) {
            ...
            $pdf->SetXY(15, 24);
            $pdf->Cell(90, 4, utf8_decode('Trivselenkät Assistenter'), 0, 0, 'L', FALSE);
            ...
            $pdf->setY(55);
            $pdf->SetX(12);
            $pdf->MultiCell(186, 6, utf8_decode('Vi strävar efter att leverera bästa möjliga arbetssituation för våra assistenter och en hög kvalitetsnivå för att kunna bemöta våra kunder på ett professionellt sätt.'), 0, 'L');
            $pdf->setY($pdf->GetY() + 2);
            $pdf->SetX(12);
            $pdf->MultiCell(186, 6, utf8_decode('Nedan följer ett antal frågor med betygsättning 1-6 där 1 är sämst och 6 är bäst.'), 0, 'L');
            $pdf->setY($pdf->GetY() + 2);
            $pdf->SetX(12);
            $pdf->MultiCell(186, 6, utf8_decode('Kryssa för det betyg som du tycker stämmer med det du har upplevt som anställd hos oss på'), 0, 'L');
            $pdf->setY($pdf->GetY());
            $pdf->SetFont('Arial', 'B', 12);
            $pdf->SetX(12);
            ...
            $pdf->Line(12, $starty, 12, $pdf->GetY());
            $pdf->Line(138, $starty, 138, $pdf->GetY());
            $pdf->Line(148, $starty, 148, $pdf->GetY());
            $pdf->Line(158, $starty, 158, $pdf->GetY());
            $pdf->Line(168, $starty, 168, $pdf->GetY());
            $pdf->Line(178, $starty, 178, $pdf->GetY());
            $pdf->Line(188, $starty, 188, $pdf->GetY());
            $pdf->Line(198, $starty, 198, $pdf->GetY());
            $starty = $pdf->GetY();
            //Filed data 1
            $i = 1;
            ...
            $pdf->setY($lasty);
            $pdf->SetX(12);
            $pdf->Cell(50, 6, utf8_decode('Övriga synpunkter:'), 0, 1, 'L', FALSE);
            $lasty = ($pdf->GetY());
            $pdf->setY($lasty);
            $pdf->SetFont('Arial', '', 12);
            $pdf->SetX(12);
            $pdf->MultiCell(186, 4, utf8_decode($review_data['field_description']), 0, 'L');
            ...
            $pdf->setY($lasty);
            $pdf->SetX(12);
            $pdf->Cell(30, 6, utf8_decode('Personnummer:'), 0, 0, 'L', FALSE);
            $pdf->SetX(42);
            $pdf->Cell(155, 6, utf8_decode($employees[$review_data['created_by']]['century'] . $employees[$review_data['created_by']]['social_security']), 0, 1, 'L', FALSE);
            $lasty = $pdf->GetY();

            $pdf->setY($lasty);
            $pdf->SetX(12);
            $pdf->Cell(15, 6, utf8_decode('Namn:'), 0, 0, 'L', FALSE);
            $pdf->SetX(28);
            $pdf->Cell(170, 6, utf8_decode(($_SESSION['company_sort_by'] == 1 ? $employees[$review_data['created_by']]['first_name'] . ' ' . $employees[$review_data['created_by']]['last_name'] : $employees[$review_data['created_by']]['last_name'] . ' ' . $employees[$review_data['created_by']]['first_name'])), 0, 1, 'L', FALSE);
...
$form_datas = $customer_forms->get_form_3();
...
if($_REQUEST['action'] == 'save' || $_REQUEST['action'] == 'pdf') {
    $smarty->display('extends:layouts/dashboard.tpl|forms/form_3.tpl');
} else {
    $smarty->display('forms/form_3.tpl');
}
