...
if($_POST['action'] == 'save' && $_POST['customer'] != '') {
    ...
    $form_data = array(
        ...
        'deviation_date' => (strtotime($_POST['deviation_date']) ? date('Y-m-d', strtotime($_POST['deviation_date'])) : NULL), 
...
if($_POST['action'] == 'pdf') {
    ...
    foreach ($field_groups as $group_id => $groups) {
        ...
        foreach ($groups['fields'] as $key_id => $field) {
            ...
            $pdf->setY($starty);
            $pdf->SetX($startx);
            $pdf->MultiCell(90, 6, html_entity_decode(utf8_decode($field['name'])), 0, 'L');
            foreach ($field['options'] as $option_index => $option) {
                $option_val = $option_index + 1;
                $pdf->setY($pdf->GetY());
                $pdf->SetX($startx + 5);
                $pdf->MultiCell(85, 6, ($review_data['answers'][$field['id']] == $option_val ? '[X]' : '[  ]') . '  ' . html_entity_decode(utf8_decode($option)), 0, 'L');
                if($field_count%2) {
                    $start_2_y = $pdf->GetY();
                } else {
                    $start_1_y = $pdf->GetY();
