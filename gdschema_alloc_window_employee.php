...
$smarty->assign('privileges_gd', $employee->get_privileges($_SESSION['user_id'], 1, (isset($_REQUEST['customer']) ? $_REQUEST['customer'] : '')));           //setting gdschema previlege
...
} else if (isset($_REQUEST['employee']) && !isset($_REQUEST['customer']) && $_REQUEST['employee'] != "") {
...
$employees_customer = $team->employees_for_customer_team_gdschema_alloc((isset($_REQUEST['customer']) ? $_REQUEST['customer'] : ''),$_REQUEST['date']);
...
$all_customers_of_employee = $employee->get_team_customers_of_employee($_REQUEST['employee']);
...
$search_employees = $employee->employees_list_for_right_click($_REQUEST['employee']);
...
if(date('Y-m-d') == $_REQUEST['date']){
    ...
    $employee_running_tasks = $obj_timetable->get_employee_running_tasks($_REQUEST['employee']);
...
$smarty->display('gdschema_alloc_window_employee.tpl');
