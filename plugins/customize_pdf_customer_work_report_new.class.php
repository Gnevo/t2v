   ...
   // var $current_customer_employee_total_oncall_hours = 0;  //this variable used for section 7 (now not used)
    ...
    function SubPart7_table($work_details, $fkkn = NULL) {
       ...
       // echo "<pre>".print_r($work_details, 1)."</pre>";
        ...
        //$total_Tid = round($total_Tid);
        //$toal_Jourtid = round($toal_Jourtid);
        //$toal_beredskap = round($toal_beredskap);//
        $total_Tid_formatted        = explode('.', sprintf('%.02f', round($obj_equipment->time_user_format($total_Tid, 60, TRUE), 2))) ;
        $toal_Jourtid_formatted     = explode('.', sprintf('%.02f', round($obj_equipment->time_user_format($toal_Jourtid, 60, TRUE), 2)));
        $toal_beredskap_formatted   = explode('.', sprintf('%.02f', round($obj_equipment->time_user_format($toal_beredskap, 60, TRUE), 2)));
    ...
    function summery_report_top($month, $year, $fkkn, $kn_headings = array(), $company_data = array()) {
        ...
        if(!empty($kn_headings) && ($fkkn == 2 || $fkkn == 3)){
            $this->SetXY(13, 8);
            $this->Cell(62, 30, '', 0, 0, 'L', TRUE);      // hide backround header image
