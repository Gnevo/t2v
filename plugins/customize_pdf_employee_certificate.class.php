    ...
    function P3_SubPart2($summary_data, $oncall_kr_times, $ob_kr_times,$timers,$angearts) {
        ...
        if(!empty($summary_data)){
            foreach($summary_data as $data){
                ...
                if(!empty($angearts[$data['year_month']]) || !empty($ob_kr_times[$data['year_month']])) {
                    $new_array = array();
                    if(!empty($angearts[$data['year_month']])) {
                        $new_array = $angearts[$data['year_month']];
                    } else if($ob_kr_times[$data['year_month']]) {
                        $new_array = $ob_kr_times[$data['year_month']];
                    }
                    for($i=0; $i < count($new_array); $i++) {
                        $this->Cell($w[4], $col_h, (isset($angearts[$data['year_month']][$i]) ? utf8_decode($angearts[$data['year_month']][$i]) : ''), 0, 0, 'L', FALSE);   //table cell 4   
                        $this->Cell($w[5], $col_h, (isset($ob_kr_times[$data['year_month']][$i]) ? $ob_kr_times[$data['year_month']][$i] : ''), 0, 0, 'L', FALSE);
