...
//        echo "<pre>".print_r($customer_details, 1)."</pre>";
    ...
    function P1_table_3($comp_paid_to_account = NULL, $reference = NULL, $ord_table_sum = NULL) {
        ...
        $this->Cell(47, 6.25, number_format($ord_table_sum,2), 'BR', 0, 'L', FALSE); //Ordinary table sum
        $this->SetY($last_y);
    }
...
//            $this->Cell(22, 5, $Q_work_dates[0], 0, 0, 'L', FALSE);
    ...
    function P2_top($collective = NULL) {
     ...
     function P2_table1($table_f_date1, $table_to_date2, $insurance_word_person, $SS_contibution, $ord_inconv_details, $qualifying_day = array()) {
        ...
        $totalReplacableHour            = $qualifying_day['totalReplacableHour'];
        $totalAdditionalHour            = $qualifying_day['totalAdditionalHour'];
        $totalUnmannedHour              = $qualifying_day['totalUnmannedHour'];
        ...
        //echo "<pre>normal_2_to_14".print_r($normal_2_to_14, 1)."</pre>";exit;
        foreach($normal_2_to_14 as $amount => $hour_value){
            //$time_in_100    =   $equipment->time_user_format($hour_value['hour'], 100);
             
            $total_replacecable_hour    =   ($totalReplacableHour +  $totalAdditionalHour);
            $total_replacecable_hour    =   $equipment->time_user_format($hour_value['hour'], 100);
            $kerense_hour               =   $equipment->time_user_format($qualifying_day_total_hour, 100);
            // $time_in_100             =   $total_replacecable_hour - $kerense_hour;
            //$time_in_100                =   $total_replacecable_hour;
            $time_in_100                =   $total_replacecable_hour - $totalUnmannedHour;
           // echo  $total_replacecable_hour."==".$kerense_hour."===".$totalUnmannedHour."===".$time_in_100;exit;
            if($time_in_100 < 0){ $time_in_100 = 0;}
        ...
        foreach($normal_2_to_14 as $amount => $hour_value){
         //  $time_in_100 = $equipment->time_user_format($hour_value['hour'], 100);
          ...
          $total_replacecable_hour      =   ($totalReplacableHour +  $totalAdditionalHour);
           ...
           // echo  $total_replacecable_hour."==".$kerense_hour."===".$time_in_100;exit;
    ...
    function P2_table3($total_ordinarie_time,$totalUnmannedHour=0) {
        ...
        $calculateHour = (int)$total_ordinarie_time - (int)$totalUnmannedHour;
        //column headings
       ...
       // $this->Cell($w[2], $col_h, $total_ordinarie_time, 'BLR', 0, 'R', FALSE);   //table cell 3
        ...
        $this->Cell($w[2], $col_h, $calculateHour, 'BLR', 0, 'R', FALSE);   //table cell 3
