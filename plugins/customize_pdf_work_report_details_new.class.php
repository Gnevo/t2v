   ...
   // echo $rh;
    ...
    function P1_Part2_Landscap() {
        ...
        foreach($this->days_in_month as $date_key){
            ...
            //echo "<pre>".print_r($this->total,1)."</pre>";
            $contents = $this->total;
            foreach ($this->headings as $heads_key=>$ordered_heads){
                foreach ($ordered_heads as $type_key=>$ordered_types){
                    foreach ($ordered_types as $type_order_key=>$types){
                        foreach ($types as $main_head=>$head){
                            if(is_array($head)){
                                foreach ($head as $head_item){
                                    
                                    //echo "<pre>".print_r($contents[$heads_key][$type_key][$type_order_key],1)."</pre>";
                                    if (array_key_exists($heads_key, $contents) && array_key_exists($type_key, $contents[$heads_key]) && array_key_exists($type_order_key, $contents[$heads_key][$type_key]) && array_key_exists($head_item,$contents[$heads_key][$type_key][$type_order_key])){
                                            $this->Cell($rw, $col_hi, number_format($contents[$heads_key][$type_key][$type_order_key][$head_item],2,'.','') , 1, 0, 'C', TRUE);
                                    }else{
                                         $this->Cell($rw, $col_hi,'', 1, 0, 'C', TRUE);
                                    }
                                }
                                ...
                                if (array_key_exists($heads_key, $contents) && array_key_exists($type_key, $contents[$heads_key]) && array_key_exists($type_order_key, $contents[$heads_key][$type_key]) && array_key_exists($head,$contents[$heads_key][$type_key][$type_order_key])){
                                            $this->Cell($rw, $col_hi, number_format($contents[$heads_key][$type_key][$type_order_key][$head],2,'.','') , 1, 0, 'C', TRUE);
                                }else{
                                     $this->Cell($rw, $col_hi,'', 1, 0, 'C', TRUE);
                                }
                            }                          
            ...
            $contents = $this->total_leave;
            //echo "<pre>".print_r($this->headings_leave,1)."</pre>";
            foreach ($this->headings_leave as $leave_type=>$heads_main){
                foreach ($heads_main as $heads_key=>$ordered_heads){
                    foreach ($ordered_heads as $type_key=>$ordered_types){
                        foreach ($ordered_types as $type_order_key=>$types){
                            foreach ($types as $main_head=>$head){
                                if(is_array($head)){
                                    foreach ($head as $head_item){  
                                        ...
                                        if (array_key_exists($leave_type, $contents) && array_key_exists($heads_key, $contents[$leave_type]) && array_key_exists($type_key, $contents[$leave_type][$heads_key]) && array_key_exists($type_order_key, $contents[$leave_type][$heads_key][$type_key]) && array_key_exists($head_item,$contents[$leave_type][$heads_key][$type_key][$type_order_key])){
                                            ...
                                            $this->Cell($rw, $col_hi,number_format($contents[$leave_type][$heads_key][$type_key][$type_order_key][$head_item],2,'.',''), 1, 0, 'C', TRUE);
                                        }else{
                                            $this->Cell($rw, $col_hi,'', 1, 0, 'C', TRUE);
                                        }
                                ...
                                elseif($head == 'sum'){
                                    if (array_key_exists($leave_type, $contents) && array_key_exists($heads_key, $contents[$leave_type]) && array_key_exists($type_key, $contents[$leave_type][$heads_key]) && array_key_exists($type_order_key, $contents[$leave_type][$heads_key][$type_key]) && array_key_exists($head,$contents[$leave_type][$heads_key][$type_key][$type_order_key])){

                                        $this->Cell($rw, $col_hi,number_format($contents[$leave_type][$heads_key][$type_key][$type_order_key][$head],2,'.',''), 1, 0, 'C', TRUE);
                                    }else{
                                        $this->Cell($rw, $col_hi,'', 1, 0, 'C', TRUE);
                                    }
                                }
