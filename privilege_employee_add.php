...
$team = new team();
$smarty = new smartySetup(array("user.xml", "messages.xml", "button.xml","privilege.xml","tooltip.xml"));
$smarty->assign('menu', array('mainmenu' => 1, 'submenu' => 9));
//$query_string = $_SERVER['QUERY_STRING'];
//$smarty->assign('selected_emp',$query_string);
$selected_emp = '';
if(isset($_POST['selected_emp'])){
    if($_POST['select_all'] == '1'){
        $smarty->assign('pre_role',$_POST['pre_role']);
        $selected_emp = '';
        $employees_selected = $team->get_employee_autocomplete('',$_POST['pre_role']);
        for($i=0;$i<count($employees_selected);$i++){
            if($selected_emp == ''){
                $selected_emp = $employees_selected[$i]['username'];
            }else{
               $selected_emp = $selected_emp.",". $employees_selected[$i]['username'];
            }
        }
        $smarty->assign('employees_selected',$employees_selected);
        $smarty->assign('select_all','1');
        $_SESSION['select_all'] = "1";
    }else{
        $_SESSION['selected_prev'] = $_POST['selected_emp'];
        $smarty->assign('selected_emp',$_POST['selected_emp']);
        $smarty->assign('pre_role',$_POST['pre_role']);
        $selected_emp = $_POST['selected_emp'];
        $_SESSION['select_all'] = "";
    } 
}
$smarty->assign('selected_emp',$selected_emp);
$smarty->assign('tab','1');
//echo "<pre>". print_r($privileges, 1)."</pre>";
$privileges = $employee->get_privileges_employee($selected_emp);
$no_change = 0;
...
if(isset($_POST['curr_tab'])){
    ...
    $smarty->assign('pre_role',$_POST['roles']);
    $selected_emp = $_POST['employees'];
    ...
    if($curr_tab == 1){
        ...
        $employee->report_available_employees = isset($_POST['employee_available_report']) && trim($_POST['employee_available_report']) == 1 ? $_POST['employee_available_report'] : 0;
        ...
        // $employee->mc_notes_attchment = isset($_POST['mc_notes_attchment']) && trim($_POST['mc_notes_attchment']) == 1 ? $_POST['mc_notes_attchment'] : 0;
    ...
    // Last Edit sreerag 07/05/2018  privilege setting
    if($data){
        $messages->set_message('success', 'privilege_saved');
    }else{
        $messages->set_message('fail', 'privilege_failed');
    }
}
if(isset($_POST['select_all']) && $_POST['select_all'] == '1'){
    $smarty->assign('select_all','1');
}
$selected = explode(',', $selected_emp);
$smarty->assign('message', $messages->show_message());
$smarty->assign('selected_emp',$selected_emp);
$employees_selected = $employee->get_employee_detail_privilege($selected);
$smarty->assign('employees',$employees_selected);
$smarty->display('extends:layouts/dashboard.tpl|privilege_employee_add.tpl');
?>