...
{*{if $flg eq 2}
    <span id="signing" class="signing">
        <span id="signing_message" name="signing_message" class="signing_error">
            {$translate.invalid_username_or_password}
        </span>
        <span class="signing_form">
            <label for="username">{$translate.username}</label>
            <input name="username" id="username" type="text" value="{$uname}" />

            <label for="password">{$translate.password}</label>
            <input name="password" id="password" type="password" value="{$pword}" />
        </span>
    ...
    {if $insert eq "true"}
        <span id="signing" class="signing">
            <span id="signing_message" name="signing_message" class="signing_success">
                {$translate.signing_done_sucessfully}
            </span>
            {if $login_user_role eq 1}
                <a name="login" id="login" class="delete" title="{$translate.delete}" href="javascript:void(0)" onclick="sign_remove()"></a>
                 <input type="hidden" id="sign_sutl_id" value="{$signin_sutl}">
            {/if}
        </span>
    {elseif $insert eq "false"}
        <span id="signing" class="signing">
            <span id="signing_message" name="signing_message" class="signing_error">
                {$translate.error_occured_in_signing_try_again}
            </span>
            <span class="signing_form">
                <label for="username">{$translate.username}</label>
                <input name="username" id="username" type="text" value="{$uname}" />
