...
{literal}
    ...
    function check_dates()
    {
            ...
            if(hrs != "" & date_from != "" & date_to != "")
            {
                            $.ajax({
                                            async:false,
                                            url:"{/literal}{$url_path}{literal}ajax_emp_contract_check_date.php",
   ...
   // function show_hours()
    ...
    function validate()
    {
            ...
            {
                    $("#err_msg").html("'Date to' should be greater than Date from'");
                        $("#err_msg").addClass('error');
                    return false;
            }
            return check_dates();
...
<div class="tbl_hd"><span class="titles_tab">{$translate.add_contr} - {$user_display_name}</span>
    <a onclick="saveForm()" href="javascript:void(0)" class="save"><span class="btn_name">{$translate.save}</span></a>
    <a onclick="resetForm()" href="javascript:void(0)" class="reset"><span class="btn_name">{$translate.reset}</span></a>
...
<div class="add_contract_main">
    ...
    <div class="contract_left_area">
        ...
        <div class="contract_details_forms" onkeyup="show_hours()">
            <form id="form" name="form" method="post" action="{$url_path}contract/employee/add/{$username}/{if $data.hour != ""}{$data.id}/{/if}" onsubmit="return validate();">
