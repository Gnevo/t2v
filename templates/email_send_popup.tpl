            ...
            {foreach from=$employees_group item=employee}
                if(username == '{$employee.customer_username}'){
                    {foreach from=$employee.employees_customer item=empl}
                        ...
                        value = '{$empl.first_name} {$empl.last_name}<{$empl.email}>';
                        if(mail_ids == ''){
                            mail_ids = value+', ';
                        }
                        else{
...
function set_all(){
     if($('#all').attr('checked')){
        var values = "";
        {foreach from=$employees item=empl}
           if(values == ""){
               values = '{$empl.first_name} {$empl.last_name}<{$empl.email}>'+",";
            }
            else{
                values = values+'{$empl.first_name} {$empl.last_name}<{$empl.email}>'+",";
            }
         {/foreach}
            {foreach from=$employees_group item=employee}
  ...
  <div id="options_panel">
    ...
    {foreach from=$employees_group item=employee}
        ...
        <div class="mailing_group">
            ...
            {foreach from=$employee.employees_customer item=empl}
                <li>
                    <label>{$empl.first_name} {$empl.last_name}({$empl.email})</label>
                    <span class="mail_grup_customer">
                    <input type="checkbox" id="cech_{$empl.username}_{$employee.customer_username}" name="cech_{$empl.username}_{$employee.customer_username}" onclick="addElements('{$empl.username}','ce','{$employee.customer_username}')"  value="{$empl.first_name} {$empl.last_name}<{$empl.email}>"/>
                    </span>
                </li>                
            {/foreach}
            </ul>
    ...
    {foreach from=$employees_group item=employee}
        {if $employee.customer_name == 'ALL'}
        <li>
            <label>{$employee.employees.first_name} {$employee.employees.last_name}({$employee.employees.email})</label>
            <span class="mail_grup_customer">
            <input type="checkbox" id="cach_{$employee.employees.username}" name="cach_{$employee.employees.username}" onclick="addElements('{$employee.employees.username}','ca','0')" value="{$employee.employees.first_name} {$employee.employees.last_name}<{$employee.employees.email}>"/>
            </span>
        </li>
