{block name="script"}
    ...
    <script src="{$url_path}js/rleavereport.js" type="text/javascript" ></script>
    <script>
        ...
        <table class="table_list">
            <div class="option_strip">
                <div style="color:red; display:none;" align="center" id="errormsg" >{$translate.todate_greaterthan_fromdate_error}</div>
                    ...
                    <div class="span12 no-ml" id="leave_types">
                        {foreach from=$leave_types key=leave_type_key item=leave_type}
                            <div class="clearfix mr pull-left">
                                <label><input type="checkbox" checked="checked" value="{$leave_type_key}" title="{$leave_type}" class="leave_type" name="{$leave_type}" /> {$leave_type}</label>
                            </div>
                        {/foreach}
                    ...
                    {$translate.employee} <input type="text" name="emp" id="emp" maxlength="150"  autocomplete="off" aria-haspopup="true" aria-autocomplete="list" role="textbox" autocomplete="off" class="ui-autocomplete-input" style="margin-top:5px;"/>
