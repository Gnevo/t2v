...
function scroller()
    {
        $('.scroll_content, #memory_slot_grouping').jScrollPane();
...
$(document).ready(function(){
        {if $slots_detail_width != ""}
            $("#assigned_slots").css("width", "{$slots_detail_width}");
        {/if}
        {if $slots_time_width != ""}
            $("#memory_slots").css("width", "{$slots_time_width}");
        {/if}
...
$(".memory_time").draggable({ revert: true, appendTo: "#memory_slots", helper: 'clone' });
...
function empRemove(url){
    if(confirm('{$translate.confirm_delete}')){
        wrapLoader("#timetable_assign");
        $('#alloc_action').load(url,function(response, status, xhr){ uwrapLoader("#timetable_assign"); });
    }
}
...
function AssignCustomerMultiple(url){
    url = url+"&ids="+$('#multi_slot_id').val();
    var slots =  $("#multi_assign").val();
    var slots_array = slots.split(",");
    var over_lap = 0;
...
function employee_assign_slot(num,slot,slot_id){
    var multi_assign = $("#multi_assign").val();
    var multi_slot_id = $("#multi_slot_id").val();
    if($('#slot_'+num).attr('checked')){
        if(multi_assign == ""){
            multi_assign = slot;
            multi_slot_id = slot_id;
        }else{
            multi_assign = multi_assign+","+slot;
            multi_slot_id = multi_slot_id+","+slot_id;
        }
    }else{
        var temp_slots = "";
        var temp_slot_id = "";
        var temp_array = multi_assign.split(",");
        var temp_array_ids = multi_slot_id.split(",");
        for(var i=0;i<temp_array.length;i++){
            if(temp_array[i] != slot){
               if(temp_slots == ""){
                    temp_slots = temp_array[i];
                    temp_slot_id = temp_array_ids[i];
               }else{
                   temp_slots = temp_slots+","+temp_array[i]; 
                   temp_slot_id = temp_slot_id+","+temp_array_ids[i]; 
               }
            }
        }
        multi_assign = temp_slots;
        multi_slot_id = temp_slot_id;
    }
    
    $("#multi_assign").val(multi_assign);
    $("#multi_slot_id").val(multi_slot_id);
    var multi_assign_array = multi_assign.split(",");
    var array_empl = new Array();
    var j=0;
    for(var i=0;i<multi_assign_array.length;i++){
        var slots_array = multi_assign_array[i].split("-");   
        {foreach from=$employees_to_assign item=empls}
            var username = "{$empls.username}";
            {foreach from=$empls.time_slot item=time_slots}
                if((parseFloat("{$time_slots.time_from}") >= parseFloat(slots_array[0]) && parseFloat("{$time_slots.time_from}") <  parseFloat(slots_array[1])) ||(parseFloat("{$time_slots.time_to}") > parseFloat(slots_array[0]) && parseFloat("{$time_slots.time_to}") <=  parseFloat(slots_array[1])) || (parseFloat("{$time_slots.time_from}") < parseFloat(slots_array[0]) && parseFloat("{$time_slots.time_to}") >  parseFloat(slots_array[1]) ) ){
                     array_empl[j]=username;
                     j++;
                }
            {/foreach} 
       {/foreach}
   ...
   {/foreach}
    for(var i=0;i<array_empl.length;i++){
         $("#emp_"+array_empl[i]).hide();
    }
    
}
function customer_assign_slot(num,slot,slot_id){
    var multi_assign = $("#multi_assign").val();
    var multi_slot_id = $("#multi_slot_id").val();
    if($('#slot_'+num).attr('checked')){
...
//alert($('#timetable_assign').parent().val());
   $("#timetable_assign").dialog("close");
}
</script>
    ...
    <div class="left_col">
        <div class="time"> <span class="duration">0900 - 1400 <a href="#" class="btn_edit"></a></span>
            <form id="slot_drop" name="slot_drop" method="post" action="" class="cust_slot_frm clearfix">
                ...
                <input type="hidden" name="multi_assign" id="multi_assign"  value="" />
                <input type="hidden" name="multi_slot_id" id="multi_slot_id"  value="" />
...
<!--<div id="worker_company_name">
    {if $employee.name}<a href="javascript:void(0);" onclick="loadContent('{$url_path}gdschema_alloc.php?date={$cur_date}&employee={$employee.userid}')" id="worker_name">{$employee.name} {$employee.week_worked_hour}({$employee.contract_week_hour})<span style="float:right">{$cur_date}</span></a>{/if}
    {if $customer.name}<a href="javascript:void(0);" onclick="loadContent('{$url_path}gdschema_alloc.php?date={$cur_date}&customer={$customer.userid}')" id="company_name">{$customer.name} - FK: {$customer.week_worked_hour_fk}({$customer.contract_week_hour_fk}) KN: {$customer.week_worked_hour_kn}({$customer.contract_week_hour_kn}){if $employee.name == ''}<span style="float:right;">{$cur_date}</span>{/if}</a>{/if}
</div>-->
<div id="worker_company_name" class="clearfix">
    {if $employee.name}<a href="javascript:void(0);" onclick="loadContent('{$url_path}gdschema_alloc.php?date={$cur_date}&employee={$employee.userid}')" id="worker_name">{$employee.name} {$employee.week_worked_hour}({$employee.contract_week_hour})<span style="margin-left:33px;">{$cur_date}</span></a>{/if}
    {if $customer.name}<a href="javascript:void(0);" onclick="loadContent('{$url_path}gdschema_alloc.php?date={$cur_date}&customer={$customer.userid}')" id="company_name">{$customer.name} - FK: {$customer.week_worked_hour_fk}({$customer.contract_week_hour_fk}) KN: {$customer.week_worked_hour_kn}({$customer.contract_week_hour_kn}){if $employee.name == ''}<span style="margin-left:33px;">{$cur_date}</span>{/if}</a>{/if}
    <div class="save_close_buttons"><a class="save_close_btn" href="javascript:void(0);" onclick="dialogueClose()">Avbryt</a> <a class="save_close_btn" href="javascript:void(0);" onclick="dialogueClose()">Spara</a></div> 
</div>
<div class="clearfix" id="options">
    {if $employee.name == '' && $customer.name !=''}
        <div class="select_skill_scrollers">
            <div class="select_skilltitle">Välj anställd</div>
            <div id="scrollingarea"> 
                <div id="scrollings" class="clearfix" >
                    <ul>
                        {foreach from=$employees_to_assign item=emp}
                            <li  id="emp_{$emp.username}" onclick="AssignEmployeesMultiple('{$url_val}&empl={$emp.username}&action=multiple_slot_assign')">{$emp.last_name} {$emp.first_name} ({$emp.code})</li>
                        {foreachelse}
                            no employees
                        {/foreach}
    ...
    {if $employee.name != '' && $customer.name ==''}
        <div class="select_skill_scrollers">
            <div class="select_skilltitle">{$translate.select_customer}</div>
            <div id="scrollingarea"> 
                <div id="scrollings" class="clearfix" >
                    <ul>
                        {foreach from=$customers_to_assign item=cust}
                            <li  id="emp_{$cust.username}" onclick="AssignCustomerMultiple('{$url_val}&cust={$cust.username}&action=multiple_slot_assign_cust')">{$cust.last_name} {$cust.first_name} ({$cust.code})</li>
                        {foreachelse}
                            no customers
                        {/foreach}
    ...
    <div id="assigned_slots" >
        ...
        <div id="assigned_inner">
            <div class="scroll_content" style="overflow: hidden; padding: 0px;">
                {assign i 0}
                <div style="width: 290px;float: left">
                    {foreach $slot_entries_for_first_column as $slot_det}
                        <div id="d{$slot_det.id}" class="{if $slot_det.status == 1}custom_time_slots clearfix{else if $slot_det.status == 2}custom_time_slots_leave clearfix{else}custom_time_slots_incomplete clearfix{/if}" >
                            ...
                            {if $slot_det.employee == ''}<input type="checkbox" class="slot_assign_check" id="slot_{$i}" style="margin-top: 5px;margin-left: 2px;float: right;" onclick="employee_assign_slot('{$i}','{$slot_det.slot}','{$slot_det.id}')"/> {assign i $i+1}{/if}
                            {if $slot_det.customer == ''}<input type="checkbox" class="slot_assign_check" id="slot_{$i}" style="margin-top: 5px;margin-left: 2px;float: right;" onclick="customer_assign_slot('{$i}','{$slot_det.slot}','{$slot_det.id}')"/> {assign i $i+1}{/if}
                        </div>
                    {/foreach}
                </div>
    ...
    {if $privileges_gd.add_slot == 1}
        <div id="memory_slots">
            ...
            <div id="memory_inner" class="clearfix">
                ...
                <div id="memory_slot_grouping" class="clearfix" style="height: 395px;">
                    <div class="scroll_memory_inner">
                        {foreach $memory_slots1 as $available_slots}
                            <div class="memory_time clearfix">
                                <div style="float: left;padding-top: 2px;"><input type="checkbox" name="slot_{$available_slots.id}" id="slot_{$available_slots.id}"  onclick="multipleSlotTempAdd('{$available_slots.id}')" value="{$available_slots.time_from|cat:'-'|cat:$available_slots.time_to}"/><span>{$available_slots.time_from|cat:'-'|cat:$available_slots.time_to}<span></div>
