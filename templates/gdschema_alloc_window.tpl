{block name='style'}
    ...
    border: 1px solid #ccc; border-radius:5px;width:auto;float:left;padding:0px 5px;}
...
{elseif $employee.name == '' && $customer.name !=''*}
    ...
    {$url_val_slot = 'date='|cat:$cur_date|cat:'&employee='|cat:'&customer='|cat:$customer.userid|cat:'&emp_alloc='|cat:$emp_alloc}
    {$url_value_slot_remain = $url_path|cat:'ajax_gdschema_alloc_remainig_slots.php?date='|cat:$cur_date|cat:'&employee='|cat:'&customer='|cat:$customer.userid|cat:'&emp_alloc='|cat:$emp_alloc}
...
{*                   main header panel*}
                    <div class="widget-header span12">
                        <h1 class="pull-left"> 
                            ...
                            {$translate.customer}: {*$customer.name*}
                            <select class="ml" id="cmb_customer" name="cmb_customer" style="height: 28px;margin-top: -2px;">
                                {foreach $search_customers AS $s_customer}
                                    <option value="{$s_customer.username}" {if $s_customer.username eq $customer.userid}selected='selected'{/if}>{if $sort_by_name == 1}{$s_customer.first_name|cat:' '|cat:$s_customer.last_name}{elseif $sort_by_name == 2}{$s_customer.last_name|cat:' '|cat:$s_customer.first_name}{/if}</option>
                                {/foreach}
                            </select>
                        </h1>
                        <div class="pull-right" style="margin: 5px;">
                            {if $back_button_url != '' || $back_button_url != null}<a class="btn btn-normal pull-right" href="{$back_button_url}" style="margin-left: 5px;"><span style="margin: 0px 5px 0px 0px;" class="icon-arrow-left"></span>{if $from_page == 'mc_leave'}{$translate.back_mc_leave}{else if $from_page == 'export'}{$translate.back_export}{else if $from_page == 'comengo'}{$translate.back_comengo}{else}{$translate.back_report}{/if}</a>{/if}
                            {if $customer.name != "" || $customer.name != null}<button class="btn btn-default btn-normal pull-right" type="button" onclick="navigatePage('{$url_path}month/gdschema/{$selected_year}/{$selected_month}/{$customer.userid}/{if isset($smarty.get.return_page) and $smarty.get.return_page eq 'emp_work_report'}/{$smarty.get.employee}/{$smarty.get.return_page}/{/if}', 1);" style="margin-left: 5px;">{$translate.monthly_view}</button>{/if}
                            {if $customer.name != "" || $customer.name != null}<button class="btn btn-default btn-normal pull-right" type="button" onclick="navigatePage('{$url_path|cat:$back_url}', 1);" style="margin-left: 5px;">{$translate.weekly_view}</button>{/if}
                            {if $customer.name != "" || $customer.name != null}<button class="btn btn-default btn-normal pull-right" type="button" onclick="navigatePage('{$url_path}gdschema_day_customer.php?date={$cur_date}&customer={$customer.userid}&action=1',1)" style="margin-left: 5px;">{$translate.timeline_view}</button>{/if}
                                            ...
                                            <div class="widget-header span12">
                                                <div class="day-slot-wrpr-header-left pull-left"> 
                                                    <h1 {if (($cur_date|date_format:'%u') eq 7) || ($cur_date|in_array:$holidays)} style="color:red; font-weight: bold;"{/if}> {$translate.assigned_slots} | {$cur_date}
                                                        {if $customer_running_tasks|count gt 0 and $today_date eq $cur_date}
                                                            ...
                                                            {/foreach}
                                                            <div class="cursor_help pull-right ml" title="{$customer_running_task_title}"><img src='{$url_path}images/clock.gif'/></div>
                                                        {/if}
                                                    </h1>
                                                </div>
                                                <div style="margin: 5px 0 0 0;" class="pull-right day-slot-wrpr-header-right">
                                                    ...
                                                    <!-- <button class="btn btn-default btn-normal pull-right mr ml" type="button" onclick="navigatePage('{$url_path}gdschema_day_customer.php?date={$cur_date}&customer={$customer.userid}&action=1',1)" title="{$translate.timeline_view}"><i class="icon-bar-chart cursor_hand"/></button> -->
...
{*                                            slots display*}
                                            <div tabindex="2" class="span12 slots-full-view-body no-min-height" id="slot_display_wraper" data-date='{$cur_date}'>
                                                <div class="row-fluid">
                                                    {assign i 0}
                                                                                    ...
                                                                                    <div class="span7">
                                                                                        ...
                                                                                        <div class="row-fluid">
                                                                                            <div class="span12 customer-names-wrpr">
                                                                                                <span class="span12 day-customer-name no-min-height">{if $slot_det.employee neq ''}{$slot_det.emp_name}{else}<label class="unmanned_slot">{$translate.gdschema_unmanned_slot}</label>{/if} <i style="display: none;" class="glyphicons icon-remove pull-right delete-customer"></i></span>
                                                                                            </div>
                                                                                        </div>
                                                                                    </div>
                                                                                    <div class="span5">
                                                                                        <div class="row-fluid">
                                                                                            <ul class="day-slot-info-icons pull-right">
                                                                                                ...
                                                                                                {*if ($slot_det.status != 2 && $slot_det.status != 4) && $slot_det.signed_in == 0}
                                                                                                    <li>
                                                                                                        <div class="day-slote-lock" type="button">{if $slot_det.status != 3}<span class="icon-lock"></span>{/if}</div>
                                                                                                    </li>
                                                                                                {/if*}
                                                                                                <li></li>
                                                                                                {if $slot_det.status neq 2 and $slot_det.tl_flag == 1 and $slot_det.signed_in eq 0 and $slot_det.status neq 4}
                                                                                                    <li style=""><input class="check-box get_slot_check m_check" type="checkbox" value="{$slot_det.id}" id="slot_{$i}" data-slot-no="{$i}" data-slot-time-range="{$slot_det.slot}" {if ($slot_det.privileges_gd.add_employee eq 1 or $slot_det.privileges_gd.copy_day_slot eq 1 or $slot_det.privileges_gd.delete_day_slot eq 1) and $slot_det.employee eq ''}onclick="employee_assign_slot('{$i}','{$slot_det.slot}','{$slot_det.id}');"{/if}></li>
                                                                                                {/if}
                                                                                            </ul>
                                                    ...
                                                    <div class="box-generic">
                                                        <div class="tabsbar tabsbar-2" style="margin: -5px 0 7px;">
                                                            <ul class="row-fluid row-merge">
                                                                <li class="span4{if !isset($right_panel) || $right_panel eq '' || $right_panel eq 'emp_assign_list'} active{/if}"><a href="#tab-assign-emp-list" data-toggle="tab" id="tb_assign_emp_list"> {$translate.employee}</a></li>
                                                            ...
                                                            <div class="tab-pane{if !isset($right_panel) || $right_panel eq '' || $right_panel eq 'emp_assign_list'} active{/if}" id="tab-assign-emp-list" style="background:none !important;">
                                                                <div tabindex="1" style="overflow: auto;" class="span12 day-deatiles-employee input-group" >
                                                                    <div class="row-fluid customer-names-pills-wrpr" id="assignment-employees-wrpr">
                                                                        {foreach from=$employees_to_assign item=emp}
                                                                            <div class="customer-name-slots" data-emp-name='{$emp.username}' style="float: none;">
                                                                                <span class="widget-stats-emplyee widget-stats-2 employees" onclick="loadPopupSchemaEmployeeAssignment('{$url_val_slot}&empl={$emp.username}&action=multiple_slot_assign','{$emp.last_name} {$emp.first_name}','{$emp.dont_popup_flag}');">
                                                                                    <span>{if $sort_by_name == 1}{$emp.first_name|cat:' '|cat:$emp.last_name}{elseif $sort_by_name == 2}{$emp.last_name|cat:' '|cat:$emp.first_name}{/if} ({$emp.code}){if $emp.substitute eq 1} [{$translate.substitute}]{/if}</span>
                                                                                </span>
                                                                            </div>
                                                                        {/foreach}
                                                                    </div>
                                                                            ...
                                                                            <div class="widget-body" style="padding:5px;">
                                                                                <div class="row-fluid">
                                                                                    <div class="span8 customer-name no-mb no-min-height" style="margin:5px;"> 
                                                                                        <span style="margin-right: 5px;" class="icon-user"></span>{$customer.name}
                                                                                    </div>
                                                                                    ...
                                                                                    <div class="span12" style="margin-left: 0px;">
                                                                                        <div class="input-prepend date hasDatepicker datepicker" id="dtPickerNewSlotDate">
                                                                                            ...
                                                                                            <input class="form-control span12 slot_date" id="new_slot_date" value="{$cur_date}" placeholder="{$translate.date}" onblur="load_avail_emps_within_period_for_new_slot(this);" type="text"/>
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
...
{*                                                                        copy to weeks options*}
                                                                        ...
                                                                        {*<div class="slot-wrpr-buttons span12 no-mt" style="margin:5px 0;">
                                                                            <button type="button" class="btn btn-success span12" onclick="manEntry();"><span class="icon-save"></span> {$translate.save}</button>
                                                                        </div>*}
                                                                    </div>
                                    ...
                                    {*                    <input id="this_tl_flag" type="hidden" value=""/>*}
                                                        ...
                                                        <h2><span class="icon-user" title="{$translate.customer}"></span> 
                                                            <span id="sdCustomer">{$customer.name}</span>
                                                            <input class="this_slot_customer_id" id="sdCustomerID" type="hidden" value="{$customer.userid}"/>
                                                        </h2>
                                                        <div class="span12" style="margin-left: 0px;">
                                                            <div class="input-prepend span11">
                                                        ...
                                                        <div class="span12 slots-full-view-body" style="overflow-y: auto; padding-right: 5px !important;">
                                                            <div id="goto-customers-list" class="row-fluid span12" style="padding-bottom: 8px !important; padding-right: 4px; margin-left: 0;">
                                                                {foreach $search_customers AS $custl}
                                                                    <div style="margin-left: 0px;" class="span12">
                                                                        <div style="margin-top: 4px;" class="span12 child-slots">
                                                                            <label onclick="navigatePage('{$url_path}gdschema_alloc_window.php?date={$cur_date}&customer={$custl.username}',1);">
        ...
        $(document).ready(function() {
            ...
            $("#cmb_customer").change(function(){
                ...
                if(selected_customer != ''){
                    navigatePage('{$url_path}gdschema_alloc_window.php?date={$cur_date}&customer='+selected_customer+'&emp_alloc={$emp_alloc}', 1);
                }
            });
        });
...
{*            var slot_date = $('#slot_details_main_wraper_group #slot_details_date').val();*}
        ...
        function loadPopupSchemaEmployeeAssignment(url,emp, dont_popup_flag) {
            ...
            if(slots_array.length == 0){
                ...
                dialog_box_new.load('{$url_path}ajax_employee_slots_view.php?'+url);
                ...
                /*if (dont_popup_flag == '1')
                    ...
                    var full_url = '{$url_path|cat:'gdschema_process_schemaAssign.php?'}'+url+'&ids='+ selected_ids;
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $("#slot_display_wraper").droppable({
                    ...
                    drop: function( event, ui ) {
                        ...
                        var process_details_obj = { 'date': selected_date,
                                    'employee'  : '{if $login_user_role eq 3}{$login_user}{/if}',
                                    'customer'  : '{$customer.userid}',
                                    'emp_alloc' : '{$login_user}',
                                    'time_from' : time_from,
                                    'time_to'   : time_to,
            ...
            {if $privileges_gd.add_employee eq 1}
                ...
                $("#slot_display_wraper .slot-day:not(.signed_slot,.slot-theme-leave,.slot-theme-candg)").droppable({
                        ...
                        drop: function( event, ui ) {
                            ...
                            var url_data = '{$url_val_slot}&empl='+dragged_employee+'&action=multiple_slot_assign&ids='+selected_slot;
                            var full_url = '{$url_path|cat:'ajax_alloc_action.php?'}'+url_data;
                                ...
                                slot_theme += '<div class="span12" style="margin-left: 0px;">\n\
                                    <div class="input-prepend span11">\n\
                                        ...
                                        <select class="form-control custom_slot_fkkn span12" name="custom_slot_fkkn">\n\
                                            <option {if $customer.fkkn eq 1}selected="selected"{/if} value="1">{$translate.fk}</option>\n\
                                            <option {if $customer.fkkn neq 1}selected="selected"{/if} value="2">{$translate.kn}</option>\n\
                                            <option value="3">{$translate.tu}</option>\n\
                                        </select>\n\
                                    </div>\n\
                                </div>';
        ...
        function load_avail_emps_within_period_for_new_slot(this_obj){
            ...
            if($.trim($('.add-new-slots-month .slot_date').val()) != '' && $.trim($(this_obj).parents('.time_slots_theme').find('.slot_from').val()) != '' && $.trim($(this_obj).parents('.time_slots_theme').find('.slot_to').val()) != ''){
                ...
                //get all other slot details
                var main_obj = { 'selected_date': slot_date,
                                'selected_customer': '{$customer.userid}',
                                'action': 'multiple_add',
                                'current_slot': { 'time_from': slot_from, 'time_to': slot_to },
                                'other_time_slots': [ ] }; 
            ...
            if(slot_date == ''){
                ...
                $( '.add-new-slots-month .create-slotes-panel .time_slots_theme' ).each(function( index ) {
                    ...
                    if(tmp_slot_from !== false && tmp_slot_to !== false){
                        ...
                        var temp_obj = { 'time_from': tmp_slot_from, 'time_to': tmp_slot_to, 'customer': '{$customer.userid}'};
        ...
        function manEntry_proceed(data_obj){
            ...
            var url_atl = 'date='+slot_date+'&employee=&customer={$customer.userid}&emp_alloc={$login_user}&action=man_slot_entry&sub_action=multiple_add&type_check=18';
            if(weekly_past_value){
    ...
    <script>
                            ...
                            switch(key){
                                case "go_to_employee":
                                    ...
                                    if(ids_temp.length == 1){
                                        ...
                                        if(slot_details_hub.length == 1){
                                            ...
                                            if(temp_empl_id != ''){
                                                navigatePage('{$url_path}gdschema_alloc_window_employee.php?date={$cur_date}&employee='+temp_empl_id,1);
                                            }
                                        }
                                    }
                                    if(temp_empl_id == ''){
                                ...
                                case "delete_slot":
                                   if(ids != ''){
                                        ...
                                        var url_post_data = { 'ids': ids, 'action' : 'multiple_slots_remove', 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'request_from': 'gd_alloc_window' };
                                        bootbox.dialog( '{$translate.confirm_delete_slot}', [{
                                                "label" : "{$translate.no}",
                                                "class" : "btn-danger"
                                            }, {
                               ...
                               case 'delete_customer':
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': 'delete_customers', 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'request_from': 'gd_alloc_window' };
                               ...
                               case 'delete_employee':
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': 'delete_employees', 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'request_from': 'gd_alloc_window' };
                               ...
                               case 'change_tu':
                                   ...
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': slot_fkkn_change, 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'request_from': 'gd_alloc_window' };
                                            ...
                                            if((slot_type_change == 12 || slot_type_change == 13) && mixed_normal_oncall_types == true){
                                                ...
                                                if(slot_type_change == 12) //complementary
                                                    ...
                                                    bootbox.dialog( prompt_msg, [{
                                                                    ...
                                                                    $.ajax({
                                                                        ...
                                                                        success:function(data){
                                                                            if(data == 'success'){
                                                                                ...
                                                                                var url_post_data = { 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'ids': ids, 'action': 'change_type', 'slot_type': slot_type_change, 'request_from': 'gd_alloc_window' };
                                                            ...
                                                            "callback": function() {
                                                                var url = '{$url_path}ajax_right_click_actions.php';
                                                                var url_post_data = { 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'ids': ids, 'action': 'change_type', 'slot_type': slot_type_change, 'request_from': 'gd_alloc_window', 'normal_oncall_auto_change': true };
                                                                navigatePage(url,1, url_post_data); 
                                                            }
                                                    }]);
                                            }
                        ...
                        {if $privileges_gd.candg_approve eq 1}
                            options.items.candg_approve = { 
                                    ...
                                    callback: function(key, opt){ 
                                            var urls = '{$url_path}ajax_right_click_actions.php';
                                            var url_post_data = { 'ids': ids, 'action': 'slot_approve_candg', 'page_date': '{$cur_date}', 'customer': '{$customer.userid}', 'request_from': 'gd_alloc_window' };
        ...
        {if $privileges_gd.add_employee eq 1 or $privileges_gd.add_customer eq 1}
            ...
            function saveChangeUserMultipleConfirm(){
                ...
                var url = "customer={$customer.userid}&page_date={$cur_date}&method=1&request_from=gd_alloc_window&ids="+selected_slots;
        ...
        function copyMonthlySlot(){
            ...
            if(ids != ''){
                var url = '{$url_path}ajax_slot_process.php?page_date={$cur_date}&customer={$customer.userid}&action=copy_select&slots='+ids;
                var url_data = 'page_date={$cur_date}&customer={$customer.userid}&action=copy_select&slots='+ids;
                wrapLoader("#main_content #external_wrapper");
                $.ajax({
        ...
        function pasteSlot(action_type, date, week_year){
            ...
            if(date == ''){
                if(week_year != ''){
                    var url = '{$url_path}ajax_slot_process.php?date='+week_year+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select';
                    var url_data = 'date='+week_year+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&type_check=8'
                    var atl_req_data = 'date='+week_year+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&type_check=8';
                }else{
                    var url = '{$url_path}ajax_slot_process.php?date={$cur_date}&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&to_single_day=TRUE'; 
                    var url_data = 'date={$cur_date}&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&type_check=8&to_single_day=TRUE';
                    var atl_req_data = 'date={$cur_date}&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&type_check=8&to_single_day=TRUE';
                }
            }
            else{
                var url = '{$url_path}ajax_slot_process.php?date='+date+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&to_single_day='+action_type; 
                var url_data = 'date='+date+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&type_check=8&to_single_day='+action_type;
                var atl_req_data = 'date='+date+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select&type_check=8&to_single_day='+action_type;
            }
            check_atl_warning(atl_req_data, function(this_url){ 
                                wrapLoader("#external_wrapper");
        ...
        function pasteSlotDay(action_type, date, week_year){
            ...
            if(date == ''){
                if(week_year != ''){
                    var url = '{$url_path}ajax_slot_process.php?date='+week_year+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select_day';
                    var url_data = 'date='+week_year+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select_day&type_check=8'
                    var atl_req_data = 'date='+week_year+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select_day&type_check=8';
                }else
                    return false;
            }
            else{
                var url = '{$url_path}ajax_slot_process.php?date='+date+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select_day&to_single_day='+action_type; 
                var url_data = 'date='+date+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select_day&type_check=8&to_single_day='+action_type;
                var atl_req_data = 'date='+date+'&customer={$customer.userid}&emp_alloc={$login_user}&action=paste_select_day&type_check=8&to_single_day='+action_type;
            }
            check_atl_warning(atl_req_data, function(this_url){ 
                                wrapLoader("#external_wrapper");
                                $('#div_alloc_action').load(this_url,function(response, status, xhr){ uwrapLoader("#external_wrapper"); reload_content(); });
           ...
           $('#tab-assign-emp-list #assignment-employees-wrpr .customer-name-slots').mousedown(function(e){ 
                    if( e.button == 2 ) { 
                        ...
                        if(slots_array.length == 0){
                            ...
                            navigatePage('{$url_path}gdschema_alloc_window_employee.php?date={$cur_date}&employee='+employee_id,1);
                        }  
                        return false; 
                    }
                    return false;  
