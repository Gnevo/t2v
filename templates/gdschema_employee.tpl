...
{if $flag_emp_access eq 1}
    <div class="row-fluid{if isset($show_right_panel) and $show_right_panel} show_main_right{/if}" id="gdweek_wraper">
            ...
            <div class="accordion span12 no-mb" id="accordion2" style="margin-left: 0px;">
                <div class="accordion-group">
                    <div class="accordion-heading">
                        <a class="accordion-toggle accordion-toggle-employee {if !(isset($smarty.cookies.opened_weekly_top_widget) and $smarty.cookies.opened_weekly_top_widget eq true)}collapsed{/if}" data-toggle="collapse" data-parent="#accordion2" href="#collapseOne">
                ...
                <div class="widget-body" style="padding:5px;">
                    <div class="row-fluid">
                        ...
                        <div class="pull-left customer-name" style="margin:5px 5px 0px 5px;"> 
                            ...
                            {* {$translate.employee}: *}
                            {*if $sort_by_name == 1}{$employee_data.first_name|cat:' '|cat:$employee_data.last_name}
                            {elseif $sort_by_name == 2} {$employee_data.last_name|cat:' '|cat:$employee_data.first_name}{/if}
                            {if $employee_data.code neq ''} ({$employee_data.code}){/if*}
                            {if $contract_exist_flag}<span class="pull-right text-warning show_contract_btn" style="font-size: 17px;margin-left: 8px;"><i class="icon icon-exclamation-sign cursor_hand"></i></span>{/if}
                            ...
                            <select class="pull-right ml" id="cmb_employee" name="cmb_employee" style="height: 28px;margin-top: -3px;width: auto !important;">
                                {foreach $search_employees as $s_employee}
                                    <option value="{$s_employee.username}" {if $s_employee.username eq $employee_data.username}selected='selected'{/if}>{if $sort_by_name == 1}{$s_employee.first_name|cat:' '|cat:$s_employee.last_name}{elseif $sort_by_name == 2}{$s_employee.last_name|cat:' '|cat:$s_employee.first_name}{/if}</option>
                                {/foreach}
                            </select>
                        </div>
                        <div class="pull-right no-print">
                            {if $privileges_gd.add_slot == 1}<span class="btn  btn-default pull-right ml" id="add-slots" {*onclick="popupAddSlot();"*} title="{$translate.tooltip_new_slot}"><i class="icon-plus"></i></span>{/if}
                            <button type="button" class="btn btn-default btn-normal pull-right" title="{$translate.monthly_view}" onclick="navigatePage('{$url_path}month/gdschema/employee/{$first_date_year}/{$first_date_month}/{$employee}/', 1);"><i class="icon-calendar"></i> {$translate.monthly_view}</button>
                            {*if $process_previlege}<button type="button" class="btn btn-default btn-normal  btn-manage pull-right ml" title="{$translate.show_actions}" ><i class="icon-ok"></i> {$translate.big_process}</button>{/if*}
                            ...
                            {if $rpt_page_url neq ''}<a class="btn btn-default  pull-right no-print" href="{$rpt_page_url}"><span class="icon-arrow-left"></span>{if $from_page == 'emp_work_report'}{$translate.back_report}{else}{$translate.back_mc_leave}{/if}</a>{/if}
                        </div>
            ...
            {if $contract_exist_flag}
                <div class="row-fluid customer-info no-print">
                    ...
                    <div class="span12 no-ml">
                        {if $work_hours.weekly_nomal neq '0' or $contract_hours.weekly_nomal neq '0' or $work_hours.weekly_oncall neq '0' or $contract_hours.weekly_oncall neq '0' or (isset($work_hours.monthly_nomal) and isset($work_hours.monthly_oncall))}
                            <table class="footable table table-striped table-bordered table-white table-primary" style="margin-bottom: 1px;">
                                <thead>
                                    <tr>
                                        <th data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.weekly_normal_hour}</th> 
                                        <th data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.weekly_oncall_hour}(J)</th> 
                                        {if isset($work_hours.monthly_nomal) and isset($work_hours.monthly_oncall)}
                                            <th data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.monthly_normal_hour}</th> 
                                            <th data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.monthly_oncall_hour}(J)</th> 
                                        {/if}
                                    </tr>
                                    <tr>
                                        <td data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$work_hours.weekly_nomal}h ({$contract_hours.weekly_nomal}h)</td> 
                                        <td data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$work_hours.weekly_oncall}h ({$contract_hours.weekly_oncall}h)</td> 
                                        {if isset($work_hours.monthly_nomal) and isset($work_hours.monthly_oncall)}
                                            <td data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$work_hours.monthly_nomal}h ({$contract_hours.monthly_nomal}h)</td> 
                                            <td data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$work_hours.monthly_oncall}h ({$contract_hours.monthly_oncall}h)</td> 
                                        {/if}
                                    </tr>
                                </thead>
                            </table>
                        {/if}
                        {if isset($work_hours.periods) and $work_hours.periods|count gt 0}
                            <table class="footable table table-striped table-bordered table-white table-primary" style="margin-bottom: 0px;">
                                <thead>
                                    <tr>
                                        <th data-class="expand" class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.contract_period}</th> 
                                        <th data-hide="phone,tablet"  class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.contract_period_total_normal_hour}</th>
                                        <th data-hide="phone,tablet"  class="table-col-center" style=" background:#fff; color:#7C7C7C;">{$translate.contract_period_total_oncall_hour}(J)</th> 
                                    </tr>
                                </thead>
                                <tbody>
                                    {foreach $work_hours.periods as $period}
                                        <tr>
                                            ...
                                            <td class="table-col-center">{$period.work_nomal}h ({$period.contract_nomal}h)</td>
                                            <td class="table-col-center">{$period.work_oncall}h ({$period.contract_oncall}h)</td>
                                        </tr>
                                    {/foreach}
                                </tbody>
                            </table>
                            ...
                            <th>   
                                <div id="tabs2" class="scroll_tabs_theme_light scroll_tabs_theme_light_employee">
                                    ...
                                    {foreach $week_numbers as $week_number}
                                        <span class="week_no_spn{if $week_number.selected} active-week{/if}" onclick="navigatePage('{$url_path}employee/gdschema/{$week_number.id}/{$employee}/8/', 1);" title="{$translate.go_to_slot_page_week} {$week_number.value}" data-yearweek="{$week_number.id}">{$week_number.value}</span>
                                    {/foreach} 
                                </div>
                            </th>
                        </tr>
                    ...
                    <table class="footable table table-striped table-bordered table-white table-primary  week-slot-calender" style="margin:0 " id="week-slot-calender">
                        <thead>
                            ...
                            {foreach $employee_week as $employee_day}
                                {$i=$i+1}    
                                <td class="no-padding weekly_day"  data-date="{$employee_day.date}">
                                    <div class="day-select day-select-employee no-mb" {*$employee_day.signed == 0*}>
                                        <span class="hide-small-devices" {if ($privileges_gd.delete_slot == 1 || $privileges_gd.add_customer == 1 || $privileges_gd.add_employee == 1 || $privileges_gd.fkkn == 1 || $privileges_gd.slot_type == 1 || $privileges_gd.remove_customer == 1 || $privileges_gd.remove_employee == 1 || $privileges_gd.add_slot == 1)}onclick="navigatePage('{$url_path}gdschema_alloc_window_employee.php?date={$employee_day.date}&employee={$employee}', 1);" title="{$translate.enter_into_day_slots}" {if (($employee_day.date|date_format:'%u') eq 7) || ($employee_day.date|in_array:$holidays)} style="color:red; font-weight: bold;"{else if ($employee_day.date eq $today_date)} style="font-weight: bold;"{/if} {else} {if (($employee_day.date|date_format:'%u') eq 7) || ($employee_day.date|in_array:$holidays)} style="cursor: default; color:red; font-weight: bold;"{else if ($employee_day.date eq $today_date)} style=" cursor: default; font-weight: bold;"{/if}  onclick="messagePrivilege()"{/if}>  {$translate.{$employee_day.day.day}}<br/>{$employee_day.date}</span>
                                        <div class="visible-only-small-devices" {if ($privileges_gd.delete_slot == 1 || $privileges_gd.add_customer == 1 || $privileges_gd.add_employee == 1 || $privileges_gd.fkkn == 1 || $privileges_gd.slot_type == 1 || $privileges_gd.remove_customer == 1 || $privileges_gd.remove_employee == 1 || $privileges_gd.add_slot == 1)}onclick="navigatePage('{$url_path}gdschema_alloc_window_employee.php?date={$employee_day.date}&employee={$employee}', 1);" title="{$translate.enter_into_day_slots}" {else}style="cursor: default;" onclick="messagePrivilege()"{/if}>
                                            <input style="width:100%; font-size: 7px !important; padding: 0; text-align: center; margin: 0; background: none !important; border: 0; color: #000 !important;" disabled="" type="text" value="{$translate.{$employee_day.day.day}}">
                                            <input style="width:96%; margin: 0 auto; background: none !important; border: 0; text-indent: -36px;font-size: 10px !important; color: #000 !important;"  disabled="" type="text" value="{$employee_day.date}">
                                        </div>
                                        
                                        {if $employee_running_tasks|count gt 0 and $today_date eq $employee_day.date}
                                            ...
                                            {/foreach}
                                            <div class="cursor_help pull-right" title="{$employee_running_task_title}"><img src='{$url_path}images/clock.gif'/></div>
                                        {/if}
                                        <div onclick="navigatePage('{$url_path}gdschema_day_employee.php?date={$employee_day.date}&employee={$employee}&action=1',1)" class="cursor_hand pull-left" title="{$translate.timeline_view}"><i class="icon-bar-chart cursor_hand"/></div>
                                    </div>
                                </td>    
                            {/foreach}
                            </tr>
                        ...
                        <tbody>
                            <tr class="gradeX expandable" id="table-row-1">
                                ...
                                {foreach $employee_week as $employee_day}
                                    {$i=$i+1}    
                                    <td id="td{$i}" class="weekly_day" style="height: 500px !important;" data-date="{$employee_day.date}">
{*                                        <div class="day-select" {if $employee_day.signed == 0 && ($privileges_gd.delete_slot == 1 || $privileges_gd.add_customer == 1 || $privileges_gd.add_employee == 1 || $privileges_gd.fkkn == 1 || $privileges_gd.slot_type == 1 || $privileges_gd.remove_customer == 1 || $privileges_gd.remove_employee == 1 || $privileges_gd.add_slot == 1)}onclick="navigatePage('{$url_path}gdschema_alloc_window_employee.php?date={$employee_day.date}&employee={$employee}', 1);" title="{$translate.enter_into_day_slots}" {else}style="cursor: default;" onclick="messagePrivilege()"{/if}>{$translate.{$employee_day.day.day}}<br/>{$employee_day.date}</div>*}
                                        {foreach $employee_day.slots as $day_slot}
                                            {if $login_user_role neq 3 or ($login_user_role eq 3 and ($privileges_gd.not_show_employees eq 0 || ($privileges_gd.not_show_employees eq 1 and $day_slot.employee eq $login_user)))}
                                                <span class="collapse-slot-2 clearfix" id="slot_thread_{$day_slot.id}">
                                                    <div 
                                                        class="slot-week slot_bountery {if $day_slot.status eq 2}slot-theme-leave{elseif $day_slot.status eq 0 || $day_slot.status eq 3}slot-theme-incomplete{elseif $day_slot.status eq 1 and $day_slot.created_status eq 1}slot-theme-candg-accept{elseif $day_slot.status eq 4}slot-theme-candg{elseif $day_slot.type eq 10}slot-theme-pm{else}slot-theme-complete{/if} span12 {if $day_slot.signed eq 1}signed_slot{/if}{if $swap_copied_slot eq $day_slot.id} objblink{/if} {if $login_user_role neq 3 or ($login_user_role eq 3 and $day_slot.employee eq $login_user)}slot-week-draggable{/if} {if $day_slot.show_details neq 1}donot_show_slot{/if}" 
                                                        onmouseover="tooltip.pop(this, '#slot_details_{$day_slot.id}', { position:0, offsetX:-20, effect:'slade'{*, overlay:true*} });"
                                                        ...
                                                        <div class="inner-panel span12{if $day_slot.signed eq 1} striped{/if}">
                                                          ...
                                                          <div class="week-slottype visibility-hidden">
                                                            ...
                                                            <div class="span12 week-slottype-employe" style="margin:0;">
                                                                <div class="week-slot-name" style="position: absolute;margin: -2px 0 0 12px;">{$day_slot.cust_name}</div>
                                                            ...
                                                            <div class="slot-week-checkbox-wrpr" >
                                                                ...
                                                                {if $day_slot.status == 2}
                                                                    <div class="week-slot-badge-wrpr hide-small-devices"><span class="label week-slot-badge">{$day_slot.leave_data.leave_name}</span></div>
                                                                {/if}    
                                                                {if $day_slot.tl_flag == 1 && $day_slot.signed == 0 && $day_slot.status neq 2 and ($privileges_gd.process eq 1 or $privileges_gd.add_employee eq 1 or $privileges_gd.add_customer eq 1 or $privileges_gd.fkkn eq 1 or $privileges_gd.slot_type eq 1 or $privileges_gd.delete_slot eq 1 or $privileges_gd.remove_employee eq 1 or $privileges_gd.remove_customer eq 1)}
                                                                    <input type="checkbox" class="check-box pull-right m_check hide-small-devices" value="{$day_slot.id}" />
                                                                {/if}
                                                            </div>
                                                        </div>
                                                    </div>
...
{*    slot_expanded_views*}
        <div id="slot_expanded_views" >
            {foreach $employee_week as $employee_day}
                {foreach $employee_day.slots as $slot}
...
{*            main-right*}
        <div class="span4 main-right slot-form{if !isset($show_right_panel) || !$show_right_panel} hide{/if}" style="margin-top: 8px; padding: 10px;" id="stickyPanelParent">
            
{*            add slot/memory slot*}
            {if $privileges_gd.add_slot == 1}
                <div id="slot_creation_main_wraper_group" class="clearfix {if !(isset($show_right_panel) and $show_right_panel and $right_panel eq 'memory_slots')} hide{/if}">
    {*                add new slot*}
                    <div class="add-new-slots-month hide">
                        <div id="btnGroupStickyPanel" class="span12">
                            <div class="row-fluid" style="margin-bottom: 5px;">
                                <div class="span12">
                                    <button type="button" class="btn btn-default-special span12 btn-large btn-default-special-employee" id="show-memory-slots-btn"><span class="icon-level-down"></span> {$translate.memory_slots}</button>
                                </div>
                            </div>
                            ...
                            <div class="widget-body" style="padding:5px;">
                                <div class="row-fluid">
                                    <div class="span8 customer-name no-min-height" style="margin:5px;"> 
                                        <span style="margin-right: 5px;" class="icon-group"></span>{$employee_data.first_name|cat: ' '|cat:$employee_data.last_name}
                                    </div>
...
{*            slot manage options*}
            <div id="slot_details_main_wraper_group" class="hide">
                <div class="slot-wrpr span12" id="slot-wrpr-slots">
                    ...
                    <h2 class="span12 no-ml no-mb"><span class="icon-group" title="{$translate.employee}"></span> 
                        <span id="sdEmployee">{$employee_data.first_name|cat: ' '|cat:$employee_data.last_name}</span>
                        <input class="this_slot_employee_id" id="sdEmployeeID" type="hidden" value="{$employee}"/>
                    </h2>
                    <div class="span12" style="margin-left: 0px;">
                        <div class="input-prepend span11">
                    ...
                    <div class="span12 form-wrpr hide" id="Franvaro-box" style="margin: 0 0 15px 0;">
                        ...
                        <div class="btn-group leave-type">
                            {foreach from=$leave_types key=leave_type_key item=leave_type}
                                <a unselectable="on" href="javascript:void(0);" id="leave_type{$leave_type_key}" class="btn btn-default" name="leave_type" value="{$leave_type_key}" onclick="setLeaveTypeSingleSlot({$leave_type_key});">{$leave_type}</a>
                            {/foreach}
                        </div>
                    ...
                    <div class="span12 slots-full-view-body" style="overflow-y: auto; padding-right: 5px !important;">
                        <div id="goto-customers-list" class="row-fluid span12" style="padding-bottom: 8px !important; padding-right: 4px; margin-left: 0;">
                            {foreach $righclick_customers_for_goto AS $rcust}
                                <div style="margin-left: 0px;" class="span12">
                                    <div style="margin-top: 4px;" class="span12 child-slots">
                                        <label onclick="navigatePage('{$url_path}customer/gdschema/{$year_week}/{$rcust.username}/8/',1);">
        ...
        $(document).ready(function() {
            ...
            $("#cmb_employee").change(function(){
                ...
                if(selected_employee != ''){
                    navigatePage('{$url_path}employee/gdschema/{$year_week}/'+selected_employee+'/8/', 1);
                }
            });
        });
...
{*    single slot manage options*}
    <script type="text/javascript">
        ...
        function reload_content(data_values){
            ...
            navigatePage('{$url_path}employee/gdschema/{$year_week}/{$employee}/8/', 1, passing_data_object);
        }
        ...
        {if $privileges_gd.add_slot eq 1}
            function clone_relation_leave_slot(){
                bootbox.dialog( '{$translate.confirm_clone_leave_relation}', [{
                        ...
                        "callback": function() {
                            var slot_id = $('#slot_details_main_wraper_group #sdID').val();
                            var url = '{$url_path}ajax_alloc_action.php?employee={$employee}&week_num={$year_week}&req_from=gd_employee&slotid='+slot_id+'&action=clone_leaveslot';
                            navigatePage(url, 1);
                        }
                }]);
             }
...
{*    add new slot functions*}
    <script>
                        ...
                        drop: function( event, ui ) {
                            ...
                            //slot dragging
                                ...
                                if(this_slot_date != target_date){
                                    ...
                                    var data_post = 'slot_id='+source_data_slot+'&to_date='+target_date+'&action=drop&week_num={$year_week}&pEmployee={$employee}&request_from=gd_employee&return_type=json';
                                    var base_url = '{$url_path}ajax_alloc_action_month.php?'+data_post;
            ...
            if(slot_date == ''){
                ...
                $( '.add-new-slots-month .create-slotes-panel .time_slots_theme' ).each(function( index ) {
                    ...
                    if(tmp_slot_from !== false && tmp_slot_to !== false){
                        ...
                        var temp_obj = { 'time_from': tmp_slot_from, 'time_to': tmp_slot_to, 'employee': '{$employee}', 'customer': tmp_slot_customer};
        ...
        function manEntry_proceed(data_obj){
            ...
            var url_atl = 'date='+slot_date+'&employee={$employee}&customer=&emp_alloc={$login_user}&action=man_slot_entry&sub_action=multiple_add&type_check=18';
            if(weekly_past_value){
   ...
   <script>
                            ...
                            switch(key){
                                ...
                                case "go_to_customer":
                                    ...
                                    if(ids_temp.length == 1){
                                        ...
                                        if(slot_details_hub.length == 1){
                                            ...
                                            if(temp_cust_id != ''){
                                                navigatePage('{$url_path}customer/gdschema/{$year_week}/'+temp_cust_id+'/8/',1);
                                            }
                                        }
                                    }
                                    if(temp_cust_id == ''){
                                ...
                                case "delete_slot":
                                   if(ids != ''){
                                        ...
                                        var url_post_data = { 'ids': ids, 'action' : 'multiple_slots_remove', 'week_num': '{$year_week}', 'employee': '{$employee}', 'request_from': 'gd_employee' };
                                        bootbox.dialog( '{$translate.confirm_delete_slot}', [{
                                                "label" : "{$translate.no}",
                                                "class" : "btn-danger"
                                            }, {
                               ...
                               case 'delete_customer':
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': 'delete_customers', 'week_num': '{$year_week}', 'employee': '{$employee}', 'request_from': 'gd_employee' };
                               ...
                               case 'delete_employee':
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': 'delete_employees', 'week_num': '{$year_week}', 'employee': '{$employee}', 'request_from': 'gd_employee' };
                               ...
                               case 'change_tu':
                                   ...
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': slot_fkkn_change, 'week_num': '{$year_week}', 'employee': '{$employee}', 'request_from': 'gd_employee'};
                                            ...
                                            if((slot_type_change == 12 || slot_type_change == 13) && mixed_normal_oncall_types == true){
                                                ...
                                                if(slot_type_change == 12) //complementary
                                                    ...
                                                    bootbox.dialog( prompt_msg, [{
                                                                    ...
                                                                    $.ajax({
                                                                        ...
                                                                        success:function(data){
                                                                            if(data == 'success'){
                                                                                ...
                                                                                var url_post_data = { 'week_num': '{$year_week}', 'employee': '{$employee}', 'ids': ids, 'action': 'change_type', 'slot_type': slot_type_change, 'request_from': 'gd_employee' };
                                                            ...
                                                            "callback": function() {
                                                                var url = '{$url_path}ajax_right_click_actions.php';
                                                                var url_post_data = { 'week_num': '{$year_week}', 'employee': '{$employee}', 'ids': ids, 'action': 'change_type', 'slot_type': slot_type_change, 'request_from': 'gd_employee', 'normal_oncall_auto_change': true };
                                                                navigatePage(url,1, url_post_data);
                                                            }
                                                    }]);
                                            }
                        ...
                        {if $privileges_gd.candg_approve eq 1}
                            options.items.candg_approve = { 
                                    ...
                                    callback: function(key, opt){ 
                                            var urls = '{$url_path}ajax_right_click_actions.php';
                                            var url_post_data = { 'ids': ids, 'action': 'slot_approve_candg', 'week_num': '{$year_week}', 'employee': '{$employee}', 'request_from': 'gd_employee' };
        ...
        {if $privileges_gd.add_employee eq 1 or $privileges_gd.add_customer eq 1}
            ...
            function saveChangeUserMultipleConfirm(){
                ...
                var url = "employee={$employee}&week_num={$year_week}&request_from=gd_employee&method=1&ids="+selected_slots;
        ...
        $(document).ready(function() {
            ...
            $('#accordion2').on('show', function (e) {
                if(!$('#check1').hasClass('loaded')){
                    ...
                    wrapLoader('#check1');
                    $('#check1').load('{$url_path}ajax_customer_employee_work_summary.php?year_week={$year_week}&employee={$employee}', function(response, status, xhr) { $('#check1').css('min-height', ''); uwrapLoader('#check1'); $('#check1').addClass('loaded'); });
                }
