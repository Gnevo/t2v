                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h4  class="panel-title panel-title-employee clearfix">
                               <div class="pull-left" style="padding:5px;">{$translate.monthly_view} - {$translate.employee}</div>
                                <div class="pull-right no-print">
                                    <ul class="pull-right">
                                        ...
                                        {if $privileges_general.create_template eq 1 or $privileges_general.use_template eq 1}
                                            <li class="collapsed cursor_hand" id="li_tmplt_btn" data-toggle="collapse" data-parent="#accordion" href="#manage-template" aria-expanded="false" aria-controls="collapseTwo"><span id="spn_tmplt_btn" class="icon-plus cursor_hand"></span><a href="javascript:void(0);"><span>{$translate.manage_template}</span></a></li>
                                        {/if}
                                        {if $rpt_page_url neq ''}<li><span class="icon-arrow-left"></span><a href="{$rpt_page_url}"><span>{if $from_page == 'CUST_ADD'}{$translate.back_customer}{else}{$translate.back_mc_leave}{/if}</span></a></li>{/if}
                                    </ul>
                                </div>
                            </h4>
                        </div>
                                ...
                                <div class="pull-left" >
                                    <select class="span12" id="cmb_employee" name="cmb_employee" style="height: 28px;">
                                        ...
                                        {foreach $search_employees as $s_employee}
                                            <option value="{$s_employee.username}" {if $s_employee.username eq $selected_employee}selected='selected'{/if}>{if $sort_by_name == 1}{$s_employee.first_name|cat:' '|cat:$s_employee.last_name}{elseif $sort_by_name == 2}{$s_employee.last_name|cat:' '|cat:$s_employee.first_name}{/if}</option>
                                        {/foreach}
                                    </select>
                                </div>
                                ...
                                <div class="pull-right" id="count-info-switch">
                                    <a href="javascript:void(0)" class="widget-stats widget-stats-1 span12" style="height:auto;">
                                       <div class="pull-left mr">
                                            <span class="glyphicons shield"><i></i><span class="txt">{$translate.normal_report_short}</span></span>
                                            <span class="count {if $work_hours.monthly_nomal gt $contract_hours.monthly_nomal}overflow_contract{else if $work_hours.monthly_nomal lt $contract_hours.monthly_nomal}underflow_contract{/if}">{$work_hours.monthly_nomal}h {if $contract_hours.monthly_nomal neq 0}<span style="color: green;">({$contract_hours.monthly_nomal}h)</span>{/if}</span>
                                       </div>
                                        <div class="pull-left">
                                            <span class="glyphicons shield"><i></i><span class="txt">{$translate.oncall_report_short}</span></span>
                                            <span class="count {if $work_hours.monthly_oncall gt $contract_hours.monthly_oncall}overflow_contract{else if $work_hours.monthly_oncall lt $contract_hours.monthly_oncall}underflow_contract{/if}">{$work_hours.monthly_oncall}h {if $contract_hours.monthly_oncall neq 0}<span style="color: green;">({$contract_hours.monthly_oncall}h)</span>{/if}</span>
                                        </div>
                                    </a>
                                </div>
                            </div>
                        ...
                        <div class="panel-body span12" >
                                ...
                                <div style="" class="widget-header span12">
                                    ...
                                    <div class="span5" style="border-left: 1px solid rgb(204, 204, 204); margin-left: 0px; padding-left: 10px;">
                                        <h1 class="heading-form">{$translate.preview}</h1>
                                    </div>
                                </div>
                                    <div class="row-fluid">
                                        <div class="span12" style="margin: 5px 0px 0px;">
                                            ...
                                            <div class="form-group span5 mt text-center" style="border-left: 1px solid rgb(204, 204, 204); margin-left: 0px; padding-left: 10px;">
                                                {if $privileges_general.use_template eq 1}
                                                    <button type="button" class="btn btn-success btn_tmplate_apply {if !$employee_last_template_id}hide{/if}"  style="margin:10px 0 0 0 !important " onclick="applySchedule()"> {$translate.preview}</button>
                                                    <span class="ml emp_last_template_date_block {if !$employee_last_template_id}hide{/if}" style="vertical-align: -moz-middle-with-baseline; vertical-align: -webkit-baseline-middle; font-size: 11px;"><i>[{$employee_schedule_main_details.from_date} <strong>{$translate.to}</strong> {$employee_schedule_main_details.to_date}]</i></span>
                                                {/if}
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="span12">
                    <table style="margin:0;" class="table table-bordered table-white table-responsive table-primary table-Anställda slot-calender slot-calender-employee">
                        ...
                        <thead class="no-print">
                            <tr>
                                <th style="" class="table-col-center"><span class="btn btn-block btn-default span12" onclick="navigatePage('{$url_path}month/gdschema/employee/{$selected_year-1}/{$selected_month}/{$selected_employee}/',1);"  title="{$translate.tltp_goto_previous_year}"><i class='icon-double-angle-left icon-large '></i></span></th>
                                <th class="table-col-center"><span class="btn btn-block btn-default span12" onclick="navigatePage('{$url_path}month/gdschema/employee/{$prv_year}/{$prv_month}/{$selected_employee}/',1);" title="{$translate.tltp_goto_previous_month}"><i class=' icon-angle-left icon-large '></i></span></th>
                                <th class="table-col-center" colspan="4"><span class="btn btn-default calender-today" onclick="navigatePage('{$url_path}month/gdschema/employee/{$today_year}/{$today_month}/{$selected_employee}/',1);" title="{$translate.tltp_goto_todays_month}"><i></i>{$translate.today}</span></th>
                                <th class="table-col-center"><span class="btn btn-block btn-default span12" onclick="navigatePage('{$url_path}month/gdschema/employee/{$next_year}/{$next_month}/{$selected_employee}/',1);" title="{$translate.tltp_goto_next_month}"><i class=' icon-angle-right icon-large '></i></span></th>
                                <th class="table-col-center"><span class="btn btn-block btn-default span12" onclick="navigatePage('{$url_path}month/gdschema/employee/{$selected_year+1}/{$selected_month}/{$selected_employee}/',1);" title="{$translate.tltp_goto_next_year}"><i class='icon-double-angle-right icon-large '></i></span></th>
                            </tr>
                        </thead>
                    </table>
                        ...
                        <tbody>
                            {foreach $month_weeks as $month_week}
                                <tr class="gradeX expandable week_row "  style="height: 150px;">
                                    <td class="week_no_td" data-yearweek='{$month_week.week.year}|{$month_week.week.week}' style="background:#f6fccf !important;">
                                        <ul class="calender-col-header span12" id="toggle-view" style="border-bottom: none; background: none;">
                                            <li class="no-print hide-small-devices" title="{$translate.tltp_mini_maxi_week}"><input class="btn-mini btn-default btn-collapse-table-row btn-mini-employee" type="button" value="-" /></li>
                                            <li onclick="navigatePage('{$url_path}employee/gdschema/{$selected_year}|{$month_week.week.week}/{$selected_employee}/',1);" title="{$translate.tltp_goto_week}" class=" cursor_hand">{$month_week.week.week}</li>
                                    ...
                                    {foreach $month_week.days as $week_day}
                                        ...
                                        {else}
                                            <td class="monthly_day{if $week_day.date eq $today_date} selected_date{/if} clearfix" data-date='{$week_day.date}' {*ondragover="allowDrop(event)" ondrop="drop(event)"*}>
                                                <ul class="calender-col-header span12 monthlyslot_date calender-col-header-employee  clearfix">
                                                    <li onclick="navigatePage('{$url_path}gdschema_alloc_window_employee.php?date={$week_day.date}&employee={$selected_employee}',1);" title="{$translate.enter_into_day_slots}" class=" cursor_hand"{if (($week_day.date|date_format:'%u') eq 7) || ($week_day.date|in_array:$holidays)} style="color:red; font-weight: bold;"{else if ($week_day.date eq $today_date)} style="font-weight: bold;"{/if}>{if $week_day.date eq $today_date}{$week_day.day} {$translate.today}{else}{$week_day.day}{/if}</li>
                                                    ...
                                                    {if $employee_running_tasks|count gt 0 and $today_date eq $week_day.date}
                                                        ...
                                                        {/foreach}
                                                        <li class="span3 pull-right pt no-print"><span class="cursor_help" title="{$employee_running_task_title}"><img src='{$url_path}images/clock.gif'/></span></li>
                                                    {/if}
                                                    <li onclick="navigatePage('{$url_path}gdschema_day_employee.php?date={$week_day.date}&employee={$selected_employee}&action=1',1)" class="span3 pull-right pt no-print" title="{$translate.timeline_view}"><i class="icon-bar-chart cursor_hand"/></li>
                                                </ul>
...
{*                                                        <div class="clearfix"></div>*}
                                                            <span class="collapse-slot clearfix" id="slot_thread_{$slot.id}">
                                                                <div class="slot monthlyslotview span12 {if $slot.status eq 2}slot-theme-leave{elseif $slot.status eq 4}slot-theme-candg{elseif $slot.status eq 0 or $slot.status eq 3}slot-theme-incomplete{elseif $slot.status eq 1 and $slot.created_status eq 1}slot-theme-candg-accept{elseif $slot.type eq 10}slot-theme-pm{else}slot-theme-complete{/if} {if $slot.signed eq 1}signed_slot{/if}{if $swap_copied_slot eq $slot.id} objblink{/if} {if $login_user_role neq 3 or ($login_user_role eq 3 and $slot.employee eq $login_user)}monthlyslotview-draggable{/if} {if $slot.show_details neq 1}donot_show_slot{/if}"  
                                                                     onmouseover="tooltip.pop(this, '#slot_details_{$slot.id}', { position:0, offsetX:0, offsetY:20, effect:'slade'{*, overlay:true*} });" data-slot-id="{$slot.id}" >
                                                                    ...
                                                                    <div class="{if $slot.signed eq 1}striped{/if} span12 slot_bountery">
                                                                        {if in_array($slot.type,array(3,9,13,14,17))}<div class="slot-icon-small-oncall small-icon"></div>{/if}
                                                                        ...
                                                                        <div class="notification-info-customer">{$slot.slot} ({$slot.slot_hour}){if trim($slot.cust_name) neq ''}<br/><span class='spn-slot-emp-name' title="{htmlspecialchars($slot.cust_name)}" style="margin-left: 0px;text-overflow: ellipsis;overflow: hidden;">{$slot.cust_name}</span>{/if}</div>
                            ...
                            <div class="row-fluid" style="margin-bottom: 5px;">
                                <div class="span12">
                                    <button type="button" class="btn btn-default-special span12 btn-default-special-employee btn-large" id="show-memory-slots-btn">
                                        <span class="icon-level-down"></span> {$translate.memory_slots}</button>
                                </div>
                            </div>
                            ...
                            <div class="widget-body" style="padding:5px;">
                                <div class="row-fluid">
                                    <div class="span8 customer-name mt no-min-height" style="margin-left:5px;"> 
                                        <span style="margin-right: 5px;" class="icon-group"></span>{$employee_details.first_name|cat: ' '|cat:$employee_details.last_name}
                                    </div>
                                    ...
                                    <div class="span12" style="margin-left: 0px;">
                                        <div class="input-prepend date hasDatepicker datepicker" id="dtPickerNewSlotDate">
                                            <span class="add-on icon-calendar"></span>
                                            <input class="form-control span12 slot_date" id="new_slot_date" placeholder="{$translate.date}" type="text"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div id="memory-slots">
                        <div class="row-fluid">
                            <div class="span12 no-ml">
                                <button type="button" class="btn btn-default-special span12 btn-large btn-default-special-employee" id="create-slot"><span class="icon-level-down"></span> {$translate.click_to_add_new_time_slot}</button>
                            </div>
                        </div>
                        ...
                        <div class="row-fluid no-ml">
                            <div class="span12 no-ml">
                                <button type="button" class="btn btn-default-special span12 mb btn-default-special-employee" onclick="multipleMemorySlotAdd();"><span class="icon-plus"></span> {$translate.add_multiple_timeslots}</button>
            ...
            <div id="slot_details_main_wraper_group" class="hide">
                <div class="slot-wrpr span12" id="slot-wrpr-slots">
                    ...
                    <div class="span12" style="margin:0;">
                        <div class="input-prepend">
                            <span class="add-on  icon-time " title="{$translate.time}"></span>
                            <input class="form-control span5" id="sdTFrom" placeholder="{$translate.from}" type="text" style="border-top-right-radius: 0px; border-bottom-right-radius: 0px; margin-right: -1px;"/>
                            <span class="add-on">{$translate.to}</span>
                            <input class="form-control span5" id="sdTTo" placeholder="{$translate.to}" type="text" style="margin-left: -1px;"/>
                        </div>
                    </div>
                    <h2 class="span12 no-mb"><span class="icon-group" title="{$translate.employee}"></span> 
                        <span id="sdEmployee">{$employee_details.first_name|cat: ' '|cat:$employee_details.last_name}</span>
                        <input class="this_slot_employee_id" id="sdEmployeeID" type="hidden" value="{$selected_employee}"/>
                    </h2>
                    <div class="span12" style="margin-left: 0px;">
                        <div class="input-prepend span11">
                    ...
                    <div class="span12 slots-full-view-body" style="overflow-y: auto; padding-right: 5px !important;">
                        <div id="goto-customers-list" class="row-fluid span12" style="padding-bottom: 8px !important; padding-right: 4px; margin-left: 0;">
                            {foreach $righclick_customers_for_goto AS $rcust}
                                <div style="margin-left: 0px;" class="span12">
                                    <div style="margin-top: 4px;" class="span12 child-slots">
                                        <label onclick="navigatePage('{$url_path}month/gdschema/{$selected_year}/{$selected_month}/{$rcust.username}/',1);">
...
{block name='script'}
{*    <script src="{$url_path}js/plugins/calendars/fullcalendar/fullcalendar/fullcalendar.js"></script><!-- Calendar Plugin -->*}
{*    <script src="{$url_path}js/demo/calendar.js"></script><!-- Calendar Page Demo Script -->*}
 ...
        }
    ...
    <script type="text/javascript">
        ...
        function reload_content(data_values){
            ...
            navigatePage('{$url_path}month/gdschema/employee/{$selected_year}/{$selected_month}/{$selected_employee}/',1, passing_data_object, true, _fn_callbak);
        }
        ...
        {if $privileges_gd.add_slot eq 1}
            function clone_relation_leave_slot(){
                bootbox.dialog( '{$translate.confirm_clone_leave_relation}', [{
                        ...
                        "callback": function() {
                            var slot_id = $('#slot_details_main_wraper_group #sdID').val();
                            var url = '{$url_path}ajax_alloc_action.php?employee={$selected_employee}&gd_year={$selected_year}&gd_month={$selected_month}&req_from=monthly_view_employee&slotid='+slot_id+'&action=clone_leaveslot';
                            navigatePage(url, 1);
                        }
                }]);
             }
...
{*    add new slot functions*}
    <script>
                ...
                $("#monthlyviewtbl td.monthly_day").droppable({
                        ...
                        drop: function( event, ui ) {
                            ...
                            //slot dragging
                                ...
                                var url_atl = 'date='+target_date+'&employee={$employee}&customer='+selected_customer+
                                    '&id='+source_data_slot+'&action=drop&type_check=15';
                                var data_post = 'slot_id='+source_data_slot+'&to_date='+target_date+'&action=drop&pYear={$selected_year}&pMonth={$selected_month}&pEmployee={$selected_employee}&request_from=gd_monthly_view_employee&return_type=json';
                                base_url = '{$url_path}ajax_alloc_action_month.php?'+data_post;
        ...
        function get_slot_add_theme(){
            var slot_theme = '<div class="slot-wrpr span12 time_slots_theme" id="slot-wrpr-month" style="margin-bottom:5px !important;">\n\
                                ...
                                <div class="span12" style="margin-left: 0px;">\n\
                                    <div class="input-prepend">\n\
                                        <span class="add-on  icon-time " title="{$translate.time}"></span>\n\
                                        <input class="form-control span5 custom_slot slot_from" id="new_slot_from" name="slot_from" value="" placeholder="{$translate.from}" type="text"  style="border-top-right-radius: 0px; border-bottom-right-radius: 0px; margin-right: -1px;"/>\n\
                                        <span class="add-on">{$translate.to}</span>\n\
                                        <input class="form-control span5 custom_slot slot_to" id="new_slot_to" name="slot_to" value="" placeholder="{$translate.to}" type="text"  style="margin-left: -1px;"/>\n\
                                    </div>\n\
                                </div>\n\
                                <div class="span12" style="margin-left: 0px;">\n\
                                    <div class="input-prepend span11">\n\
                                        ...
                                        <select id="custom_slot_customer" name="custom_slot_customer" class="form-control custom_slot_customer span12">\n\
                                            <option value="">{$translate.select}</option>\n\
                                            {foreach from=$non_signed_customers_of_employee item=cust}\n\
                                                <option value="{$cust.username}">{$cust.last_name} {$cust.first_name}</option>\n\
                                            {/foreach}\n\
                                        </select>\n\
                                    </div>\n\
            ...
            if(slot_date == ''){
                ...
                $( '.add-new-slots-month .create-slotes-panel .time_slots_theme' ).each(function( index ) {
                    ...
                    if(tmp_slot_from !== false && tmp_slot_to !== false){
                        ...
                        var temp_obj = { 'time_from': tmp_slot_from, 'time_to': tmp_slot_to, 'employee': '{$selected_employee}', 'customer': tmp_slot_customer};
        ...
        function manEntry_proceed(data_obj){
            ...
            var url_atl = 'date='+slot_date+'&employee={$selected_employee}&customer=&emp_alloc={$login_user}&action=man_slot_entry&sub_action=multiple_add&type_check=18';
            if(weekly_past_value){
    ...
    <script>
                            ...
                            switch(key){
                                ...
                                case "delete_slot":
                                   if(ids != ''){
                                        ...
                                        var url_post_data = { 'ids': ids, 'action' : 'multiple_slots_remove', 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}' };
                                        bootbox.dialog( '{$translate.confirm_delete_slot}', [{
                                                "label" : "{$translate.no}",
                                                "class" : "btn-danger"
                                            }, {
                               ...
                               case 'delete_customer':
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': 'delete_customers', 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}' };
                               ...
                               case 'delete_employee':
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': 'delete_employees', 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}' };
                               ...
                               case 'change_tu':
                                   ...
                                   if(ids != ''){
                                       var urls = '{$url_path}ajax_right_click_actions.php';
                                       var url_post_data = { 'ids': ids, 'action': slot_fkkn_change, 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}'};
                                            ...
                                            if((slot_type_change == 12 || slot_type_change == 13) && mixed_normal_oncall_types == true){
                                                ...
                                                if(slot_type_change == 12) //complementary
                                                    ...
                                                    bootbox.dialog( prompt_msg, [{
                                                                    ...
                                                                    $.ajax({
                                                                        ...
                                                                        success:function(data){
                                                                            if(data == 'success'){
                                                                                ...
                                                                                var url_post_data = { 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}', 'ids': ids, 'action': 'change_type', 'slot_type': slot_type_change };
                                                            ...
                                                            "callback": function() {
                                                                var url = '{$url_path}ajax_right_click_actions.php';
                                                                var url_post_data = { 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}', 'ids': ids, 'action': 'change_type', 'slot_type': slot_type_change, 'normal_oncall_auto_change': true };
                                                                navigatePageWithMaintainScrollPosition(url,1, url_post_data); 
                                                            }
                                                    }]);
                                            }
                        ...
                        {if $privileges_gd.candg_approve eq 1}
                            options.items.candg_approve = { 
                                    ...
                                    callback: function(key, opt){ 
                                            var urls = '{$url_path}ajax_right_click_actions.php';
                                            var url_post_data = { 'ids': ids, 'action': 'slot_approve_candg', 'sel_year': '{$selected_year}', 'sel_month': '{$selected_month}', 'employee': '{$selected_employee}' };
        ...
        {if $privileges_gd.add_employee eq 1 or $privileges_gd.add_customer eq 1}
            ...
            function saveChangeUserMultipleConfirm(){
                ...
                var url = "employee={$selected_employee}&sel_year={$selected_year}&sel_month={$selected_month}&method=1&ids="+selected_slots;
        ...
        function pasteSlotEmp(action_type, date, week_year){
            ...
            if(date == ''){
                if(week_year != ''){
                    var url = '{$url_path}ajax_slot_process.php?date='+week_year+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select';
                    var url_data = 'date='+week_year+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&type_check=8'
                    var atl_req_data = 'date='+week_year+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&type_check=8';
                }else{
                    var year_month = '{$selected_year|cat:'|'|cat:$selected_month}';
                    var url = '{$url_path}ajax_slot_process.php?date='+year_month+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&sub_action=past_in_month'; 
                    var url_data = 'date='+year_month+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&type_check=8&sub_action=past_in_month';
                    var atl_req_data = 'date='+year_month+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&type_check=8&sub_action=past_in_month';
                }
            }
            else{
                var url = '{$url_path}ajax_slot_process.php?date='+date+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&to_single_day='+action_type; 
                var url_data = 'date='+date+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&type_check=8&to_single_day='+action_type;
                var atl_req_data = 'date='+date+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select&type_check=8&to_single_day='+action_type;
            }
            check_atl_warning(atl_req_data, function(this_url){ 
                                wrapLoader("#external_wrapper");
                                $('#div_alloc_action').load(this_url,function(response, status, xhr){ 
                                    uwrapLoader("#external_wrapper"); 
                                    if(action_type == 'TRUE'){
                                        get_day_refresh(date, null, '{$selected_employee}', true);
                                    } 
                                    else
                                        navigatePageWithMaintainScrollPosition('{$url_path}month/gdschema/employee/{$selected_year}/{$selected_month}/{$selected_customer}/',1); });
        ...
        function pasteSlotDayEmp(action_type, date, week_year){
            ...
            if(date == ''){
                if(week_year != ''){
                    var url = '{$url_path}ajax_slot_process.php?date='+week_year+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select_day';
                    var url_data = 'date='+week_year+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select_day&type_check=8'
                    var atl_req_data = 'date='+week_year+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select_day&type_check=8';
                }else
                    return false;
            }
            else{
                var url = '{$url_path}ajax_slot_process.php?date='+date+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select_day&to_single_day='+action_type; 
                var url_data = 'date='+date+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select_day&type_check=8&to_single_day='+action_type;
                var atl_req_data = 'date='+date+'&employee={$selected_employee}&emp_alloc={$login_user}&action=paste_select_day&type_check=8&to_single_day='+action_type;
            }
            check_atl_warning(atl_req_data, function(this_url){ 
                                wrapLoader("#external_wrapper");
                                $('#div_alloc_action').load(this_url,function(response, status, xhr){ uwrapLoader("#external_wrapper"); navigatePageWithMaintainScrollPosition('{$url_path}month/gdschema/employee/{$selected_year}/{$selected_month}/{$selected_customer}/',1); });
                            }, url, "#external_wrapper");
        }
        ...
        $(document).ready(function() {
            ...
            $("#cmb_employee").change(function(){
                ...
                if(selected_employee != ''){
                    navigatePage('{$url_path}month/gdschema/employee/{$selected_year}/{$selected_month}/'+selected_employee+'/',1);
                } else 
                    $('.monthly_strips').html('');
            });
        ...
        $(document).ready(function(){
            ...
            $(".monthPicker").datepicker({
                ...
                //if(ev.viewMode == 'months'){
                    ...
                    navigatePage('{$url_path}month/gdschema/employee/'+year+'/'+month+'/{$selected_employee}/',1);
                //}
            });
        });
    </script>
    ...
    <script>
        ...
        {if $privileges_general.create_template eq 1}
            function saveSchedule(){
                ...
                if(start_date == '')
                    ...
                    var main_obj = { 'employee': '{$selected_employee}', 'start_date': start_date, 'end_date': end_date };

                    wrapLoader('#external_wrapper');
                    $.ajax({
                        url:"{$url_path}ajax_save_monthly_employee_schedule.php",
                        type:"POST",
                        dataType: 'json',
                        data: main_obj,
                        success:function(data){
                            ...
                            if(data.transaction){
                                ...
                                $('.emp_last_template_date_block').html('<i>['+start_date+' <strong>{$translate.to}</strong> '+end_date+']</i>');
        ...
        {if $privileges_general.use_template eq 1}
            function applySchedule(){
                navigatePage('{$url_path}schedule/employee/month/gdschema/{$selected_year}/{$selected_month}/{$selected_employee}/', 1);
            }
        {/if}
    </script>
{/block}