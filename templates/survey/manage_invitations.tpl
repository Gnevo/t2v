...
$(document).ready(function() {
    $('.manage_forms, #tools_rack_group, #tools_rack_user, #tools_rack_survey, .answer_hold, .Surveys_drop_zoneinner').jScrollPane();
...
$(".group_block, .user_block, .tool_srvy").draggable({ revert: true, appendTo: ".tabname_content", helper: 'clone' });
$(".drop_zone").droppable({
        accept: ".formlist_block",
        activeClass: "active",
        hoverClass: "hover",
        drop: function(event,ui){
                if (ui.draggable.hasClass("group_block")){
...
$(".ctrlstrip-close").live("click",function(){
        var this_id = $(this).parent().children('.ctrl-option-inner').children('input').val();
        $(this).parent().remove();
        if ($(this).parent().children('.ctrl-option-inner').hasClass("group_class")){
            ...
            reinitScroll('#tools_rack_group');
        }else if ($(this).parent().children('.ctrl-option-inner').hasClass("user_class")){
                ...
                <div class="invite_body questions">
                    <div class="menubar_title clearfix">
                        <div class="search_click">
                            <div class="menu_title"> Invitations </div>
                            <a class="search_icon"> <span class="icon"></span> </a> 
                        </div>
                        <div class="searchclose_section">
                            <input class="serchbox" type="text" id="invite_search" onemptied="fetch_invite()" oninput="fetch_invite()" data-key-placeholder="placeholder_search" placeholder="Search..." />
                            ...
                            <div class="answer_hold answer-hold-attrib" style="height: 275px">
                                <div class="drop_zone">
                                    {if $show_invitation eq 1}
                                            ...
                                            {/foreach}
                                    {/if}
                                    <div class="drop_zone_inner">Drag & Drop Groups and Individual Users</div>
                                </div>
                            </div>
                        </div>
                        <div class="settings_box clearfix">
                            <a class="save" href="javascript:void(0)" onClick="saveForm(1)"><span class="btn_name">{$translate.save}</span></a>
                            {if $show_invitation eq 1}<a class="delet" href="javascript:void(0)" onClick="saveForm(2)"><span class="btn_name">Delete</span></a>{/if}
                            <a class="add new" href="{$url_path}manage/invitations/"><span class="btn_name">{$translate.new_invitation}</span></a>
                        </div>
                    </div>
                </div>
            </form>
